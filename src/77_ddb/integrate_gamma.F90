...
!scalars
 ...
 integer :: irec, symrankkpt_phon,nbranch,nsppol,ngkkband
...
! *************************************************************************
 ...
 ABI_ALLOCATE(elph_ds%gamma_rpt,(2,nbranch**2,nsppol,nrpt))
 ierr = ABI_ALLOC_STAT
 if (ierr /= 0 ) then
   write (message,'(a)')' out of memory in array elph_ds%gamma_rpt '
   MSG_ERROR(message)
 end if
