...
!!  fullinit, fullinit8 = flags (see notes)
...
!!  sciss, sciss8 = scissor correction (Ha)
...
!!  rprim, sciss, symrel, tnons, tolwfr, typat, usepaw, wtk, xred, zion
!!
!! PARENTS
!!      mblktyp1,mblktyp5
!!
...
#undef ABI_FUNC
#define ABI_FUNC 'cmpar8'
 use interfaces_77_ddb, except_this_one => cmpar8
...
!Arguments -------------------------------
!scalars
 integer,intent(in) :: dimekb,fullinit8,iscf8,ixc8,natom8,nkpt8,nsppo8,nsym8
 ...
 real(dp),intent(in) :: ecut8,kptnr8,pawecutdg8,sciss8,tolwf8
 real(dp),intent(inout) :: ecut,kptnrm,pawecutdg,sciss,tolwfr
!arrays
...
!scalars
 ...
 real(dp) :: ekbcm8,ekbcmp,tol=2.0d-14

! *********************************************************************

!DEBUG
!write(std_out,*)' cmpar8 : rprim=',rprim
!write(std_out,*)' cmpar8 : rprim8=',rprim8
!ENDDEBUG
...
!been inititialized by a ground state input file
 if(fullinit/=0)then
   call chkr8(sciss,sciss8,' sciss',tol)
 else
!  Otherwise, takes the meaningful value
   sciss=sciss8
 end if
...
!Now compare the NC pseudopotential information
 if (usepaw==0) then
   if(dimekb/=0 .and. fullinit/=0 .and. fullinit8/=0 )then
     do ii=1,dimekb
       do itypat=1,ntypat
   ...
   else if(dimekb/=0 .and. fullinit8/=0)then
     do ii=1,dimekb
       do itypat=1,ntypat
...
!Now compare several PAW dataset informations
 if (usepaw==1) then
   if (fullinit/=0 .and. fullinit8/=0) then
     do itypat=1,ntypat
