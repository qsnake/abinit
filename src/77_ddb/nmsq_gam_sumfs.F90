...
!        weights for distance from the fermi surface
         ...
         zgemm_tmp_mat = reshape(h1_mat_el_sq(:,ibeff,:,isppol,ikpt_phon),(/2,elph_ds%nbranch,elph_ds%nbranch/))
...
!        sum over bands in gkq_sum_bands
         do ipert1=1,elph_ds%nbranch
           gkq_sum_bands(1,ipert1,ipert1) = gkq_sum_bands(1,ipert1,ipert1) + pi*sd1*sd2*tmp_mat2(1,ipert1,ipert1)
         end do
