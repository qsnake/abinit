...
!!
!! FUNCTION
!! Compute the electrooptic tensor and the raman tensors
!! of zone-center phonons
!!
!!
!! COPYRIGHT
...
!! prtmbm= if equal to 1 write out the mode by mode decomposition
!!         of the EO tensor
...
!DEBUG
!write(std_out,*)'electrooptic : enter' ; stop
!ENDDEBUG

!rijk(1:3*natom,:,:,:) = mode by mode decomposition of the
!electrooptic tensor
...
& epsinf(3,3)*epsinf(2,1)*epsinf(1,2)
 ...
 if (abs(dtm) < tol6) then
   flag = 0
   write(message,'(a,a,a,a,a,a,a,a,a,a)')&
&   ' electrooptic: WARNING -',ch10,&
