...
!!
!! CHILDREN
!!      destroy_tetra,get_tetra_weight,init_tetra,matr3inv,nullify_tetra
!!
!! SOURCE
...
!Local variables-------------------------------
!scalars
 integer :: ikpt, ikptgs, ib1, isppol, iband, istat
 integer :: nene, ifermi
 integer :: ierr
...
!  fix small window around fermie for tetrahedron weight calculation
   ...
   if (istat /= 0) stop 'ep_fs_weights: error in allocating tmp_eigen '
   ABI_ALLOCATE(tweight,(k_obj%nkpt,nene))
   ABI_ALLOCATE(dtweightde,(k_obj%nkpt,nene))
   istat = ABI_ALLOC_STAT
   if (istat /= 0) stop 'ep_fs_weights: error in allocating tweight,dtweightde'
...
&       max_occ,nene,k_obj%nkpt,tetrahedra,&
&       tweight,dtweightde)
