...
type, public :: atprj_type
  ...
  character(len=fnlen), pointer :: filename(:,:)
end type atprj_type

public :: atprj_nullify
public :: atprj_init
public :: atprj_print
...
!!****f* m_atprj/atprj_nullify
!!
!! NAME
!! atprj_nullify
!!
!! FUNCTION
!! nullify atprj datastructure
!!
!! INPUT
!!
!! OUTPUT
...
subroutine atprj_nullify(t_atprj)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'atprj_nullify'
!End of the abilint section

  implicit none

  type(atprj_type), intent(out) :: t_atprj
  nullify(t_atprj%iatprj_bs)
  nullify(t_atprj%filename)
end subroutine atprj_nullify
!!***
...
!End of the abilint section
  ...
  character(len=4) :: imodestring, iatomstring
...
! for each phonon mode and atom for projection, open a file
  ...
  do imode = 1, 3*natom
    ...
    do iatom = 1, natprj_bs
      ...
      t_atprj%filename(imode,iatom) = trim(outfile_radix)//"_mod"//imodestring//"_iat"//iatomstring
...
!End of the abilint section
  ...
  if(associated(t_atprj%iatprj_bs))  then
    ABI_DEALLOCATE(t_atprj%iatprj_bs)
  end if
  if(associated(t_atprj%filename))  then
