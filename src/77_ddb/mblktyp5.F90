...
!Evaluate the maximal dimensions of arrays
 ...
 msym = 192
 do iddb=1,nddb
   call inprep8(dimekb_tmp,filnam(iddb+1),lmnmax_tmp,mband_tmp,mblktyp,&
...
!    Read the blocks from the input database.
     ...
     choice=1
     do iblok=1,nblok
       call read_blok8(ddb_blk,iblok,nband(1),mpert,&
...
!        Check the wavevector identities
         diff=abs(ddb_blk%qpt(1,iblok1)-ddb_blk%qpt(1,iblok2))
         diff=diff+abs(ddb_blk%qpt(2,iblok1)-ddb_blk%qpt(2,iblok2))
         diff=diff+abs(ddb_blk%qpt(3,iblok1)-ddb_blk%qpt(3,iblok2))
!        DEBUG
!        write(std_out,*),'ddb_blk%qpt(1,iblok1)=',ddb_blk%qpt(1,iblok1)
!        write(std_out,*),'ddb_blk%qpt(1,iblok2)=',ddb_blk%qpt(1,iblok2)
!        write(std_out,*),'ddb_blk%qpt(2,iblok1)=',ddb_blk%qpt(2,iblok1)
!        write(std_out,*),'ddb_blk%qpt(2,iblok2)=',ddb_blk%qpt(2,iblok2)
!        write(std_out,*),'ddb_blk%qpt(3,iblok1)=',ddb_blk%qpt(3,iblok1)
!        write(std_out,*),'ddb_blk%qpt(3,iblok2)=',ddb_blk%qpt(3,iblok2)
!        ENDDEBUG
...
!  DEBUG
!  write(std_out,*)'ddb_blk%qpt',ddb_blk%qpt
...
&   temp,' blocks are merged; the new DDB will have ',nblok,&
&   ' blocks.'
   call wrtout(std_out,message,'COLL')
 end if
...
&     occ,occopt,pawecutdg,rprim,sciss,spinat,symafm,symrel,tnons,tolwfr,&
&     tphysel,tsmear,typat,usepaw,wtk,xred,zion,znucl) 
     call psddb8 (choice,dimekb,ekb,fullinit,indlmn,lmnmax,&
&     nblok,ntypat,ddbuntmp,pawtab,pspso,usepaw,useylm,vrsddb)
     do iblok=1,nblok
       if(mgblok(ii)==1) then
         call read_blok8(ddb_blk,ii,nband(1),mpert,&
&         msize,nkpt,ddbuntmp,blkval2(:,:,:,:),kpnt(:,:,ii))
         choice=2
         call write_blok8(ddb_blk,ii,choice,nband(1),mpert,&
