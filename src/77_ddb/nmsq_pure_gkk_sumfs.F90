...
&         sd1*sd2*pi * reshape(h1_mat_el_sq(:,ibeff,:,ikpt_phon,isppol),(/2,nbranch,nbranch/))
       end do !ib2
     end do !ib1
!    
...
!    The sum over Fermi surface bands is done here, and fed into (ib1,ib2)=(1,1)
     h1_mat_el_sq(:,1,:,ikpt_phon,isppol) = reshape(gkq_sum_bands,(/2,nbranch**2/))
