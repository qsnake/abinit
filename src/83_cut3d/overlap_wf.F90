...
!! cwave1 = auxiliary wave function, a projector for Wannier 
...
!arrays
 ...
 real(dp),allocatable :: eigen1(:),fofgout(:,:),fofr(:,:,:,:)
 real(dp),allocatable :: fofr1(:,:,:,:),kpgnorm(:),occ1(:)
!no_abirules
...
!DEBUG   ! XG090627 Keep this debugging feature, that avoid a bug in gcc44 on testf.
 write(std_out,*)' overlap_wf : before reading '
!ENDDEBUG
...
!DEBUG   ! XG090627 Keep this debugging feature, that avoid a bug in gcc44 on testf.
 write(std_out,*)' overlap_wf : after reading '
!ENDDEBUG
...
& gbound,gbound,&
& istwfk(ckpt),kg_k,kg_k,mgfft,mpi_enreg,1,ngfft,npw_k,&
& npw_k,n4,n5,n6,3,paral_kgb,tim_fourwf,weight,weight)
 ...
 ABI_DEALLOCATE(cgcband1)
 ABI_DEALLOCATE(denpot)
 ABI_DEALLOCATE(fofgout)
 ABI_DEALLOCATE(fofr)
 ABI_DEALLOCATE(fofr1)
