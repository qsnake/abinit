...
module defs_rectypes
 ...
 use defs_datatypes, only : pawfgr_type
...
! Integer arrays
  integer,pointer :: pspinfo(:,:) !--for any typat:  (momang,typat)=nproj
...
! Real (real(dp)) arrays
  real(dp),pointer :: radii(:,:)   !--Local radii of nl psp
                                  !  part radii(l+1,psp)

  real(dp),pointer :: projec(:,:,:)         !--p(r,lmn,typat), is the
  !product of Y_lm(r)*nlproj_ln(r) calculated on the grid for any psps.

  real(dp),pointer :: mat_exp_psp_nl(:,:,:,:) !--Matrix of exponential
                                                !  of non-local psp
                                                ! mat_exp_psp_nl(nproj,nproj,l+1,psp)

  real(dp),pointer :: eival(:,:,:) !--Matrix of exponential(eigenvalues)
  real(dp),pointer :: eivec(:,:,:,:) !--Matrix of exponential(eigenvectors)
...
! Integer arrays
  integer,pointer ::  displs(:) !--For mpi
  integer,pointer ::  vcount(:)
...
! Real (real(dp)) scalar
  ...
  real(dp),pointer :: ZT_p(:,:)  !--Is the Fourier transform of the
                                      !  green kernel
