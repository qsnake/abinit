...
! Integer scalar
  ...
  real(dp), pointer :: gam_eig2nkq(:,:,:,:,:,:,:)  ! one half second derivatives of the electronic eigenvalues
 ...
 do idtset=1,ndtset_alloc
   ...
   if(getgam_eig2nkq/=0)then
     ...
     if(dtsets(idtset_gam)%optdriver/=RUNL_RESPFN)then
       write(message, '(4a,i5,a,i5,2a,i5,3a)' )ch10,&
&        ' init_results_respfn : ERROR -',ch10,&
...
&        '  So, it seems that two gamma q point calculations should be stored, while this is not yet allowed.'
 ...
 nullify(results_respfn%gam_eig2nkq)
...
!  write(std_out,*)' associated(results_respfn%gam_eig2nkq)=',associated(results_respfn%gam_eig2nkq)
!  call flush(6)
!ENDDEBUG
   if(associated(results_respfn%gam_eig2nkq))then
