...
!ENDDEBUG
 ...
 if(typevarphys=='DPR' .or. typevarphys=='LEN' .or. &
& typevarphys=='ENE' .or. typevarphys=='BFI'       )typevar='DPR'
...
!end if
 ...
 if(errcod/=0)then
   ...
   write(message, '(8a,i8,a)' ) ch10,&
&   ' inarray : ',ch10,&
&   '  An error occurred reading data for keyword "',trim(cs),'",',&
&   ch10,'    looking for ',narr,' array elements.'
   call wrtout(std_out,message,'COLL')
