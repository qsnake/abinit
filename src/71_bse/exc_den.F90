...
!Local variables ------------------------------
!scalars
 integer :: nfft1,nfft2,nfft3,spin
 integer :: it,itp,ik_ibz,ikp_bz,ik_bz,band,iv,ivp,ic,icp,ir,i1,i2,i3,iik,istat
 integer :: state,nstates,min_idx,eig_unt,den_unt,sden_unt,hsize
...
!allocate and load wavefunctions in real space
 ABI_ALLOCATE(wfr,(nfftot*Wfd%nspinor,BSp%nbnds,Wfd%nkibz))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,"out of memory: exc_den, wfr")
 ...
 ABI_ALLOCATE(exc_ene_cplx,(nstates))
 ...
 ABI_ALLOCATE(exc_vec,(hsize))
 read(eig_unt) exc_vec

 close(eig_unt)
 
 ABI_ALLOCATE(wfrk,(nfftot,BSp%nbnds))
 istat = ABI_ALLOC_STAT
 if(istat/=0) stop 'out of memory: exc_den, wfrk'
...
!calculate ground state density
 ...
 do ik_bz = 1, BSp%nkbz
   ...
   if (iik==1) then
     do ir=1,nfftot
       wfrk(ir,1:BSp%homo) = wfr(ktabr(ir,ik_bz),1:BSp%homo,ik_ibz)
     end do
   else
     do ir=1,nfftot
       wfrk(ir,1:BSp%homo) = conjg(wfr(ktabr(ir,ik_bz),1:BSp%homo,ik_ibz))
     end do
   end if
