...
!scalars
 ...
 integer :: nspinor,nsppol,ISg,ios,mpi_err,tmp_size
 ...
 integer :: iv,ivp,ic,icp,jj,nrows,istat,sender,my_ncols
...
!arrays
 ...
 integer(XMPI_OFFSET_KIND),allocatable :: offset_of_block(:)
#endif
...
!************************************************************************
 ...
 ABI_ALLOCATE(igfftg0,(BSp%npweps))
 ABI_ALLOCATE(ktabr_k,(nfftot_osc))
 ABI_ALLOCATE(ktabr_kp,(nfftot_osc))
 !
 ! Workspace arrays for wavefunctions and oscillator matrix elements.
 ABI_ALLOCATE(rhxtwg_vpv,(BSp%npweps))
 istat = ABI_ALLOC_STAT
 ABI_ALLOCATE(rhxtwg_cpc,(BSp%npweps))
 istat = ABI_ALLOC_STAT

 ABI_ALLOCATE(ur_ckp,(nspinor*nfftot_osc))
 ABI_ALLOCATE(ur_vkp,(nspinor*nfftot_osc))
 ABI_ALLOCATE(ur_ck ,(nspinor*nfftot_osc))
 ABI_ALLOCATE(ur_vk ,(nspinor*nfftot_osc))

 if (Wfd%usepaw==1) then 
   ABI_ALLOCATE(Cp_vk,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_vk,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_ck,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_ck,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_ckp,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_ckp,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_vkp,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_vkp,0,Wfd%nlmn_atm)

   ABI_ALLOCATE(Cp_tmp1,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_tmp1,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_tmp2,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_tmp2,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_tmp3,(Wfd%natom,nspinor))
   call cprj_alloc(Cp_tmp3,0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cp_tmp4,(Wfd%natom,nspinor))
...
&      ". Writing coupling excitonic Hamiltonian on file "//TRIM(fname),"; file size= ",two*dpc*tot_nels*b2Gb," [Gb]."
 ...
 if (use_mpiio) then
   ...
   ABI_ALLOCATE(offset_of_block,(nblocks))
 ...
 do block=1,nsppol
   ...
   ABI_ALLOCATE(t_start,(0:nproc-1))
   ABI_ALLOCATE(t_stop,(0:nproc-1))
   call xmpi_split_work2_i8b(nels,nproc,t_start,t_stop,msg,ierr) 
   if (ierr/=0) then 
     MSG_WARNING(msg)
   end if

   ABI_ALLOCATE(hsize_of,(0:nproc-1))
   hsize_of=0
   do rank=0,nproc-1
...
&     ' Calculating coupling blocks. Memory required: ',bsize_my_block*b2Mb,' Mb. '
   ...
   ABI_ALLOCATE(my_bsham,(t_start(my_rank):t_stop(my_rank)))
   istat = ABI_ALLOC_STAT
...
&        " Calculating direct Coulomb term for (spin1, spin2) ",spin1,spin2," using diagonal approximation for W_{GG'} ..."
     ...
     ABI_ALLOCATE(ctccp,(BSp%npweps))
     ABI_ALLOCATE(vc_sqrt_qbz,(BSp%npweps))

#ifdef DEV_MG_DEBUG_MODE
     ABI_ALLOCATE(ttp_check,(BSp%nreh(block),BSp%nreh(block)))
     ttp_check=0
#endif
     ...
     do ikp_bz=1,BSp%nkbz ! Loop over kp  
       ...
       do ik_bz=1,ikp_bz ! Loop over k
         ...
         if (Wfd%usepaw==1.and.ik_bz/=ikp_bz) then
           ABI_ALLOCATE(Pwij_q,(Cryst%ntypat))
...
&                dim_rtwg,rhxtwg_cpc,tim_fourdp,Wfd%MPI_enreg)
...
&                     dim_rtwg,rhxtwg_vpv,tim_fourdp,Wfd%MPI_enreg)
...
&                      Gsph_c%gvec,Cp_vkp,Cp_vk,Pwij_q,rhxtwg_vpv)
     ...
     ABI_DEALLOCATE(ttp_check)
#endif

     ABI_DEALLOCATE(ctccp)
   ...
   if (do_exchange_term) then
     ...
     ABI_ALLOCATE(rhotwg2,(BSp%npweps))
     ABI_ALLOCATE(vc_sqrt_qbz,(BSp%npwvec))
     ...
     if (iq_ibz==1) then 
       do ig=1,BSp%npwvec
         ISg = Gsph_Max%rottb(ig,itim_q,isym_q) 
         vc_sqrt_qbz(ISg)=Vcp%vcqlwl_sqrt(ig,1)
       end do
     else 
...
&        " BSE matrix cannot be written with a single MPI-IO call. "
     ...
     neh2=BSp%nreh(block)
     ABI_ALLOCATE(bsize_frecord,(neh2))
     ...
     do sender=1,nproc-1
       ...
       if (my_rank==master)  then
         ABI_ALLOCATE(buffer,(hsize_of(sender)))
       end if
       ...
       if (my_rank==master) then
         if (prev_nrows>0) then ! backspace the file if the last record written was not complete.
           ...
           backspace(bsh_unt)
           ABI_ALLOCATE(prev_col,(prev_nrows))
   ...
   ABI_DEALLOCATE(t_start)
   ABI_DEALLOCATE(t_stop)
   ABI_DEALLOCATE(hsize_of)
 end do ! block
 !
 ! ===========================================
 ...
 if (nsppol==2) then
   ...
   ABI_ALLOCATE(col_start,(0:nproc-1))
   ABI_ALLOCATE(col_stop,(0:nproc-1))
   ...
   if (use_mpiio) then
     ...
     do sender=1,nproc-1
       if (my_rank==master)  then
         ABI_ALLOCATE(buffer_2d,(neh1,ncols_of(sender)))
       end if
 ...
 if (use_mpiio) then
   ...
   ABI_DEALLOCATE(offset_of_block)
#endif
 end if
 ...
 ABI_DEALLOCATE(ktabr_k)
 ABI_DEALLOCATE(ktabr_kp)
 ABI_DEALLOCATE(rhxtwg_vpv)
 ABI_DEALLOCATE(rhxtwg_cpc)
 ABI_DEALLOCATE(ur_ckp)
 ABI_DEALLOCATE(ur_vkp)
 ABI_DEALLOCATE(ur_vk)
 ABI_DEALLOCATE(ur_ck)
 !
 ! * Deallocation for PAW.
 if (Wfd%usepaw==1) then
   call cprj_free(Cp_vk)
   ABI_DEALLOCATE(Cp_vk)
   call cprj_free(Cp_ck)
   ABI_DEALLOCATE(Cp_ck)
   call cprj_free(Cp_ckp)
   ABI_DEALLOCATE(Cp_ckp)
   call cprj_free(Cp_vkp)
   ABI_DEALLOCATE(Cp_vkp)
   call cprj_free(Cp_tmp1)
   ABI_DEALLOCATE(Cp_tmp1)
   call cprj_free(Cp_tmp2)
   ABI_DEALLOCATE(Cp_tmp2)
   call cprj_free(Cp_tmp3)
   ABI_DEALLOCATE(Cp_tmp3)
   call cprj_free(Cp_tmp4)
   ABI_DEALLOCATE(Cp_tmp4)
 end if
