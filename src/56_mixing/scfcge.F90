...
!DEBUG
!write(std_out,*)' scfcge: before three mutually exclusive parts'
!ENDDEBUG
...
&       ch10,'  Some might be relevant. Otherwise, raise nband.'
       return
     end if
...
!          Take care of vtrial
           f_fftgr(ifft,isp,1)=vtrial(ifft,isp)

!          Take care of vresid
           temp=f_fftgr(ifft,isp,i_vresid(2))
           f_fftgr(ifft,isp,i_vresid(2))=f_fftgr(ifft,isp,i_vresid(2))&
&           +ratio*(f_fftgr(ifft,isp,i_vresid(1))-f_fftgr(ifft,isp,i_vresid(2)))&
&           +gamma*(f_fftgr(ifft,isp,i_vresid(3))-f_fftgr(ifft,isp,i_vresid(2)))
           f_fftgr(ifft,isp,i_vresid(3))=temp

!          Take care of rhor
           temp=f_fftgr(ifft,isp,i_rhor(2))
           f_fftgr(ifft,isp,i_rhor(2))=f_fftgr(ifft,isp,i_rhor(2))&
&           +ratio*(rhor(ifft,isp)-f_fftgr(ifft,isp,i_rhor(2)))&
&           +gamma*(f_fftgr(ifft,isp,i_rhor(3))-f_fftgr(ifft,isp,i_rhor(2)))
           f_fftgr(ifft,isp,i_rhor(3))=temp

!          Take care of vrespc
           temp=f_fftgr(ifft,isp,i_vrespc(2))
           f_fftgr(ifft,isp,i_vrespc(2))=f_fftgr(ifft,isp,i_vrespc(2))&
&           +ratio*(f_fftgr(ifft,isp,i_vrespc(1))-f_fftgr(ifft,isp,i_vrespc(2)))&
&           +gamma*(f_fftgr(ifft,isp,i_vrespc(3))-f_fftgr(ifft,isp,i_vrespc(2)))
           f_fftgr(ifft,isp,i_vrespc(3))=temp
...
!DEBUG
!if(moved_atm_inside==1)then
!write(std_out,'(a,3es18.8)' )' scfcge : xred1out',xred(:,1)
!write(std_out,'(a,3es18.8)' )' scfcge : xred2out',xred(:,2)
!end if
!ENDDEBUG

!DEBUG
!write(std_out,*)' scfcge : exit '
!ENDDEBUG
