...
!!
!! PARENTS
!!      ep_setupqpt,getshell,inkpts,inqpt,m_ab6_kpoints,m_bz_mesh,m_gamma
!!      nonlinear,refineblk,testkgrid,thmeig
!!
!! CHILDREN
!!      mati3inv,matr3inv,metric,smallprim,smpbz,symkpt,wrtout
!!
!! SOURCE
...
! real(dp) :: cart(3,3)
 ...
 real(dp),allocatable :: kpt_fullbz(:,:),spkpt(:,:),wtk_folded(:),wtk_fullbz(:)
...
!stop
!write(std_out,*)' kptrlen=',kptrlen
!write(std_out,'(a,9i4)' )' kptrlatt=',kptrlatt(:,:)
!write(std_out,'(a,i4)' )' nshiftk=',nshiftk
!do ii=1,nshiftk
!write(std_out,*)' ii, shiftk=',ii,shiftk(:,ii)
!end do
!write(std_out,'(3es16.6)' )rprimd(:,1)
!write(std_out,'(3es16.6)' )rprimd(:,2)
!write(std_out,'(3es16.6)' )rprimd(:,3)
!ENDDEBUG
...
!        DEBUG
!        write(std_out,*)' getkgrid : nshiftk2=',nshiftk2
!        write(std_out,*)' shiftk_pair(1:nshiftk2)',shiftk_pair(1:nshiftk2)
!        ENDDEBUG
...
!          DEBUG
!          write(std_out,*)' klatt2 in cartesian coordinates :'
!          do ii=1,3
!          cart(:,ii)=klatt2(1,ii)*gprimd(:,1)+&
!          &                 klatt2(2,ii)*gprimd(:,2)+&
!          &                 klatt2(3,ii)*gprimd(:,3)
!          write(std_out,'(a,3es16.6)' ) 'cart2(:,ii)=',cart(:,ii)
!          end do
!          write(std_out,*)' klatt3 in cartesian coordinates :'
!          do ii=1,3
!          cart(:,ii)=klatt3(1,ii)*gprimd(:,1)+&
!          &                 klatt3(2,ii)*gprimd(:,2)+&
!          &                 klatt3(3,ii)*gprimd(:,3)
!          write(std_out,'(a,3es16.6)' ) 'cart3(:,ii)=',cart(:,ii)
!          end do
!          ENDDEBUG
!          Back to real space
...
&               ktransf_invt(3,:)*shiftk2(3,jshiftk)
       ...
       end if ! End of the condition for the trial shift to be half a lattice vector
...
!DEBUG
!write(std_out,*)' getkgrid : afer nkshift reduction '
!write(std_out,'(a,9i4)' )' kptrlatt2=',kptrlatt2(:,:)
!write(std_out,'(a,i4)' )' nshiftk2=',nshiftk2
!do ii=1,nshiftk2
!write(std_out,*)' ii, shiftk2=',ii,shiftk2(:,ii)
!end do
!stop
!ENDDEBUG
...
!DEBUG
!write(std_out,*)' getkgrid : will enter metric with rprimd_super',rprimd_super
!write(std_out,*)' getkgrid : nkptlatt=',nkptlatt
!ENDDEBUG
...
!brav=1 is able to treat all bravais lattices.
 ...
 if (PRESENT(kbz_p)) then ! Returns list of k-points in the Full BZ.
   ABI_ALLOCATE(kbz_p,(3,nkpt_fullbz))
   kbz_p(:,:) = spkpt(:,1:nkpt_fullbz)
 end if

!DEBUG
!write(std_out,*)' getkgrid : after smpbz, write spkpt, kptopt= ',kptopt
!do ikpt=1,nkpt_fullbz
!write(std_out,'(i4,3es15.5)' )ikpt,spkpt(:,ikpt)
!end do
!ENDDEBUG
...
!DEBUG
!write(std_out,*)' getkgrid : exit, nkpt_computed= ',nkpt_computed
!write(std_out,*)' getkgrid : exit, kptrlatt= ',kptrlatt
!ENDDEBUG
