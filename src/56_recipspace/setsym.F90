...
!DEBUG
!write(std_out,*)' setsym : before irrzg '
!write(std_out,*)' nspden,nsppol,nsym,ngfft(1),ngfft(2),ngfft(3),nfft',&
!&            nspden,nsppol,nsym,ngfft(1),ngfft(2),ngfft(3),nfft
!do isym=1,nsym
!write(std_out,*)' isym,symafm(isym)=',isym,symafm(isym)
!write(std_out,*)'   tnons(1:3,isym)',tnons(1:3,isym)
!write(std_out,*)'   symrel(1:3,1,isym)',symrel(1:3,1,isym)
!write(std_out,*)'   symrel(1:3,2,isym)',symrel(1:3,2,isym)
!write(std_out,*)'   symrel(1:3,3,isym)',symrel(1:3,3,isym)
!end do
!irrzon(:,:,:)=0
!phnons(:,:,:)=zero
!ENDDEBUG
...
!  Locate irreducible zone in reciprocal space for symmetrization:
   call irrzg(irrzon,nspden,nsppol,nsym,ngfft(1),ngfft(2),ngfft(3),phnons,&
&   symafm,symrel,tnons)
 end if
