...
!!
!! PARENTS
!!      ep_setupqpt,getkgrid,initberry,m_phdos,mkifc9,scphon_qpoint_init
!!      sigma_interp,thm9
!!
!! CHILDREN
...
!  
   if( (ngkpt(1)<=0.or.ngkpt(2)<=0.or.ngkpt(3)<=0) .and.&
&   (ngkpt(1)/=0.or.ngkpt(2)/=0.or.ngkpt(3)/=0) ) then
...
!    To accomodate the shifts, boundmin and boundmax don't start from 0
     boundmin(jj)=-ceiling(maxval(shiftk(jj,:))) !-1
     boundmax(jj)=-floor(minval(shiftk(jj,:))) !0
     do ii=1,3
