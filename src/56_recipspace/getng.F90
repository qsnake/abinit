...
!! option_lob=1 for the old version of lob; 2 for the new version of lob
...
!!
!! PARENTS
!!      invars2m,m_fft_prof,mrgscr,newsp,scfcv,suscep
!!
!! CHILDREN
...
subroutine getng(boxcutmin,ecut,gmet,me_fft,mgfft,nfft,ngfft,nproc_fft,nsym,option_lob,paral_fft,symrel,ngfftc,use_gpu_cuda)
 ...
 use m_fft_mesh,   only : print_ngfft
...
#define ABI_FUNC 'getng'
 ...
 use interfaces_56_recipspace, except_this_one => getng
...
!arrays
 ...
 integer,intent(in),optional :: ngfftc(3),use_gpu_cuda
...
!scalars
 ...
 integer :: plane,testok,tobechecked
...
!  In case of FFT parallelism, one need ngfft(2) and ngfft(3) to be multiple of nproc_fft
   if(paral_fft_==1 .and. (option_lob==1 .or. option_lob==2))then
     msrch(2)=0
     do ii=1,msrch(1)
...
&       '  can handle. The code has to be changed and compiled.'
 ...
 end do !  End of the infinite do-loop : will either "exit", or "call leave_new"
...
!DEBUG
!write(std_out,'(a,3i3,a,a,3i3,a,a,3i3)') &
!& ' getng : divisor=',divisor(:,1),ch10,&
!& '                 ',divisor(:,2),ch10,&
!& '                 ',divisor(:,3)
!write(std_out,*)' tobechecked=',tobechecked
!ENDDEBUG
...
!  DEBUG
!  write(std_out,*)' getng : enter triple loop '
!  write(std_out,*)'imin',imin(1:3)
!  write(std_out,*)'imax',imax(1:3)
!  write(std_out,*)'ngcurrent',ngcurrent(1:3)
!  ENDDEBUG
...
!        DEBUG
!        write(std_out,'(a,3i6,a,i3,a,es16.6)' )' getng : current trial triplet',ngtrial(1:3),&
...
&       '  However, ngfft(3)=',ngfft(3),' and nproc_fft=',nproc_fft
   ...
   write(message,'(a,i5)')'  paral_fft_ should be 0 or 1, but its value is ',paral_fft_
   MSG_BUG(message)
 end if
...
!#if defined HAVE_FFTW3
 if (ngfft(7)==312) then      ! FFTW3 supports leading dimensions but at the price of a larger number of FFTs
   ngfft(4)=2*(ngfft(1)/2)+1  ! to be executed along z when the zero-padded version is used.
   ngfft(5)=2*(ngfft(2)/2)+1  ! One should check whether the augmentation is beneficial for FFTW3.
#if 0
   ngfft(4)=ngfft(1)
   ngfft(5)=ngfft(2)
#endif
...
!#endif
 ...
 call print_ngfft(ngfft,header="FFT mesh",unit=std_out,mode_paral="COLL")
