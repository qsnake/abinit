...
!!
!! PARENTS
!!      denfgr,exc_plot,m_wfs,outnesting,pawmkaewf,spin_current,xc_kernel
!!      xc_kernel_ADA
!!
!! CHILDREN
!!
...
!    DEBUG
!    write(std_out,*)1+n1*(nslice-1),n1+n1*(nslice-1)
!    ENDDEBUG
     nslice=nslice+1
   end do
   nsym=nslice-n2
...
!  DEBUG
!  write(std_out,*)1+n1*(nsym-1),n1+n1*(nsym-1)
!  write(std_out,*)' done xy plane at z = ',nslice-n2
!  ENDDEBUG
 end do
...
!  write(std_out,*)1+n1*(nslice-1),n1+n1*(nslice-1)
 ...
 write (nunit,'(8es16.8)') datagrid(1+n1*(nsym-1):n1+n1*(nsym-1)),datagrid(1+n1*(nsym-1))
!write(std_out,*)1+n1*(nsym-1),n1+n1*(nsym-1)
!write(std_out,*)' done upper xy plane '
