...
&   .and. exp(-mult*dsq(0,0,xx*ngfft(3)))<tol14 ) exit
 end do
...
& ' on the boundary    ', exp(-mult*dsq(ngfft(1),0,0)),ch10, &
& ' no zero            ', count(T_p>tol14),ch10, &
& ' n_green            ', n_green,ch10, &
& ' erreur_n_green     ', exp(-mult*dsq(n_green*ngfft(1),0,0)), &
& exp(-mult*dsq(0,n_green*ngfft(2),0)), &
& exp(-mult*dsq(0,0,n_green*ngfft(3))),ch10,&
