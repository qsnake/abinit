...
!!
!! COPYRIGHT
!! Copyright (C) 2002-2012 ABINIT group (MMancini)
...
! *********************************************************************
 ...
 write(msg,'(3a)')&
      & ' -Load on procs------------',ch10,& 
      & '   me  device        points';call wrtout(std_out,msg,'COLL')
 do icpu=0,rset%mpi%nproc-1
...
&                      rset%mpi%me,gratio,ngfft,proc_pt_dev)
 ...
 call xsum_mpi(recpar%vcount,rset%mpi%commcart,ierr)
...
! *********************************************************************
 ...
 call find_set_gpu(mpi_ab%nproc,mpi_ab%commcart,rset%GPU%map,rset%ngpu)

 !--Get the topology of the machine
 call get_topo(rset%mpi%nproc,rset%ngpu,rset%tp) 
 if(rset%tp>4)then
   write (msg,'(4a)')' m_rec : ERROR- ',ch10,&
     &     ' number of gpu>number of cpu is not implemented',ch10
   call wrtout(std_out,msg,'COLL')
   call leave_new('COLL')
 endif
...
!  rset%ngpu = 0; rset%GPU%map=-1
 ...
 nullify(rset%pawfgr%coatofin)
 nullify(rset%pawfgr%fintocoa)
...
! *********************************************************************
 ...
 if(any(psps%pspcod/=3) .and. nlrec%nlpsp ) then
   write (msg,'(4a)')' vtorhorec : WARNING- ',ch10,&
     &  ' The non-local part of psp is used in Recursion only for hgh-psp',ch10
   call wrtout(std_out,msg,'COLL')
   nlrec%nlpsp = .False.
   if (associated(metrec%gcart))  then
     ABI_DEALLOCATE(metrec%gcart)
   end if
 end if

 nullify(nlrec%mat_exp_psp_nl)
 nullify(nlrec%pspinfo)
 nullify(nlrec%radii)
 nullify(nlrec%indlmn)
 nullify(nlrec%projec)
 nullify(nlrec%eival)
 nullify(nlrec%eivec)
 ...
 if(any(psps%pspcod==3) .and.  nlrec%nlpsp) then
  ...
  if(debug)then
   do jj=1,psps%npsp
    write(msg,'(a)')' Exponential matrices:';call wrtout(std_out,msg,'COLL')
    do ii=1,psps%mpsang
...
! *********************************************************************
 ...
 ! @recursion_type
 ! @pawfgr_type
 if (associated(rset%ZT_p))  then
   ABI_DEALLOCATE(rset%ZT_p)
 end if
 if (associated(rset%pawfgr%fintocoa))  then
   ABI_DEALLOCATE(rset%pawfgr%fintocoa)
 end if
 if (associated(rset%pawfgr%coatofin))  then
   ABI_DEALLOCATE(rset%pawfgr%coatofin)
 end if
 if (associated(rset%par%displs))  then
   ABI_DEALLOCATE(rset%par%displs)
 end if
 if (associated(rset%par%vcount))  then
   ABI_DEALLOCATE(rset%par%vcount)
 end if
 if (associated(rset%nl%mat_exp_psp_nl))  then
   ABI_DEALLOCATE(rset%nl%mat_exp_psp_nl)
 end if
 if (associated(rset%nl%eival))  then
   ABI_DEALLOCATE(rset%nl%eival)
 end if
 if (associated(rset%nl%eivec))  then
   ABI_DEALLOCATE(rset%nl%eivec)
 end if
 if (associated(rset%nl%pspinfo))  then
   ABI_DEALLOCATE(rset%nl%pspinfo)
 end if
 if (associated(rset%nl%radii))  then
   ABI_DEALLOCATE(rset%nl%radii)
 end if
 if (associated(rset%nl%indlmn))  then
   ABI_DEALLOCATE(rset%nl%indlmn)
 end if
 if (associated(rset%nl%projec))  then
   ABI_DEALLOCATE(rset%nl%projec)
 end if
 if (associated(rset%inf%gcart))  then
