...
!!
!! COPYRIGHT
!!  Copyright (C) 2009-2012 ABINIT group (the_author)
...
!Local variables-------------------------------
 ...
 real(dp) :: project,raggio,rloc,denom,step
...
!      tolloc = 1.d0*abs(minval(nlrec%mat_exp_psp_nl(:nlrec%pspinfo(iangol+1,ipsp),:nlrec%pspinfo(iangol+1,ipsp),iangol+1,ipsp)))
       do while(abs(err)>1.d-2)
         raggio = raggio + step
         err = project(raggio,iproj,iangol,rloc)/sqrt(denom)
         ii = ii+1
       end do
...
!            --Calculate the radii
             ...
             partial = project(nrm(rr),iproj,iangol,rloc)*denom
...
!write(msg,'(a,a,a,a,a,a,i6)') ch10,&
!&  ' pspnl_operat_rec: BUG -',ch10,&
