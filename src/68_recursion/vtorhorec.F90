...
!!
!! COPYRIGHT
!! Copyright (C) 2008-2012 ABINIT group (SLeroux,MMancini).
...
!!
!! CHILDREN
!!      calcnrec,cudarec,entropyrec,fermisolverec,gran_potrec,leave_new
!!      nlenergyrec,prtwork,recursion,reshape_pot,symrhg,timab,transgrid,wrtout
!!      xallgatherv_mpi,xbarrier_mpi,xmax_mpi,xredistribute,xsum_mpi
!!
!! NOTES
!!  at this time :
...
!arrays
 ...
 real(dp),intent(out) :: grnl(3*dtset%natom),rhor(nfftf,dtset%nspden)
...
&           tim_fourdp,dtset%natom,projec,1)
 ...
 call xbarrier_mpi(rset%mpi%commcart)
...
!  by any proc on the coarse grid; bufsize_f on the fine grid
   ...
   call xsum_mpi(bufsize,rset%mpi%commcart,ierr)
...
&   rset%mpi%commcart,ierr)
   ...
   call xsum_mpi(vcount_0,rset%mpi%commcart,ierr)
   displs_0 = 0
   ...
   call xsum_mpi(vcount_1,rset%mpi%commcart,ierr)
   displs_1 = 0
...
!  --variables allocation and initialisation-------
   write (msg,'(a)')' - TRANSGRID USING -----';call wrtout(std_out,msg,'COLL')
   call timab(604,1,tsec2) !--start time-counter: transgrid
...
!    --Send on all procs rho,an,bn--
     call xallgatherv_mpi(rholocal(1:rset%par%npt),bufsize(rset%mpi%me),rholoc_2,&
&     bufsize,bufdispl,rset%mpi%commcart,ierr)
     do irec = 0,nrec
       call xallgatherv_mpi(ablocal_1(1:rset%par%npt,irec,1),bufsize(rset%mpi%me),&
&       ablocal_2(:,irec,1),bufsize,bufdispl,rset%mpi%commcart,ierr)
       call xallgatherv_mpi(ablocal_1(1:rset%par%npt,irec,2),bufsize(rset%mpi%me),&
&       ablocal_2(:,irec,2),bufsize,bufdispl,rset%mpi%commcart,ierr)
     end do
...
!    from procs to allprocs
     call xsum_mpi(ablocal_f,rset%mpi%commcart,ierr)
...
!    any procs to obtain the same point as in the standard recursion
   ...
   write(msg,'(a)')' - TRANSGRID NOT USED --'; call wrtout(std_out,msg,'COLL')
 end if
!!--End transgrid
!!###############################################################
!###################################
...
&       gran_pot_v_2(ipoint,4))           
     ...
     do ii1 = 0,dim_entro
       call xallgatherv_mpi(entropy_v_2(:,ii1),bufsize(rset%mpi%me),&
&       entropy_v_c(:,ii1),bufsize,bufdispl,&
&       rset%mpi%commcart,ierr)   
       call xallgatherv_mpi(gran_pot_v_2(:,ii1),bufsize(rset%mpi%me),&
...
&       gran_pot_local3,gran_pot_local4)
     ...
     call xsum_mpi(entropy,rset%mpi%commcart ,ierr)
     call xsum_mpi(gran_pot,rset%mpi%commcart ,ierr)
     if(rset%debug)then
       call xsum_mpi(entropy1,rset%mpi%commcart ,ierr)
       call xsum_mpi(entropy2,rset%mpi%commcart ,ierr)
       call xsum_mpi(entropy3,rset%mpi%commcart ,ierr)
       call xsum_mpi(entropy4,rset%mpi%commcart ,ierr)
       call xsum_mpi(gran_pot1,rset%mpi%commcart ,ierr)
       call xsum_mpi(gran_pot2,rset%mpi%commcart ,ierr)
       call xsum_mpi(gran_pot3,rset%mpi%commcart ,ierr)
       call xsum_mpi(gran_pot4,rset%mpi%commcart ,ierr)
     end if
   end if
...
!--Check if the convergence is reached for rho
 ...
 drhomax = drho
 call xmax_mpi(drho,drhomax,rset%mpi%commcart,ierr)
...
!--Density on all procs
 ...
 if(rset%mpi%nproc /= 1)then
   call xallgatherv_mpi(rholocal,rset%par%ntranche,rhor(:,1),&
...
!--2nd EKIN CALCULATION: this method is used
 noekin2 : if(get_K_S_G==1)then
   ...
   call xsum_mpi(intrhov,rset%mpi%commcart ,ierr)
...
!!  needed to convergence, then redefine nrec.
 if(initialized==0 .and. dtset%ntime>0) call  Calcnrec(rset,b2local)
!--------------------------------------------------------
...
!--Structured debugging : if rset%debug=T, stop here.
 if(.false.)then !(rset%debug)
   ...
   call wrtout(std_out,msg,'COLL')
   write(msg,'(2(a1,a),l1,a)') ch10,' vtorhorec : exit ',&
&   ch10,'  rset%debug=-',rset%debug,', debugging mode => stop '
   call wrtout(std_out,msg,'COLL')
   call leave_new('COLL')
 end if
