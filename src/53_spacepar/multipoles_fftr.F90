...
!!
!! COPYRIGHT
!! Copyright (C) 2003-2012 ABINIT group (MJV, XG)
...
#define ABI_FUNC 'multipoles_fftr'
 ...
 use interfaces_53_spacepar, except_this_one => multipoles_fftr
...
!average over cell
 call mean_fftr(tmpsp,dipole(1,:),mpi_enreg,nfft,nfftot,nspden)
...
!multiply value at each point by xred2
 ...
 ABI_DEALLOCATE(wrapfft)
!average over cell
 call mean_fftr(tmpsp,dipole(2,:),mpi_enreg,nfft,nfftot,nspden)
...
!multiply value at each point by xred3
 ...
 ABI_DEALLOCATE(wrapfft)
!average over cell
 call mean_fftr(tmpsp,dipole(3,:),mpi_enreg,nfft,nfftot,nspden)
