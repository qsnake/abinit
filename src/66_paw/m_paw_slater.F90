...
MODULE m_paw_slater
 ...
 use m_radmesh,      only : calc_slatradl, isame_radmesh, simp_gen, deducer0
 ...
 public :: paw_mkdijexc_core       ! Calculate the onsite matrix element of the Fock operator generated by the core. 
                                   ! between two all-electron partial waves for each type of atom.
 public :: paw_dijhf

 public :: nullify_slatrad
 public :: init_slatrad4
 public :: destroy_slatrad

 interface nullify_slatrad
   module procedure nullify_slatrad_0D
   module procedure nullify_slatrad_1D
 end interface nullify_slatrad
...
!! SOURCE
 ...
 type,public :: slatrad_t
  ...
  integer, pointer :: intgl_select(:)   SET2NULL
  ...
  real(dp), pointer :: intgl(:)   SET2NULL
  ! intgl(1:nintgl)
  ! The integrals:
...
!! SOURCE
 ...
 type,public :: sltrad_cshell_t
  ...
  integer, pointer :: rlphic_select(:,:)   SET2NULL
  ...
  real(dp), pointer :: rlphic_int(:)   SET2NULL
...
!!
!! CHILDREN
!!      destroy_slatrad,init_slatrad4,klmn2ijlmn,print_ij,wrtout
!!
!! SOURCE
...
! *************************************************************************
 ...
 !@slatang_cshell_t
 call nullify_sltang_cshell(Slatang3l)

 ! * Calculate $\Gaunt^{lsl,msl}_{lc,mc;li,mi}$
 lgnt_max = MAX(l_max,lc_max)
 ABI_ALLOCATE(  realgnt,((2*lgnt_max-1)**2*(lgnt_max)**4))
 ABI_ALLOCATE(gntselect,((2*lgnt_max-1)**2, lgnt_max**2*(lgnt_max**2+1)/2))

 call realgaunt(lgnt_max,ngnt,gntselect,realgnt)

 ! Below we need $\Gaunt_{lsl,0}_{lc,0;li,0}$
 ABI_ALLOCATE(g000,(2*lgnt_max-1,lc_max,l_max))
 g000 = zero
 ...
 do klm=1,lm2_size
   ...
   ABI_ALLOCATE(tmp_sgg,(lslat_max*lc_max))
   tmp_sgg = zero
   ...
   if (im == jm .and. il==jl) then
     ...
     ABI_ALLOCATE(Slatang3l(klm)%sggselect,(lslat_max,lc_max))
   ...
   if (nsggsel > 0) then
     ABI_ALLOCATE(Slatang3l(klm)%sgg,(nsggsel))
     Slatang3l(klm)%sgg = tmp_sgg(1:nsggsel)
   end if
   ABI_DEALLOCATE(tmp_sgg)
 end do !klm
...
!!****f* m_paw_slater/nullify_sltang_cshell
!! NAME
!!  nullify_sltang_cshell
!!
!! FUNCTION
!!  Nullify the pointers in a structure of type slatang_cshell_t
!!
!! SIDE EFFECTS
...
subroutine nullify_sltang_cshell(Slatang3l)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_sltang_cshell'
!End of the abilint section
...
! *********************************************************************
 ...
 do ii=1,SIZE(Slatang3l)
   nullify(Slatang3l(ii)%sggselect)
   nullify(Slatang3l(ii)%sgg)
 end do

end subroutine nullify_sltang_cshell
!!***
...
! *********************************************************************
 ...
 !@slatang_cshell_t
 do ii=1,SIZE(Slatang3l)
   if (associated(Slatang3l(ii)%sggselect))   then
     ABI_DEALLOCATE(Slatang3l(ii)%sggselect)
     istat = ABI_ALLOC_STAT
   end if
   if (associated(Slatang3l(ii)%sgg      ))   then
     ABI_DEALLOCATE(Slatang3l(ii)%sgg)
     istat = ABI_ALLOC_STAT
   end if
 end do
...
!!****f* m_paw_slater/nullify_sltrad_cshell
!! NAME
!!  nullify_sltrad_cshell
!!
!! FUNCTION
!!  Nullify the pointers in a structure of type sltrad_cshell_t
!!
!! SIDE EFFECTS
...
subroutine nullify_sltrad_cshell(Slatrad3l)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_sltrad_cshell'
!End of the abilint section
...
! *********************************************************************
 ...
 do ii=1,SIZE(Slatrad3l)
   nullify(Slatrad3l(ii)%rlphic_select)
   nullify(Slatrad3l(ii)%rlphic_int)
 end do

end subroutine nullify_sltrad_cshell
!!***
...
! *********************************************************************
 ...
 !@sltrad_cshell_t
 do ii=1,SIZE(Slatrad3l)
   if (associated(Slatrad3l(ii)%rlphic_select))   then
     ABI_DEALLOCATE(Slatrad3l(ii)%rlphic_select)
     istat = ABI_ALLOC_STAT
   end if
   if (associated(Slatrad3l(ii)%rlphic_int))      then
     ABI_DEALLOCATE(Slatrad3l(ii)%rlphic_int)
     istat = ABI_ALLOC_STAT
   end if
 end do
...
!!
!! INPUTS
!!  itypat=Index defining the type of atom.
!!  ln2_size=Number of symmetrical (l,n) channels
!!  Psps<pseudopotential_type>=Info on pseudopotentials.
!!  Pawrad<pawrad_type>=paw radial mesh and related data for type itypat.
!!  Pawtab<pawtab_type>=paw tabulated starting data for type itypat.
...
subroutine init_sltrad_cshell(Slatrad3l,itypat,ln2_size,Psps,Pawrad,Pawtab,Atm,Atmrad,kln_mask)
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: ln2_size,itypat
!arrays
...
!arrays
 ...
 real(dp),pointer :: phi_i(:),phi_j(:),phicore(:)
 real(dp),allocatable,target :: phi_spl(:,:)
 real(dp),allocatable :: der(:),ypp(:)
 real(dp),pointer :: crad(:),drad(:),phi_in(:)
...
! *************************************************************************
 ...
 !@sltrad_cshell_t
 call nullify_sltrad_cshell(Slatrad3l)
 ...
 call isame_radmesh(Atmrad,Pawrad,hasameq,whichdenser)
...
&    MSG_COMMENT("Pawrad is denser than Atmrad!")
 ...
 if (do_spline==1) then
   ...
   ABI_ALLOCATE(phi_spl,(dmesh_size,ln_size))
   ABI_ALLOCATE(der,(cmesh_size))
   ABI_ALLOCATE(ypp,(cmesh_size))
 ...
 ABI_ALLOCATE(kln2ln,(6,ln2_size))

 call make_kln2ln(lmn_size,lmn2_size,ln2_size,indlmn,indklmn,kln2ln)

 ABI_ALLOCATE(ff1,(dmesh_size))
 ABI_ALLOCATE(ff2,(dmesh_size))
 ...
 do isppol=1,Atm%nsppol
   do kln=1,ln2_size
     ...
     ABI_ALLOCATE(Slatrad3l(kln)%rlphic_select,(lslat_max,lnc_size))
     ...
     ABI_ALLOCATE(tmp_integrals,(lslat_max*lnc_size))
     tmp_integrals(:) = zero
     nintg=0
     ...
     ABI_ALLOCATE(Slatrad3l(kln)%rlphic_int,(nintg))
     if (nintg>0) Slatrad3l(kln)%rlphic_int(:) = tmp_integrals(1:nintg)

     ABI_DEALLOCATE(tmp_integrals)
   end do !kln
 end do !isppol

 ABI_DEALLOCATE(ff1)
 ABI_DEALLOCATE(ff2)
 ABI_DEALLOCATE(kln2ln)

 if (do_spline==1)  then
   ABI_DEALLOCATE(phi_spl)
 end if
...
!!  lmn2_size=Number of (klmn) channels for type itypat.
!!  ndij=Usually ndij=nspden, except for spin-orbit (where ndij=nspinor**2)
!!  Psps<pawrad_type>=paw radial mesh and related data for atom of type itypat
!!  Pawtab<pawtab_type>=paw tabulated starting data for atom of type itypat
...
subroutine paw_sigxcore(itypat,cplex_dij,lmn2_size,ndij,Psps,Pawrad,Pawtab,Atm,Atmrad,dijexc_core)
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: itypat,lmn2_size,cplex_dij,ndij
!arrays
...
! *************************************************************************
 ...
 ABI_ALLOCATE(klm2lm,(6,lm2_size))
 ...
 ABI_ALLOCATE(Slatang3l,(lm2_size))
 call init_slatang_cshell(Slatang3l,l_max,lm2_size,lc_max,klm2lm)

 ABI_DEALLOCATE(klm2lm)

 ! * Integrate radial part.
 ABI_ALLOCATE(Slatrad3l,(ln2_size))

 call init_sltrad_cshell(Slatrad3l,itypat,ln2_size,Psps,Pawrad,Pawtab,Atm,Atmrad)
 ...
 opt_l=-1; pawprtvol=-1
 call print_ij(dijexc_core(:,1),lmn2_size,cplex_dij,lmn_size,1,opt_l,opt_l_index,0,pawprtvol,pack2ij,-one,1)
...
!!  Cryst<Crystal_structure>=Structure describing the crystal structure and its symmmetries.
!!  Psps<pseudopotential_type>=Variables related to pseudopotentials.
!!  Pawtab(ntypat)<pawtab_type>=paw tabulated starting data for atom of type itypat.
!!  Pawrad(ntypat)<pawrad_type>=paw radial mesh and related data for atom of type itypat.
...
subroutine paw_mkdijexc_core(ndij,cplex_dij,lmn2_size_max,Cryst,Psps,Pawtab,Pawrad,dijexc_core,pawprtvol)
...
! *************************************************************************
 ...
 ABI_ALLOCATE(Atm,(Cryst%ntypat))
 ABI_ALLOCATE(Radatm,(Cryst%ntypat))
 ...
 do itypat=1,Cryst%ntypat
   ...
   if (ic>0 .and. ic<LEN_TRIM(string)) fcore = Psps%filpsp(itypat)(1:ic)//TRIM(fcore)
   ...
   call paw_sigxcore(itypat,cplex_dij,lmn2_size,ndij,Psps,&
&    Pawrad(itypat),Pawtab(itypat),Atm(itypat),Radatm(itypat),dijexc_core(1:lmn2_size,:,itypat))
   ...
   ABI_ALLOCATE(phi_indln,(2,ln_size))
   call make_indln(lmn_size,ln_size,Psps%indlmn(:,:,itypat),phi_indln)

   ABI_ALLOCATE(overlap,(Atm(itypat)%ln_size,ln_size))
   isppol=1 ! hardcoded
   call get_overlap(Atm(itypat),Radatm(itypat),Pawrad(itypat),isppol,ln_size,phi,phi_indln,overlap)

   ABI_DEALLOCATE(phi_indln)
 ...
 do itypat=1,Cryst%ntypat
   ...
   ABI_DEALLOCATE(Radatm(itypat)%rad)
   ABI_DEALLOCATE(Radatm(itypat)%radfact)
   ABI_DEALLOCATE(Radatm(itypat)%simfact)
 end do

 ABI_DEALLOCATE(Atm)
 ABI_DEALLOCATE(Radatm)
...
!!****f* m_paw_slater/nullify_slatrad_0D
!! NAME
!!  nullify_sltrad
!!
!! FUNCTION
!!  Nullify the pointers in slatrad_t
!!
!! PARENTS
!!      m_paw_slater
!!
...
subroutine nullify_slatrad_0D(Slatrad)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_slatrad_0D'
!End of the abilint section
...
! *********************************************************************
 ...
 !@slatrad_t
 nullify(Slatrad%intgl_select)
 nullify(Slatrad%intgl)

end subroutine nullify_slatrad_0D
!!***

!----------------------------------------------------------------------

!!****f* m_paw_slater/nullify_slatrad_1D
!! NAME
!!  nullify_sltrad
!!
!! FUNCTION
...
subroutine nullify_slatrad_1D(Slatrad)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_slatrad_1D'
!End of the abilint section
...
! *********************************************************************
 ...
 do ii=1,SIZE(Slatrad)
  call nullify_slatrad_0D(Slatrad(ii))
 end do

end subroutine nullify_slatrad_1D
!!***
...
! *********************************************************************
 ...
 !@slatrad_t
 if (associated(Slatrad%intgl_select))   then
   ABI_DEALLOCATE(Slatrad%intgl_select)
   istat = ABI_ALLOC_STAT
 end if
 if (associated(Slatrad%intgl       ))   then
   ABI_DEALLOCATE(Slatrad%intgl)
   istat = ABI_ALLOC_STAT
 end if
...
subroutine init_slatrad4(Slatrad4,which_intg,itypat,ln2_size,Psps,Pawrad,Pawtab)
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: ln2_size,itypat,which_intg
!arrays
 type(pseudopotential_type),intent(in) :: Psps
 type(pawrad_type),intent(in) :: Pawrad
 type(pawtab_type),intent(in) :: Pawtab
...
!scalars
 ...
 integer :: lmn_size,lmn2_size,ln_size,istat,slt_idx
...
!arrays
 ...
 real(dp),pointer :: phi_i(:),phi_j(:),phi_k(:),phi_l(:)
 real(dp),pointer :: tphi_i(:),tphi_j(:),tphi_k(:),tphi_l(:)
 real(dp),pointer :: shapefunc(:),rad(:)
...
! *************************************************************************
 ...
 ABI_ALLOCATE(uiuj,(mesh_size))
 ABI_ALLOCATE(ukul,(mesh_size))
 ABI_ALLOCATE(ff,(mesh_size))
 ABI_ALLOCATE(tuituj,(mesh_size))
 ABI_ALLOCATE(tuktul,(mesh_size))
 ABI_ALLOCATE(tuituj_tqgl,(mesh_size))
 ABI_ALLOCATE(tuktul_tqgl,(mesh_size))
 rad => Pawrad%rad
 !
 ...
 do sln2=1,ln2_size
   ...
   do sln1=1,sln2 
     ...
     ABI_ALLOCATE(Slatrad4(slt_idx)%intgl_select,(lslat_min:lslat_max))
     ...
     if (lslat_min > lslat_max) then 
       ...
       !MSG_ERROR("")
       ABI_ALLOCATE(Slatrad4(slt_idx)%intgl,(0))
       CYCLE
     end if
     ...
     ABI_ALLOCATE(tmp_integrals,(MAX(lslat_min,lslat_max)))
     tmp_integrals=zero
     nintgl=0
     ...
     !do isl=lslat_min,lslat_max,2  ! TODO Here I can reduce the number of iterations using a step of 2.
       ...
       if (which_intg == 2) then ! Subtract the pseudo part.
         ...
         if (l_slat==0.and.kl==1.and.ll==1) call deducer0(ff,mesh_size,pawrad)
         ...
         if (l_slat==0.and.il==1.and.jl==1) call deducer0(ff,mesh_size,pawrad)
     ...
     ABI_ALLOCATE(Slatrad4(slt_idx)%intgl,(nintgl))
 ...
 ABI_DEALLOCATE(uiuj)
 ABI_DEALLOCATE(ukul)
 ABI_DEALLOCATE(tuituj)
 ABI_DEALLOCATE(tuktul)
 ABI_DEALLOCATE(ff)
 ABI_DEALLOCATE(tuituj_tqgl)
 ABI_DEALLOCATE(tuktul_tqgl)
...
!!
!! OUTPUT
!!  sigx_dij(cplex_dij*lmn2_size_max,ndij,natom)= 
...
subroutine paw_dijhf(ndij,cplex_dij,lmn2_size_max,Cryst,Psps,Pawtab,Pawrad,Pawang,Pawrhoij,sigx_dij,pawprtvol)
...
!arrays
 real(dp),target,intent(out) :: sigx_dij(cplex_dij*lmn2_size_max,ndij,Cryst%natom) !TODO use ragged arrays pawij?
 type(pawtab_type),intent(in) :: Pawtab(Psps%ntypat*Psps%usepaw)
 type(pawrad_type),intent(in) :: Pawrad(Psps%ntypat*Psps%usepaw)
 type(pawrhoij_type),intent(in) :: Pawrhoij(Cryst%natom) 

!Local variables ---------------------------------------
!scalars
 integer,parameter :: cplex=1    ! FIXME preliminary implementation
 integer :: iatom,itypat,lmn_size,lmn2_size,ispden,nspden,ln2_size,lm2_size !,isppol ln_size,
...
! *************************************************************************
 ...
 do iatom=1,Cryst%natom
   ...
   ABI_ALLOCATE(Slatrad4,(ln2_size*(ln2_size+1)/2))
   which_intg=3
   call init_slatrad4(Slatrad4,which_intg,itypat,ln2_size,Psps,Pawrad(itypat),Pawtab(itypat))
   ...
   if (ABS(pawprtvol)>=1) then ! * Print values
     ...
     opt_l=-1
     call print_ij(sigx_atm(:,1),lmn2_size,cplex_dij,lmn_size,1,opt_l,opt_l_index,0,pawprtvol,pack2ij,-one,1)
   end if

   call destroy_slatrad(Slatrad4)
   ABI_DEALLOCATE(Slatrad4)
 end do ! iatom
