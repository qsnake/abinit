...
!!
!! INPUTS
!!  atindx1(natom)=index table for atoms (sorted-->random), inverse of atindx.
...
!!
!! PARENTS
!!      accrho3,dyfnl3,energy,pawmkrhoij,wfd_pawrhoij
!!
!! CHILDREN
!!
!! SOURCE
...
!Arguments ---------------------------------------------
!scalars
 integer,intent(in) :: cplex,ipert,isppol,natom,nspinor,option
 logical,intent(in) :: usetimerev
 real(dp),intent(in) :: occ_k,wtk_k
!arrays
 integer,intent(in) :: atindx1(natom)
 type(cprj_type),intent(in) :: cwaveprj(natom,nspinor),cwaveprj1(natom,nspinor)
 type(pawrhoij_type),intent(inout) :: pawrhoij(natom)

!Local variables ---------------------------------------
!scalars
 integer :: cplex_rhoij,iatm,iatom,ilmn,iplex,j0lmn,jlmn,klmn,klmn_im,klmn_re,mu,ncpgr,nspden_rhoij
 logical :: compute_impart,compute_impart_cplex
 real(dp) :: ro11_im,ro11_re,ro12_im,ro12_re,ro21_im,ro21_re,ro22_im,ro22_re,weight
...
!Tests
 if(option==2.and.(ipert==natom+1.or.ipert==natom+5)) then
   message = ' not relevant for ipert=natom+1 or ipert=natom+5 !'
   MSG_BUG(message)
 end if
 if(option==2.and.(ipert==natom+3.or.ipert==natom+4)) then
   message = ' rhoij^(1) not yet implemented for strain perturbation !'
   MSG_ERROR(message)
 end if
