...
!!
!! CHILDREN
!!      initang,initylmr,leave_new,nderiv_gen,pawlsylm,pawshpfun,poisson,psp7nl
!!      realgaunt,simp_gen,spline,timab,wrtout
!!
!! SOURCE
...
&                  pawang,pawrad,pawspnorb,pawtab,pawxcdev)
 ...
 use m_radmesh,  only : simp_gen, poisson, nderiv_gen
...
#define ABI_FUNC 'pawinit'
 ...
 use interfaces_66_paw, except_this_one => pawinit
...
!Local variables ------------------------------
!scalars
 integer :: basis_size,i0lm,i0ln,ierr,il,ilm,ilmn,iln,iq,isel,isel1
 ...
 logical :: ang_not_allocated
 real(dp) :: dq,gnrm,gsq,intg,ql,ql1,rg,rg1,vh1,yp1,ypn
...
!* REAL GAUNT COEFFICIENTS
 ...
 if (pawang%gnt_option==1) then
   ABI_ALLOCATE(rgnt_tmp,((2*pawang%l_max-1)**2*(pawang%l_max)**4))
   call realgaunt(pawang%l_max,pawang%ngnt,pawang%gntselect,rgnt_tmp)
 else if (pawang%gnt_option==2) then
   ABI_ALLOCATE(rgnt_tmp,((4*pawang%l_max-3)**2*(2*pawang%l_max-1)**4))
 ...
 if (pawxcdev==0) then
   ...
   ang_not_allocated=(.not.associated(pawang%anginit))
   if (ang_not_allocated)  then
     ABI_ALLOCATE(pawang%anginit,(3,pawang%angl_size))
   end if
   call initang(pawang)
   if (associated(pawang%ylmrgr)) then
     option=2;ll=pawang%l_size_max+1
     call initylmr(ll,0,pawang%angl_size,pawang%angwgth,option,pawang%anginit,pawang%ylmr,&
&     ylmr_gr=pawang%ylmrgr)
   else
     option=1;ll=pawang%l_size_max
     call initylmr(ll,0,pawang%angl_size,pawang%angwgth,option,pawang%anginit,pawang%ylmr)
   end if
...
!*******************
 do itypat=1,ntypat
   ...
   ABI_ALLOCATE(indklmn_,(6,lmn2_size))
...
!  *** Remember to set to zero all optional flags ***
!  MG: pawinit is the main creation method for pawtab but it is called only if particular
!  parameters are changed wrt the previous dataset (see scfv). Therefore the following flags
!  have to be initialized in the caller. Remember to check that the flags in pawtab are correctly defined
!  in each PARENT of pawinit anytime a new flag is added. Besides make sure the parent list is updated.
!  Better approaches are possible, though.

!  pawtab(itypat)%has_kij=0
