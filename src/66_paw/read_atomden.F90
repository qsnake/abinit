...
#define ABI_FUNC 'read_atomden'
 use interfaces_66_paw, except_this_one => read_atomden
...
!scalars
 ...
 integer :: igrid,i,i1,i2,i3,io_err,itypat
...
!Initialise various variables
 ...
 if (nspden/=1) STOP 'ERROR in read_atomden: Only nspden=1 allowed.' 
 rho = rhor_paw(:,1)
...
!Calculate the r vector (reduced coord.) of the fine gridpoints
 ...
 if (igrid/=ngrid) STOP 'ERROR in read_atomden: igrid not equal to ngrid'
...
&   '_density_atom_type',itypat,'.dat'
   open(unit=300,file=trim(filename),status='old',action='read',iostat=io_err)
   if (io_err/=0) then
...
&   '_density_atom_type',itypat,'.dat'
   open(unit=300,file=trim(filename),status='old',action='read',iostat=io_err)
   read(300,*) ! Skip comment line
   do i=1,natomgr(itypat)
     read(300,*) atomrgrid(i,itypat),density(i,itypat)
   end do
   close(300)
