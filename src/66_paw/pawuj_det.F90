...
#define ABI_FUNC 'pawuj_det'
 ...
 use interfaces_66_paw, except_this_one => pawuj_det
...
!### 02. Create the file UJDET.nc
 ...
 ncid=0
!XG 20110807 Disable the use of NetCDF with UJDET : problem on Coba2.
!#if defined HAVE_TRIO_ETSF_IO
 if(.false.)write(std_out,*)ujdet_filename ! This is for the abirules
!call create_nc_file(ujdet_filename,ncid)
!#endif
...
!Calculation of response matrix 
 ...
 if (any(abs((/(vsh(ii)-vsh(ii+2), ii=1,2) /))<0.00000001)) then
   write(message, '(2a,18f10.8,a)' )  ch10,' vshift is too small: ',abs((/(vsh(ii)-vsh(ii+2), ii=1,2) /))
   call wrtout(ab_out,message,'COLL')
   return
 end if
...
!END DEBUG
 ...
 if (pawprtvol==3) then
   write(message,fmt='(3a, 150f4.1)') ch10,' Magnetisation',trim(message),magv_org
   call wrtout(std_out,message,'COLL')
 end if
