...
!!
!! CHILDREN
!!      sort_dp,spline,splint,wrtout,xbarrier_mpi,xcomm_init,xmaster_init
!!      xme_init,xsum_master
!!
!! SOURCE
   ...
   do l=l_min(itypat),l_max(itypat)
     do m=m_min(itypat),m_max(itypat)
       do n=n_min(itypat),n_max(itypat)
         do iatom=1,natom
           if (typat(iatom)==itypat) then
             if (i>n_equiv_atoms(itypat)) STOP 'ERROR in atomden: i>n_equiv_atoms'
...
!we are running in parallel
 if (nprocs>1) then
   ...
   call xsum_master(rho_temp,master,spaceComm,ierr)
   call xbarrier_mpi(spaceComm)
   if (prtvol>9) then
