...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2012 ABINIT group (JJ,MT)
...
! ************************************************************************
 ...
 ABI_ALLOCATE(der,(pawrad%mesh_size))
 ABI_ALLOCATE(ypp,(pawrad%mesh_size))
 ABI_ALLOCATE(diag,(pawrad%mesh_size))
 do inl = 1, nnl

!  spline phi onto points
   ypp(:) = zero; diag(:) = zero; ybcbeg = zero; ybcend = zero
   call spline(pawrad%rad,pawtab%phi(:,inl),pawrad%mesh_size,ybcbeg,ybcend,ypp)
...
!  need derivative of phi with respect to radius
   ...
   ypp(:) = zero; diag(:) = zero; ybcbeg = zero; ybcend = zero
   call spline(pawrad%rad,der,pawrad%mesh_size,ybcbeg,ybcend,ypp)
   call splint(pawrad%mesh_size,pawrad%rad,der,ypp,npts,points,dphi(:,inl))

!  next splint tphi onto points
   ypp(:) = zero; diag(:) = zero;
   call spline(pawrad%rad,pawtab%tphi(:,inl),pawrad%mesh_size,ybcbeg,ybcend,ypp)
