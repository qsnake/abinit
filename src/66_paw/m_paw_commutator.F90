...
!!****t* m_paw_commutator/HUr_commutator
!! NAME
!!  HUr_commutator
!!
!! FUNCTION
!!  The HUr_commutator data type stores basic dimensions and quantities 
...
!!  Note that, within the PAW formalism, a standard KS Hamiltonian only has a semi-local contribution 
...
!! SOURCE
 ...
 type,public :: HUr_commutator
  ...
  integer,pointer :: ij_select(:,:,:)     SET2NULL
  ...
  real(dp),pointer :: commutator(:,:,:)   SET2NULL
  ! commutator(3,nsel,nsppol)
 end type HUr_commutator

 public ::  nullify_Hur       
 public ::  make_Hur_commutator
 public ::  destroy_Hur        ! Deallocate the dynamic memory in the HUr_commutator object.
...
!!****f* m_paw_commutator/nullify_Hur
!! NAME
!! nullify_Hur
!!
!! FUNCTION
...
!!****f* m_paw_commutator/destroy_Hur
!! NAME
!! destroy_Hur
!!
!! FUNCTION
...
! *************************************************************************
 ...
 do iat=1,SIZE(Hur)
   if (associated(Hur(iat)%ij_select ))  then
     ABI_DEALLOCATE(Hur(iat)%ij_select)
   end if
   if (associated(Hur(iat)%commutator))  then
     ABI_DEALLOCATE(Hur(iat)%commutator)
   end if
 end do
...
!!****f* m_paw_commutator/paw_ihr_comm
!! NAME
!! paw_ihr_comm
!!
!! FUNCTION
...
!!  Cprj_kb1(natom,nspinor),Cprj_kb2(natom,nspinor) <type(cprj_type)>=
...
function paw_ihr_comm(isppol,nspinor,npw,istwfk,kpoint,Cryst,Pawtab,ug1,ug2,gvec,Cprj_kb1,Cprj_kb2,HUr) result(ihr_comm)
...
!arrays
 ...
 type(Cprj_type),intent(in) :: Cprj_kb1(Cryst%natom,nspinor),Cprj_kb2(Cryst%natom,nspinor)
 type(HUr_commutator),intent(in) :: Hur(Cryst%natom)
...
end function paw_ihr_comm
!!***

!----------------------------------------------------------------------

!!****f* m_paw_commutator/paw_cross_ihr_comm
!! NAME
!! paw_cross_ihr_comm
!!
!! FUNCTION
...
!!
!! INPUTS
!!  ihr_comm = the commutator [H,r] evaluated between states i and j, with only the plane-wave and the onsite parts included
!!  isppol=Spin index.
!!  nspinor=Number of spinori components.
...
!arrays
 real(dp) :: gspace_cart2red(3,3) !rs_cart2red(3,3),
 complex(gwpc) :: ihr_comm_cart(3,nspinor**2)
 complex(dpc) :: dphigr1(3),dphigr2(3)

! *************************************************************************

 ABI_CHECK(nspinor==1,"nspinor + pawcross not coded")
 ...
 do iatom=1,Cryst%natom
   ...
   do ifgd=1,nfgd
     ...
     cross1  = (ur_ae_onsite1(ifftsph)-ur_ae1(ifftsph))
     cross2  = (ur_ae_onsite2(ifftsph)-ur_ae2(ifftsph))

     do ilmn=1,lmn_size

       dphigr1(1:3) = CMPLX(Cprj_kb1(iatom,1)%cp(1,ilmn),Cprj_kb1(iatom,1)%cp(2,ilmn)) * &
&                       (Paw_onsite(iatom)%phi_gr(1:3,ifgd,ilmn) - Paw_onsite(iatom)%tphi_gr(1:3,ifgd,ilmn))
       dphigr2(1:3) = CMPLX(Cprj_kb2(iatom,1)%cp(1,ilmn),Cprj_kb2(iatom,1)%cp(2,ilmn)) * &
&                       (Paw_onsite(iatom)%phi_gr(1:3,ifgd,ilmn) - Paw_onsite(iatom)%tphi_gr(1:3,ifgd,ilmn))

       ihr_comm_cart(1,1)= ihr_comm_cart(1,1) - j_dpc*CONJG(cross1)*dphigr2(1) - j_dpc * CONJG(dphigr1(1))*cross2
       ihr_comm_cart(2,1)= ihr_comm_cart(2,1) - j_dpc*CONJG(cross1)*dphigr2(2) - j_dpc * CONJG(dphigr1(2))*cross2
       ihr_comm_cart(3,1)= ihr_comm_cart(3,1) - j_dpc*CONJG(cross1)*dphigr2(3) - j_dpc * CONJG(dphigr1(3))*cross2
...
!!****f* m_paw_commutator/make_Hur_commutator
!! NAME
!! make_Hur_commutator
!!
!! FUNCTION
!!  Creation method for the HUr_commutator data type.
!!
!! INPUTS
!!
!! OUTPUT
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'make_Hur_commutator'
...
! *************************************************************************
 ...
 ABI_ALLOCATE(rcart_onsite,(3,lmn2_size_max,Cryst%natom))
 ...
 do iatom=1,Cryst%natom
   ...
   ABI_ALLOCATE(rij_tmp,(3,lmn_size**2,nsppol))
   rij_tmp=zero
   ABI_ALLOCATE(vpawu,(2*lpawu+1,2*lpawu+1))
   ...
   ABI_ALLOCATE(Hur(iatom)%ij_select,(lmn_size,lmn_size,nsppol))
   Hur(iatom)%ij_select=0
   nsel(:,:)=COUNT(ABS(rij_tmp)>tol6,DIM=2)
   nmax=MAXVAL(nsel)
   ABI_ALLOCATE(Hur(iatom)%commutator,(3,nmax,nsppol))
   do isppol=1,nsppol
     ij_idx=0
     isel  =0
     do jlmn=1,lmn_size
   ...
   ABI_DEALLOCATE(rij_tmp)
 end do !iatom

 ABI_DEALLOCATE(rcart_onsite)
...
! *************************************************************************
 ...
 do itypat=1,ntypat
   ...
   ABI_ALLOCATE(rc_tmp,(3,lmn2_size))
   rc_tmp=zero
   !
   ! === Loop on (jl,jm,jn) channels
   do jlmn=1,lmn_size
