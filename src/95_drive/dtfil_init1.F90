...
!  std_in=5, ab_in=5, std_out=6, ab_out=7, tmp_unit=9, tmp_unit2=10
 integer,parameter :: unchi0=42,unddb=16,unddk=15,unem1ggp=88,unkg1=19,unkg=17,unkgq=18
 ...
 integer,parameter :: unkss=40,unscr=41,unqps=43,undelI=44

 integer :: ii,iimage,ireadden,ireadkden,ireadwf,ixx,jdtset

!integer :: iexit,iget,ii,iimage,iimage_get
!integer :: ipsp,ireadden,ireadkden,ireadwf
...
!real(dp) :: etotal
 ...
 character(len=fnlen) :: filsus,fildens1in,fildensin,filpawdensin,filkdensin,filkss,filqps,filscr
...
& ndtset,stringfile,stringvar,will_read)
 ...
 ireadden=will_read
 if (dtset%optdriver==RUNL_GSTATE.and.dtset%iscf<0) ireadden=1
...
&   ndtset,stringfile,stringvar,will_read)
   ...
   ireadkden=will_read
   if (dtset%optdriver==RUNL_GSTATE.and.dtset%iscf<0) ireadkden=1
 else
   ireadkden=0
 end if
...
!Build name of files from dtfil%filnam_ds(4)
 ...
 dtfil%fnameabo_ae_wfk=trim(dtfil%filnam_ds(4))//'_AE_WFK'
 ...
 dtfil%fnameabo_delI=trim(dtfil%filnam_ds(4))//'_DELI'
...
!'_PAW' -> dtfil%unpaw
 ...
 if(mpi_enreg%paral_compil_kpt==1)then ! parallel case : the index of the processor must be appended
   call int2char4(mpi_enreg%me,tag)
   ixx=1
   if (mpi_enreg%paral_compil_mpio == 1 .and. dtset%accesswff == IO_MODE_MPI ) ixx=3
   do ii=ixx,7
...
!'_PAWQ' -> dtfil%unpawq
 ...
 if(mpi_enreg%paral_compil==1) then
   ...
   if (mpi_enreg%paral_compil_mpio==1.and.dtset%accesswff==IO_MODE_MPI)then
     idtmpfil(1)=0              !_1wf1
     idtmpfil(2)=0              ! s1wf2
     idtmpfil(7)=0              !  WFGS
     idtmpfil(8)=0              !  WFKQ
