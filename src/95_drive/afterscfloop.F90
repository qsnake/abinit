...
!!  cprj(natom,mcprj*usecprj)=<p_lmn|Cnk> coefficients for each WF |Cnk> and each |p_lmn> non-local projector
...
!!
!! CHILDREN
!!      dtsetcopy,dtsetfree,elpolariz,energies_copy,exchange_electronpositron
!!      forces,forstr,free_full_potential,full_local_potential,getph,hartre1
!!      hdr_update,last_orthon,leave_new,localhamiltonianapplication,mag_out
!!      metric,mkrho,nhatgrid,nonlocalhamiltonianapplication,pawmknhat
!!      prtposcar,prtrhomxmn,psolver_kernel,rhohxc,scprqt,setnoccmmp
!!      spin_current,synchronizehamiltonianapplication,timab,wrtout,wvl_mkrho
!!      wvl_newvtr,wvl_nl_gradient,wvl_tail_corrections,xcden,xcomm_world
!!      xredxcart
!!
!! SOURCE
...
& xccc3d,xred,ylm,ylmgr)
 ...
 use m_efield
  ...
  use BigDFT_API, only : LocalHamiltonianApplication, last_orthon, &
       & full_local_potential, free_full_potential, NonLocalHamiltonianApplication, &
       & SynchronizeHamiltonianApplication
#endif
...
#define ABI_FUNC 'afterscfloop'
 ...
 use interfaces_95_drive, except_this_one => afterscfloop
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: istep,mcg,mcprj,mgfftf,moved_atm_inside,n3xccc,nfftf,nkxc !,mgfftc
 integer,intent(in) :: optres,optxc,prtfor,prtxml,pwind_alloc,stress_needed,usecprj
...
!scalars
 ...
 integer :: bantot,choice,cplex,ider,idir,ifft,igrad,ii,ipert,ishift,ispden
 integer :: izero,nele,ngrad,nhatgrdim,nk3xc
 integer :: optcut,optfor,optgr0,optgr1,optgr2,optrad,quit !,optxc_str
 integer :: tim_mkrho,vtrial_shift
!
! JWZ Debugging variables
! integer :: ikpt, iband, ipw, icg
!
 real(dp) :: dtaur,dtaurzero,dum,c_fermi,enxcsr
#if defined HAVE_DFT_BIGDFT
 integer :: me,nproc,spacecomm,nrhodim,i3rho_add
 real(dp) :: ekin_sum,epot_sum,eproj_sum,evsum
 real(dp), pointer :: kernelseq(:), potential(:)
#endif
 real(dp) :: rcut_coulomb,rcut_coulomb1,ucvol
 logical :: ex
...
!arrays
 ...
 real(dp),allocatable :: dmatdum(:,:,:,:),ehart1(:),grnl(:,:)
 real(dp),allocatable :: nhatgr(:,:,:),qphon(:),rhonow(:,:,:),sqnormgrhor(:,:),vhart1(:),vtrial(:)
 real(dp),allocatable :: xcart(:,:)

! *************************************************************************

!DEBUG
!write(std_out,*)' afterscfloop : enter '
!ENDDEBUG
...
!to be adjusted to optional for rhohxc
 nk3xc=1
...
!  Apply the hamiltonian one last time.
...
!  Apply vtrial and the projectors to the wavefubctions, computing HPsi.
   ABI_ALLOCATE(vtrial,(dtset%nfft * dtset%nspden))
   call wvl_newvtr(dtset, mpi_enreg, nele, vtrial_shift, vhartr, vpsp, vtrial, vxc, wvl%descr)
...
!  deallocate potential
   call free_full_potential(nproc, potential, "afterscfloop")
...
!  transform to KS orbitals
   call last_orthon(me, nproc, wvl%wfs%orbs, wvl%wfs%Glr%wfd, dtset%nsppol, &
&   wvl%wfs%comms, wvl%wfs%psi, wvl%wfs%hpsi, wvl%wfs%psit, evsum, .true.)

   if (nproc == 1) nullify(wvl%wfs%psit)
#else
...
!    Use the tails to improve energy precision.
     call wvl_tail_corrections(dtset, energies, etotal, mpi_enreg, psps, vtrial, wvl, xcart)
   end if
...
!output cprj for debugging
!JWZ Debugging
!call cprj_output(cprj)
!print cg
!do icg = 1, size(cg,dim=2)
!write(std_out,'(2f12.8)')cg(1,icg),cg(2,icg)
!end do
...
!----------------------------------------------------------------------
 ...
 if(dtset%berryopt/=0 .and. abs(dtset%berryopt) /= 5)then
   call elpolariz(atindx1,cg,cprj,dtefield,dtfil,dtset,etotal,energies%e_elecfield,gprimd,hdr,&
&   kg,dtset%mband,mcg,mcprj,dtset%mkmem,mpi_enreg,dtset%mpw,dtset%natom,nattyp,dtset%nkpt,&
...
!----------------------------------------------------------------------
 ...
 if(abs(dtset%berryopt) == 5)then
   call mag_out(dtefield,mpi_enreg,dtset%nkpt,rprimd,dtset%wtk)
 end if
...
&   npwarr,occ,paw_dmft,phnons,taug,taur,rprimd,tim_mkrho,ucvol,dtfil%unkg,wffnow,wvl%wfs,wvl%descr,option=1)
!  Print result
...
!  ider=1;izero=0;cplex=1;ipert=0;idir=0;pelev(:)=zero
!  call pawmknhat(dum,cplex,ider,idir,ipert,izero,gprimd,mpi_enreg,dtset%natom,dtset%natom,nfftf,ngfftf,&
!  &              nhatgrdim,dtset%nspden,dtset%ntypat,&dtset%paral_kgb,pawang,pawfgrtab,nhatgr,&
!  &              nhat,pawrhoij,pawrhoij,pawtab,pelev,rprimd,ucvol,xred)
!  end if
!  optxc_str=0
!  call rhohxc(dtset,enxcsr,gsqcut,psps%usepaw,kxc,mpi_enreg,nfftf,ngfftf,&
!  &   nhat,psps%usepaw,nhatgr,nhatgrdim,nkxc,nk3xc,dtset%nspden,n3xccc,optxc_str,rhog,rhor,rprimd,strsxc,&
...
&     (stress_needed==1.and.pawfgrtab(1)%rfgd_allocated==0.and.dtset%pawstgylm==1).or.&
     (pawfgrtab(1)%rfgd_allocated==0.and.dtset%pawstgylm==0)) then
       ...
       call nhatgrid(atindx1,gmet,mpi_enreg,dtset%natom,dtset%natom,nattyp,ngfftf,dtset%ntypat,&
&       optcut,optgr0,optgr1,optgr2,optrad,pawfgrtab,pawtab,rprimd,ucvol,xred)
     end if
   end if

   if (dtset%usewvl == 0) then
     call forstr(atindx1,cg,diffor,dtefield,dtset,&
&     eigen,electronpositron,energies,favg,fcart,forold,fred,gresid,grewtn,&
&     grhf,grxc,gsqcut,indsym,&
&     kg,kxc,maxfor,mcg,mgfftf,mpi_enreg,&
...
&     ucvol,dtfil%unkg,dtfil%unylm,usexcnhat,vhartr,vpsp,vxc,wffnow,wvl%descr,&
&     xccc3d,xred,ylm,ylmgr)
   else
     ABI_ALLOCATE(xcart,(3, dtset%natom))
     call xredxcart(dtset%natom, 1, rprimd, xcart, xred)
...
&     grhf, grnl, grxc, gsqcut, indsym, maxfor, mgfftf, mpi_enreg, &
&     0, n3xccc, nattyp, dtset%nfft, dtset%ngfft, &
&     dtset%ntypat, pawtab, ph1d, psps, rhog, rhor, rprimd, symrec, &
&     synlgr, nvresid, vxc, wvl%descr, xred)
     ABI_DEALLOCATE(grnl)
   end if
 end if
...
!  Compute stress tensor
   ...
   dtLocal%berryopt = 0
   optfor=0
   call forstr(atindx1,cg,diffor,dtefield,dtLocal,&
&   eigen,electronpositron,energies,favg,fcart,forold,fred,gresid,grewtn,&
&   grhf,grxc,gsqcut,indsym,&
&   kg,kxc,maxfor,mcg,mgfftf,mpi_enreg,&
&   n3xccc,nattyp,nfftf,&
&   ngfftf,nhat,nkxc,npwarr,&
&   dtset%ntypat,nvresid,occ,optfor,optres,paw_ij,pawang,&
...
&   dtfil%unylm,usexcnhat,vhartr,vpsp,vxc,wffnow,wvl%descr,xccc3d,xred,ylm,ylmgr)
...
&   nhat,psps%usepaw,nhatgr,nhatgrdim,nkxc,nk3xc,dtset%nspden,n3xccc,optxc,&
&   rhog,rhor,rprimd,strsxc,usexcnhat,vhartr,vxc,vxcavg,xccc3d)
   write(message,'(4a,1x,es17.10,2a)') ch10,&
...
!  Restore the xc potential.
   if (psps%usepaw==1.and.usexcnhat>0 .and. dtset%xclevel==2) then
     ...
     ider=1;izero=0;cplex=1;ipert=0;idir=0;qphon(:)=zero
     call pawmknhat(dum,cplex,ider,idir,ipert,izero,gprimd,mpi_enreg,dtset%natom,dtset%natom,&
&     nfftf,ngfftf,nhatgrdim,dtset%nspden,dtset%ntypat,dtset%paral_kgb,pawang,&
&     pawfgrtab,nhatgr,nhat,pawrhoij,pawrhoij,pawtab,pelev,rprimd,ucvol,xred)
   end if
   call rhohxc(dtset,enxcsr,gsqcut,psps%usepaw,kxc,mpi_enreg,nfftf,ngfftf,&
&   nhat,psps%usepaw,nhatgr,nhatgrdim,nkxc,nk3xc,dtset%nspden,n3xccc,optxc,rhog,rhor,rprimd,strsxc,&
&   usexcnhat,vhartr,vxc,vxcavg,xccc3d)
   if (nhatgrdim>0)  then
     ABI_DEALLOCATE(nhatgr)
   end if
!  Also check the Hartree energy with a real-space cutoff for the Coulomb interaction
   ABI_ALLOCATE(ehart1,(3))
   ABI_ALLOCATE(qphon,(3))
   ABI_ALLOCATE(vhart1,(nfftf))
   ...
   write(std_out,*) '%scfcv: Check cutoff Coulomb interaction, show Hartree energy:'
   write(std_out,'(1x,a,1x,a,1x,a,6x,a)') 'cutoff radius',&
...
&     rcut_coulomb1,ucvol)
     write(std_out,'(f14.8,1x,3(3x,es14.7))') rcut_coulomb1,ehart1(3),ehart1(1),ehart1(2)
...
&   '  Hartree energy=', ehart1(3)   , ' hartree',ch10
   ...
   call wrtout(std_out,message,'COLL')
   call dtsetFree(dtLocal)
   if(allocated(ehart1))  then
     ABI_DEALLOCATE(ehart1)
   end if
   if(allocated(qphon))   then
     ABI_DEALLOCATE(qphon)
   end if
   if(allocated(vhart1))   then
     ABI_DEALLOCATE(vhart1)
   end if
 end if
...
!get electronic quantities in global variables
 if (dtset%positron/=0) then
   ...
   if (dtset%positron<0.and.electronpositron_calctype(electronpositron)==1) then
     call exchange_electronpositron(cg,dtset,eigen,electronpositron,energies,fred,mcg,mpi_enreg,&
&     nfftf,ngfftf,nhat,npwarr,occ,paw_an,pawrhoij,rhog,rhor,strten,vhartr)
   end if
 end if
...
!If PAW+U and density mixing, has to update nocc_mmp
 if (psps%usepaw==1.and.dtset%usepawu>0.and.(dtset%iscf>0.or.dtset%iscf==-3)) then
   call setnoccmmp(1,0,dmatdum,0,0,indsym,dtset%natom,dtset%natpawu,&
...
!Update the content of the header (evolving variables)
 bantot=hdr%bantot
 if (dtset%positron==0) then
   call hdr_update(bantot,etotal,energies%e_fermie,hdr,dtset%natom,residm,rprimd,occ,mpi_enreg,pawrhoij,psps%usepaw,xred)
 else
   call hdr_update(bantot,electronpositron%e0,energies%e_fermie,hdr,dtset%natom,residm,rprimd,occ,&
&   mpi_enreg,pawrhoij,psps%usepaw,xred)
 end if

!XG 070612 : Do not remove this line - needed for the pathscale compiler ?!
!write(std_out,*)' afterscfloop : fred=',fred(:,:)
