...
!  Define getwfk_from_image
   if (dtset%getwfk/=0) then
     if(dtset%getwfk<0) iget=jdtset(idtset+dtset%getwfk)
     if(dtset%getwfk>0) iget=dtset%getwfk
     if (dtsets(iget)%nimage==dtset%nimage) then
       dtfil%getwfk_from_image=iget   ! Get WFK from the same image of previous dataset
...
!  Define getden_from_image
   if (dtset%getden/=0) then
     ...
     if (dtsets(iget)%nimage==dtset%nimage) then
       dtfil%getden_from_image=iget   ! Get DEN from the same image of previous dataset
     else if (dtsets(iget)%nimage>1) then
...
!  Define getpawden_from_image
   if (dtset%getpawden/=0) then
     if(dtset%getpawden<0) iget=jdtset(idtset+dtset%getpawden)
     if(dtset%getpawden>0) iget=dtset%getpawden
     if (dtsets(iget)%nimage==dtset%nimage) then
       dtfil%getpawden_from_image=iget   ! Get PAWDEN from the same image of previous dataset
     else if (dtsets(iget)%nimage>1) then
