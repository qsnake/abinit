...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (DCA, XG, GMR, AR, MKV)
...
!!
!! CHILDREN
!!      atmdata,clnmpi_grid,clnmpi_img,distrb2,finddistrproc,ingeo,initmpi_grid
!!      initmpi_img,inkpts,inqpt,intagm,inupper,invacuum,leave_new,mkrdim
!!      wrtout
!!
!! SOURCE
...
!scalars
 ...
 integer :: me,mkmem,natom,nband_k,nkpt,nkpt_me
 integer :: nproc,nqpt,nspink,nspinor,nsppol,occopt,response
 integer :: rfddk,rfelfd,rfphon,rfstrs,rfuser
 integer :: tfband,tnband,tread,tread_alt
 real(dp) :: charge,dummy,fband,kptnrm,kptrlen,rcov
...
!arrays
 ...
 real(dp),allocatable :: dprarr(:),kpt(:,:),ratsph(:),reaalloc(:),spinat(:,:)
 real(dp),allocatable :: vel(:,:),wtk(:),xred(:,:),znucl(:)
 character(len=6) :: nm_mkmem(3)

!************************************************************************

!DEBUG
!write(std_out,*)' invars1 : enter '
!write(std_out,*)' jdtset = ',jdtset
!call flush(6)
!stop
!ENDDEBUG
...
!---------------------------------------------------------------------------
 ...
 rfddk=0; rfelfd=0 ; rfphon=0 ; rfstrs=0 ; rfuser=0
 token = 'rfddk'
...
&     '  rfphon=',rfphon,'  rfddk=',rfddk,'  rfelfd=',rfelfd,'  rfstrs=',rfstrs,ch10,&
&     '  rfuser=',rfuser,ch10,&
&     '  Action : check the values of optdriver, rfphon, rfddk, rfelfd, rfstrs',ch10,&
...
!No default value for znucl
 token = 'znucl'
 call intagm(dprarr,intarr,jdtset,marr,dtset%npsp,string(1:lenstr),token,tread,'DPR')
 if(tread==1)then
...
!The default for ratsph has already been initialized
 token = 'ratsph'
 call intagm(dprarr,intarr,jdtset,marr,dtset%ntypat,string(1:lenstr),token,tread,'LEN')
 if(tread==1)then
...
!DEBUG
!write(std_out,*)' invars1 : trim(string)=',trim(string)
!ENDDEBUG
...
!Read the Zeeman field
 token = 'zeemanfield'
 call intagm(dprarr,intarr,jdtset,marr,3,string(1:lenstr),token,tread,'BFI')
 if(tread==1) then
...
!  Call ingeo for each image in turn, with the possible default values
   ...
   call ingeo(acell,dtset%berryopt,bravais,dtset%efield,dtset%genafm(1:3),iatfix,&
&   dtset%icoulomb,iimage,iout,jdtset,dtset%jellslab,lenstr,&
&   msym,natom,dtset%nimage,dtset%npsp,dtset%nspden,dtset%nsppol,&
&   dtset%nsym,dtset%ntypat,dtset%pawspnorb,dtset%ptgroupma,ratsph,&
&   rprim,dtset%slabzbeg,dtset%slabzend,dtset%spgroup,spinat,&
&   string,symafm,dtset%symmorphi,symrel,tnons,dtset%tolsym,typat,vel,xred,znucl)
...
!DEBUG
!write(std_out,*)' invars1: before inkpts, vacuum=',vacuum(:)
!stop
!ENDDEBUG
...
!  Find the q-point, if any.
   if(nqpt==1)then
     call inqpt(chksymbreak,std_out,jdtset,lenstr,natom,dtset%qptn,&
&     dtset%rprimd_orig(1:3,1:3,intimage),dtset%spinat,string,dtset%typat,&
&     vacuum,dtset%xred_orig(1:3,1:natom,intimage))
   end if
...
&   dtset%qptn,response,dtset%rprimd_orig(1:3,1:3,intimage),&
...
&   '  Action : check the input file.'
 ...
 token = 'cd_custom_imfrqs'
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) dtset%cd_custom_imfrqs=intarr(1)

 if (dtset%cd_custom_imfrqs<0) then
   write(message, '(a,a,a,a,i12,a,a,a,a)' ) ch10,&
&   ' invars1: ERROR -',ch10,&
&   '  Input cd_custom_imfrqs must be >= 0, but was ',dtset%cd_custom_imfrqs,ch10,&
&   '  This is not allowed.',ch10,&
&   '  Action : check the input file.'
   call wrtout(std_out,  message,'COLL')
   call leave_new('COLL')
 ...
 token = 'gw_custom_freqsp'
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) dtset%gw_custom_freqsp=intarr(1)

 if (dtset%gw_custom_freqsp<0) then
   write(message, '(a,a,a,a,i12,a,a,a,a)' ) ch10,&
&   ' invars1: ERROR -',ch10,&
&   '  Input gw_custom_freqsp must be >= 0, but was ',dtset%gw_custom_freqsp,ch10,&
&   '  This is not allowed.',ch10,&
&   '  Action : check the input file.'
   call wrtout(std_out,  message,'COLL')
   call leave_new('COLL')
...
!The following two values are needed to dimension the parallelism
 token = 'ntimimage'
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) dtset%ntimimage=intarr(1)
 token = 'dynimage'
 call intagm(dprarr,intarr,jdtset,marr,dtset%nimage,string(1:lenstr),token,tread,'INT')
 if(tread==1)then
 ...
 if(tread==1) then
   dtset%npimage=intarr(1)
 else
   dtset%npimage=mpi_enreg%nproc
   ii=dtset%ndynimage;if (dtset%ntimimage<=1) ii=dtset%nimage
   if (ii<mpi_enreg%nproc) dtset%npimage=ii
 end if
 ...
 token = 'npspinor'
 call intagm(dprarr,intarr,0,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) dtset%npspinor=intarr(1)
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) then
   dtset%nbdblock=intarr(1)
 else
   dtset%nbdblock=1
 end if

!paral_kgb is already read in invars0
...
!  Initialization of mpi_enreg for this dataset
!  (Prior to this line, the content of mpi_enreg was dataset-independent)
   ...
   call initmpi_img(dtset,mpi_enreg,2) ! Initialize comm over procs treating 1 img
...
!  --IF CUDA AND RECURSION:ONLY BAND PARALLELISATION
   if(dtset%tfkinfunc==2 .and. nproc/=1)then
     dtset%npband = dtset%npband*dtset%npkpt*dtset%npspinor*dtset%npfft
     dtset%npkpt = 1
     dtset%npfft = 1
     dtset%npspinor = 1
...
&     '  npfft= 1, npkpt= 1, npband=',dtset%npband,' .'
   ...
   if (dtset%npspinor>=2.and.dtset%nspinor==1) then
     dtset%npspinor=1
     dtset%npfft=2*dtset%npfft
...
&   dtset%npfft/=1.or.dtset%npkpt/=1.or.dtset%npspinor/=1))then
     if(dtset%npkpt*dtset%npfft*dtset%npband*dtset%npspinor > nproc )then
       write(message,'(10a)') ch10,&
&       ' abinit : WARNING -',ch10,&
...
&       '  At present, only a very simple algorithm is used ...'
       ...
       if(dtset%npkpt*dtset%npband*dtset%npspinor <= nproc) then
         dtset%npfft=1
         write(message,'(4a)') ch10,&
&         ' abinit : WARNING -',ch10,&
&         '  Set npfft to 1'
         call wrtout(std_out,message,'COLL')
       else if(dtset%npkpt*dtset%npspinor <= nproc)then
         dtset%npfft=1
         dtset%npband=1
         write(message,'(4a)') ch10,&
&         ' abinit : WARNING -',ch10,&
&         '  Set npfft and npband to 1'
         call wrtout(std_out,message,'COLL')
       else if(dtset%npkpt <= nproc)then
         dtset%npfft=1
         dtset%npband=1
         dtset%npspinor=1
         write(message,'(4a)') ch10,&
...
&         '  Set npfft, npband, nspinor and npkpt to 1'
     ...
     else if(dtset%npkpt*dtset%npfft*dtset%npband*dtset%npspinor < nproc)then
...
!  Set mpi_enreg
   if(dtset%paral_kgb/=0)then
     ...
     mpi_enreg%nproc_spin=min(dtset%npspinor,dtset%nspinor)
     mpi_enreg%me_kpt=-1
   else
     ...
     if (mpi_enreg%paral_img==0) then
       ...
       mpi_enreg%nproc_kpt=mpi_enreg%nproc_one_img
       mpi_enreg%me_kpt=mpi_enreg%me_one_img
     end if
   end if
...
!      is NOT taken into account. This should be changed later ...
       ABI_ALLOCATE(mpi_enreg%proc_distrb,(nkpt,mband_upper,nsppol))
       if(response==0)then
         mpi_enreg%paralbd=0
         if ( mod(dtset%wfoptalg,10)==1 .and. &
&         nkpt<mpi_enreg%nproc_kpt .and. &
         dtset%nbdblock/=1 ) then
           mpi_enreg%paralbd=dtset%nbdblock
         end if
!        Set paral_compil_respfn and ngroup_respfn so distrb2
!        calculates the right proc_distrb for the actual dtset
         ...
         do ikpt=1,nkpt
           if (mpi_enreg%paralbd == 0) then
             if(minval(abs(mpi_enreg%proc_distrb(ikpt,1,1:nsppol)-mpi_enreg%me_kpt))==0)&
&             nkpt_me=nkpt_me+1
           else !paralbd > 1
             do isppol=1,nsppol
...
!    mkmem_eff = ceiling(dtset%nkpt/mpi_enreg%nproc_kpt)
     if (dtset%usewvl == 0)then
       if (mpi_enreg%paralbd==0.or.mkmem>1) then
         if (mpi_enreg%nproc_kpt - floor(nsppol*dtset%nkpt*one/mkmem) >= mkmem) then
...
&           ' ACTION: you can reduce number of processors to ', floor(nsppol*dtset%nkpt*one/mkmem), &
&           ' without losing speed.'
           call wrtout(std_out,message,'COLL')
         end if
       else
         if (mod(mband_upper,max(1,mpi_enreg%nproc_kpt/(nsppol*dtset%nkpt)))/=0) then
           write (message,'(4a,I6,a,I6,3a,I6,3a)') ch10, &
&           ' invars1 : WARNING -',ch10, &
&           '   Your number of k-points (',dtset%nkpt,') and bands (',mband_upper,&
...
!  Clean eventually temporary allocations
   if(dtset%paral_kgb>=0)then
