...
&   ' but it should be positive and < 4.'
 ...
 if(list_number<0 .or. list_number>40)then
   write(message,'(a,a,a,a,i6,a)' )ch10,&
&   ' chkint_prt: BUG -',ch10,&
&   '  The value of list_number is',list_number,&
&   ' but it should be between 0 and 40.'
   call wrtout(unit,message,'COLL')
   call wrtout(std_out,  message,'COLL')
   call leave_new('COLL')
 end if
...
!Compose the message, and print it
 ierr=1
 write(message, '(a,a)' ) ch10,' chkint_prt: ERROR -'
...
&     trim(cond_string(icond)),' is',cond_values(icond),'.'
 ...
 write(message, '(2a,a,a,a,i5,a)' ) trim(message),ch10,&
& '  The value of the input variable ',trim(input_name),&
& ' is',input_value,', while it must be'
 if(minmax_flag==2)then
   write(message, '(2a,a,20(i4))' ) trim(message),ch10,&
...
!  The following format restricts list_values to be between -99 and 999
   if(list_number/=1)then
     write(message, '(2a,a,40(i4))' ) trim(message),ch10,&
...
!    The following format restricts minmax_value to be between -99 and 999
     write(message, '(2a,a,i4,a)' ) trim(message),ch10,&
