...
!!
!! COPYRIGHT
!! Copyright (C) 2007-2012 ABINIT group (XG).
...
!DEBUG
!write(std_out,*)' chkvars : enter '
!write(std_out,*)trim(string)
!stop
!ENDDEBUG

!Here, list all admitted variable names (max 10 per line, to fix the ideas)
!A
 list_vars=                 ' accesswff acell algalch amu angdeg atvshift awtr'
!B
 list_vars=trim(list_vars)//' bandpp bdberry bdeigrf bdgw berryopt berrystep bfield bmass'
 list_vars=trim(list_vars)//' boxcenter boxcutmin brvltt bxctmindg'
!BEGIN VARIABLES FOR @Bethe-Salpeter
 ...
 list_vars=trim(list_vars)//' bs_calctype bs_coupling bs_haydock_tol bs_hayd_term '
 list_vars=trim(list_vars)//' bs_loband bs_eh_cutoff bs_freq_mesh bs_nstates'
!END VARIABLES FOR @Bethe-Salpeter.
!C
 list_vars=trim(list_vars)//' cd_custom_imfrqs cd_imfrqs'
 list_vars=trim(list_vars)//' cd_halfway_freq cd_max_freq cd_subset_freq cd_use_tangrid'
 list_vars=trim(list_vars)//' cd_full_grid'
 list_vars=trim(list_vars)//' charge chkexit chkgwcomp chkprim chksymbreak cpus cpum cpuh'
!D
 list_vars=trim(list_vars)//' delayperm densty diecut diegap dielam dielng diemac'
 list_vars=trim(list_vars)//' diemix diemixmag diismemory dilatmx dmatpuopt dmatudiag dmatpawu dmft_dc dmft_iter '
...
!F
 list_vars=trim(list_vars)//' pawfatbnd fband fermie_nest fftalg fftcache fftgw fft_opt_lob fixmom freqremin'
 list_vars=trim(list_vars)//' freqremax freqspmin freqspmax freqsusin freqsuslo friction frzfermi fxcartfactor '
!G
 list_vars=trim(list_vars)//' genafm getbsreso getbseig getbscoup getcell gethaydock getsuscep '
 list_vars=trim(list_vars)//' getddk getden getgam_eig2nkq getkss getocc getpawden getqps getscr'
 ...
 list_vars=trim(list_vars)//' gw_custom_freqsp gw_nqlwl gw_nstep gw_qlwl gw_sctype gw_sigxcore'
 list_vars=trim(list_vars)//' gw_toldfeig gw_eet gw_eet_inclvkb gw_eet_nband gw_eet_scale gw_freqsp'
 list_vars=trim(list_vars)//' gw_use_pole_scr gw_reconst_scr gw_npoles'
...
!because no doc or test were provided ...
 ...
 list_vars=trim(list_vars)//' iprcel iprctfvw iprcfc irandom irdbscoup irdbseig irdbsreso irdddk irdden'
 list_vars=trim(list_vars)//' irdhaydock irdqps irdkss irdpawden irdscr irdsuscep irdwfk irdwfq ird1den'
...
!M
 list_vars=trim(list_vars)//' macro_uj maxnsym mdtemp mdwall mffmem mixalch'
...
!N
 ...
 list_vars=trim(list_vars)//' ngkpt ngqpt ngroup_rf nkpt nkptgw nimage nline nloalg nomegasi nnos nnsclo'
 list_vars=trim(list_vars)//' nobj nomegasf nomegasrd noseinert npband'
 list_vars=trim(list_vars)//' npfft npimage npkpt npsp npspinor npulayit npweps npwkss npwsigx'
 list_vars=trim(list_vars)//' npwwfn nqpt nqptdm nscforder nsheps nshiftk nshiftq'
...
!P
 list_vars=trim(list_vars)//' paral_kgb paral_rf pawcpxocc pawcross pawecutdg pawlcutd pawlmix pawmixdg'
 list_vars=trim(list_vars)//' pawnhatxc pawnphi pawntheta pawnzlm pawovlp pawoptmix'
 list_vars=trim(list_vars)//' pawprtden pawprtdos pawprtvol pawprtwf pawprt_k pawprt_b pawspnorb pawstgylm'
 list_vars=trim(list_vars)//' pawsushat pawujat pawujrad pawujv pawusecp pawxcdev pimass pitransform'
 list_vars=trim(list_vars)//' positron posnstep posocc postoldfe postoldff ppmfrq ppmodel'
 list_vars=trim(list_vars)//' prepanl prepgkk prepscphon  papiopt'
...
!R
 list_vars=trim(list_vars)//' random_atpos ratsph rcut'  !  XG091017 : rdmnb has been removed from this list, because no doc or test were provided ...
 list_vars=trim(list_vars)//' recefermi recgratio recnpath recnrec recptrott recrcut rectesteg rectolden'
 list_vars=trim(list_vars)//' restartxf rfasr rfatpol rfddk rfdir rfelfd rfmeth rfphon'
...
!S
 list_vars=trim(list_vars)//' scalecart sciss scphon_temp shiftk shiftq signperm '
...
!U
 list_vars=trim(list_vars)//' udtset upawu use_gpu_cuda usedmatpu usedmft useexexch usekden usepawu usewvl'
 list_vars=trim(list_vars)//' useria userib useric userid userie'
 list_vars=trim(list_vars)//' userra userrb userrc userrd userre usexcnhat useylm use_slk'
!V
 list_vars=trim(list_vars)//' vaclst vacnum vacuum vacwidth vcutgeo vdw_xc vdw_nfrag vdw_supercell'
 list_vars=trim(list_vars)//' vdw_typfrag vel vis vprtrb'
!W
 list_vars=trim(list_vars)//' wfoptalg wtatcon wtk'
 list_vars=trim(list_vars)//' wvl_crmult wvl_frmult wvl_hgrid wvl_nprccg'
 list_vars=trim(list_vars)//' w90iniprj w90prtunk'
!X
 list_vars=trim(list_vars)//' xangst xcart xc_denpos xred xyzfile '
!Y
!Z
...
!DEBUG
!write(std_out,*)' chkvars : len_trim(string)=',len_trim(string)
!write(std_out,*)' chkvars : trim(string)=',trim(string)
!ENDDEBUG
...
!    DEBUG
!    write(std_out,*)' chkvars :'
!    write(std_out,*)' index_current, index_blank, string',index_current,index_blank,string(index_current:index_endword)
!    write(std_out,*)' index_list_vars=',index_list_vars
!    if(index_list_vars/=0)then
!    write(std_out,*)' list_vars(index_list_vars:index_list_vars+12)=',&
!    &    list_vars(index_list_vars:min(index_list_vars+12,len_trim(list_vars)))
!    else
!    write(std_out,*)' not in list_vars, might be in list_logicals or list_strings '
!    endif
!    write(std_out,*)
!    ENDDEBUG
...
!      DEBUG
!      write(std_out,*)' list_logicals,index=:',list_logicals,':',index(list_strings,string(index_current:index_endword)//blank)
!      write(std_out,*)' list_strings,index=:',list_strings,':',index(list_strings,string(index_current:index_endword)//blank)
!      ENDDEBUG
