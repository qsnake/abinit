...
!!  iprcch=govern the choice of density prediction and/or forces correction
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: getcell,icoulomb,idtset,intxc,ionmov,iout,iprcch
 ...
 integer,intent(in) :: prtvol,usepaw,useylm,xclevel
 real(dp) :: ucvol
 type(MPI_type),intent(in) :: mpi_enreg
!arrays
 integer,intent(in) :: nband(nkpt*nsppol),ngfft(18),ngfftdiel(18),ngfftf(18)
 integer,intent(in) :: nloalg(5),typat(natom)
...
!cfft(marrays) =number of blocks of size nfft bytes (coarse grid, if PAW)
!cfftf(marrays)=number of blocks of size nfft bytes (fine grid, if PAW)
...
!scalars
 ...
 integer :: fftalgb,ii,iscf10,jj,l_max,l_size_max,matblk,mblk,mincat,mu,n_fftgr
 integer :: narr_fourdp,nbnd_in_blk,ndiel4,ndiel456,ndiel5,ndiel6,ngrad,nprocwf
 integer :: nspgrad,rhoij_nspden
 real(dp) :: mbcg,mbdiskpd,mbdiskwf,mbf_fftgr,mbgylm
 character(len=500) :: message
!arrays
 integer :: cadd(marrays),cfft(marrays),cfftf(marrays),cmpw(marrays)
 integer :: dttyp(marrays),nattyp(ntypat)
 integer,allocatable :: basis_size(:),l_size(:),lmn2_size(:),lmn_size(:)
 integer,allocatable :: mesh_size(:),pawver(:),shape_type(:)
...
&   '  option=',option,' while the only allowed values are 0, 1, or 2.'
 ...
 cmpw(:)=0 ; cfft(:)=0 ; cfftf(:)=0 ; cadd(:)=0 ; dttyp(:)=0

 write(message,*)'memory : analysis of memory needs '
 call wrtout(std_out,message,'COLL')
...
&   ' Values of the parameters that define the memory need of the present run '
 ...
 write(message,'( 4(a,i10),a,4(a,i10) )' ) &
& '   intxc =',intxc  ,'  ionmov =',ionmov,&
& '    iscf =',iscf   ,' xclevel =',xclevel   , ch10,&
& '  lmnmax =',lmnmax ,'   lnmax =',lnmax,&
& '   mband =',mband ,'  mffmem =',mffmem
 call wrtout(iout,message,'COLL')
 call wrtout(std_out,message,'COLL')

 write(message,'(3(4(a,i10),a),4(a,i10))')&
& 'P  mgfft =',mgfft    ,'   mkmem =',mkmem,&
& ' mpssoang=',mpssoang ,'     mpw =',mpw   ,ch10,&
& '  mqgrid =',mqgrid_vl,'   natom =',natom ,&
& '    nfft =',nfft     ,'    nkpt =',nkpt  ,ch10,&
& '  nloalg =',nloalg(1),'  nspden =',nspden,&
& ' nspinor =',nspinor  ,'  nsppol =',nsppol,ch10,&
& '    nsym =',nsym     ,'  n1xccc =',n1xccc,&
& '  ntypat =',ntypat   ,'  occopt =',occopt
 call wrtout(iout,message,'COLL')
 call wrtout(std_out,message,'COLL')
...
!Compute number of atoms per type
 nattyp(:)=0
 do ii=1,natom
...
&    *(basis_size(ii)+1)/2
     ...
     cadd(17)=cadd(17)+lmn2_size(ii)*6                  !indklmn
...
!  additional arrays
   ...
   do ii=1,ntypat
     cadd(16)=cadd(16)+nattyp(ii)*lmn2_size(ii)*rhoij_nspden*pawcpxocc   ! Rhoij and related data
     cadd(17)=cadd(17)+nattyp(ii)*(2+lmn2_size(ii))    ! (rhoijselect, ...)
   end do
 end if

!SCF history (if selected)
 if (abs(iprcch)==5.or.abs(iprcch)==6) then              ! scf_history...
   cfftf(18)=nspden*(2+1)        ; dttyp(18)=8  !    %deltarhor, %atmrho_last
   cadd(19)=natom*2+9            ; dttyp(19)=8  !    %xcart,     %rprimd_last
   cadd(19)=natom*2+9            ; dttyp(19)=8               !    %xcartdiff, %rprimd_last
   dttyp(20)=4
   if (usepaw==1) then
     do ii=1,ntypat
       cadd(19)=cadd(19)+2*nattyp(ii)*lmn2_size(ii)**rhoij_nspden*pawcpxocc  ! %pawrhoij()%rhoijp
       cadd(20)=cadd(20)+2*nattyp(ii)*(2+lmn2_size(ii))*nspden ! %pawrhoij()%rhoijselect
     end do
   end if
 end if
...
!kxc
 if (abs(iprcch)>0.and.iscf>=10) then
   cfftf(21)=cfftf(21)+3*nspden
   if (iprcch<0.and.xclevel==2) cfftf(21)=cfftf(21)+20*nspden
 end if
 if(iscf>0)then
!  f_fftgr
   if (pawmixdg==1) then
...
!Additional PAW arrays
 ...
 if (usepaw==1) then
   do ii=1,ntypat
     ...
     cadd(26)=cadd(26)+nattyp(ii)*jj                         !   %ifftsph
     cadd(25)=cadd(25)+nattyp(ii)*jj*(1-pawstgylm)*3         !   %rfgd (if pawstgylm=0)
     cadd(25)=cadd(25)+nattyp(ii)*jj*pawstgylm*l_size(ii)**2 !   %gylm (if pawstgylm=1)
     if (optforces==1) cadd(25)=cadd(25)+nattyp(ii)*jj&      !   %gylmgr,%rfgd (if pawstgylm=1)
&    *pawstgylm*(3*l_size(ii)**2+3*optstress)
     cadd(26)=cadd(26)+nattyp(ii)*l_size(ii)**2/32           ! lmselect  !now a boolean
     cadd(25)=cadd(25)+nattyp(ii)*lmn2_size(ii)*nspinor**3   ! dij
     if (iscf>0) then
       cadd(25)=cadd(25)+nattyp(ii)*lmn2_size(ii)*rhoij_nspden*pawcpxocc     ! rhoijres
       cadd(25)=cadd(25)+nattyp(ii)*lmn2_size(ii)*rhoij_nspden*pawcpxocc*n_fftgr*mffmem      ! f_paw
     end if
   end do
...
!    f_paw_disk
     if (usepaw==1) then
       dttyp(43)=8
       do ii=1,ntypat
         cadd(43)=cadd(43)+nattyp(ii)*lmn2_size(ii)*nspden*n_fftgr
       end do
     end if
   end if
...
!(9b)                    in getghc, nonlop and opernl--------------------
 mincat=min(nloalg(4),natom-ntypat+1)
...
!  considering the stress, since it will be called outside of the main chain
   cadd(97)=nloalg(3)*40        ; dttyp(97)=8
!  kpg if nloalg(5)=1
   cadd(98)=3*mpw*nloalg(5)     ; dttyp(98)=8
 else                                        ! ===== nonlop_ylm
!  gx + gxfac + gxfac_sij
