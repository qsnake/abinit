...
!!
!! CHILDREN
!!      clnmpi_fft,clnmpi_img,distrb2,getdim_nloc,getmpw,getng,initmpi_fft
!!      initmpi_img,invars2,mati3inv,memorf,memory,metric,mkrdim,prtspgroup
!!      setmqgrid,setshells,symq3,wrtout,wvl_memory
!!
!! SOURCE
...
!scalars
 ...
 integer :: mpssoang,mpw,mpw_k,mqgrid,mqgriddg,mqgrid_ff,mqgrid_vl,n1xccc,natom,nfft,nfftdg
 integer :: nfftdiel,nfftf,nkpt,nproc_fft,nptsgvec,npulayit,npwdiel,nqpt,nspden,nspinor
...
!  DEBUG
!  write(std_out,*)' invars2m : ptgroupma=',ptgroupma
!  ENDDEBUG
...
!    Should fill the values of nproc_fft and me_fft by MPI calls
     ...
     call initmpi_img(dtsets(idtset),mpi_enreg,2) ! Initialize comms over images of the cell
...
!    creation of arrays for FFT parallelization here because in initmpi_fft mgfft is not known
     ABI_ALLOCATE(mpi_enreg%nplanes_fft,(dtsets(idtset)%nkpt))
     ABI_ALLOCATE(mpi_enreg%ind_fft_planes,(dtsets(idtset)%nkpt,ngfft(2)))
   end if
...
&       ngfft,ngfftdiel,ngfftf,dtsets(idtset)%nimage,nkpt,nloalg,npsp,npulayit,npwdiel,nspden,nspinor,&
