...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (XG, RC)
...
!!      leave_new,mati3inv,metric,mkrdim,randomcellpos,symanal,symatm,symfind
!!      symlatt,symrelrot,symzat,wrtout,xredxcart
!!
!! SOURCE
...
& msym,natom,nimage,npsp,nspden,nsppol,nsym,ntypat,pawspnorb,&
& ptgroupma,ratsph,rprim,slabzbeg,slabzend,spgroup,spinat,string,symafm,&
& symmorphi,symrel,tnons,tolsym,typat,vel,xred,znucl)
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: berryopt,iimage,iout,jdtset,lenstr,msym,nimage,npsp,nspden,nsppol,ntypat,pawspnorb
...
!scalars
 ...
 integer :: nptsym,nsym_now,random_atpos,shubnikov,spgaxor,spgorig
...
!DEBUG
!write(std_out,*)' ingeo : enter '
!write(std_out,*)' ingeo : ratsph(1:ntypat)=',ratsph(1:ntypat)
!call flush(6)
!stop
!ENDDEBUG
...
!  DEBUG
!  write(std_out,'(a)' ) ' ingeo : before intagm _typat'
!  write(std_out,'(a)' ) string(1:lenstr)
!  ENDDEBUG
   token = '_typat'
   call intagm(dprarr,intarr,jdtset,marr,natrd,string(1:lenstr),token,tread,'INT')
 end if
...
!6) Read coordinates for each atom in the primitive set--------
 ...
 random_atpos=0
 token = 'random_atpos'
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,txrandom,'INT')
...
!DEBUG
!write(std_out,*)' ingeo : xred_read ='
!write(std_out,*)xred_read(:,1:natrd)
!ENDDEBUG
...
& token,txcart,'LEN')
 ...
 if(nimage/=1 .and. iimage/=1)then
   token = 'xcart_lastimg'
   call intagm(dprarr,intarr,jdtset,marr,3*natrd,string(1:lenstr),token,tread,'LEN')
   if(tread==1)then
...
!DEBUG
!write(std_out,*)' ingeo : xcart_read ='
!write(std_out,*)xcart_read
!ENDDEBUG
...
!      DEBUG
!      write(std_out,*)' ingeo : brvltt = ',brvltt
!      write(std_out,*)' ingeo : spgaxor = ',spgaxor
!      write(std_out,*)' ingeo : spgorig = ',spgorig
!      write(std_out,*)' ingeo : spgroup = ',spgroup
!      write(std_out,*) 'ingeo : before symmetry part, msym is msym = ',msym
!      ENDDEBUG
...
!      Treat the case of magnetic groups
       ...
       if(tspgroupma/=0 .and. tgenafm/=0)then
         write(message, '(a,a,a,a,i4,a,a,3es9.2,a,a,a,a,a,a,a,a)' ) ch10,&
&         ' ingeo : ERROR -',ch10,&
...
!      DEBUG
!      write(std_out,*)' ingeo : tolsym=',tolsym
!      ENDDEBUG
...
!DEBUG
!write(std_out,*)' ingeo : before symmorphi filter '
!do isym=1,nsym
!write(std_out,'(i2,9i3,3f8.4,i3)' )isym,symrel(:,:,isym),tnons(:,isym),symafm(isym)
!end do
!symmorphi=0
!ENDDEBUG
...
!DEBUG
!write(std_out,*)' ingeo : after symmorphi filter '
!do isym=1,nsym
!write(std_out,'(i2,9i3,3f8.4,i3)' )isym,symrel(:,:,isym),tnons(:,isym),symafm(isym)
!end do
!ENDDEBUG
...
!  DEBUG
!  write(std_out,*)' ingeo  : idir ',idir,'tread ',tread,'natfix',natfix,'iatfix ',iatfix
!  if(idir==3)stop
!  ENDDEBUG
...
!DEBUG
!write(std_out,*)' ingeo : end of subroutine, nsym= ',nsym
!write(std_out,*)' ingeo : ptgroupma=',ptgroupma
!write(std_out,*)' ingeo : natfix=',natfix
!write(std_out,*)' ingeo : iatfix=',iatfix
!if(.true.)stop
!ENDDEBUG
