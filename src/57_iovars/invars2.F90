...
!!
!! CHILDREN
!!      atmdata,chkneu,inkpts,intagm,invacuum,leave_new,sort_int,timab,wrtout
!!
!! SOURCE
...
&   '  cd_custom_imfrqs not equal to zero and not equal to nfreqim',ch10,&
&   '  setting nfreqim = cd_custom_imfrqs'
   call wrtout(std_out,message,'COLL')
   dtset%nfreqim = dtset%cd_custom_imfrqs
 end if
 ...
 token = 'cd_use_tangrid'
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
 if(tread==1) dtset%cd_use_tangrid=intarr(1)
 ...
 if(dtset%cd_custom_imfrqs/=0) then
   token = 'cd_imfrqs'
   call intagm(dprarr,intarr,jdtset,marr,dtset%cd_custom_imfrqs,string(1:lenstr),&
&   token,tread,'ENE')
   if(tread==1) then
     dtset%cd_imfrqs(1:dtset%cd_custom_imfrqs)=dprarr(1:dtset%cd_custom_imfrqs)
     do ifreq=1,dtset%cd_custom_imfrqs-1
...
&         '  Action : Correct this in your input file'
 ...
 if(dtset%gw_custom_freqsp/=0) then
   token = 'gw_freqsp'
   call intagm(dprarr,intarr,jdtset,marr,dtset%gw_custom_freqsp,string(1:lenstr),&
&   token,tread,'ENE')
   if(tread==1) then
     dtset%gw_freqsp(1:dtset%gw_custom_freqsp)=dprarr(1:dtset%gw_custom_freqsp)
     do ifreq=1,dtset%gw_custom_freqsp-1
...
!DEBUG
!write(std_out,*)' invars2m : npspalch=',npspalch
!ENDDEBUG
...
!DEBUG
!write(std_out,*)' invars2 : dtset%ziontypat=',dtset%ziontypat(1:ntypat)
!write(std_out,*)' invars2 : zionpsp=',zionpsp(1:npsp)
!write(std_out,*)' invars2 : ntyppure,npspalch,ntypalch=',ntyppure,npspalch,ntypalch
!write(std_out,*)' invars2 : mixalch=',dtset%mixalch(1:npspalch,1:ntypalch)
!ENDDEBUG
...
!DEBUG
!write(std_out,*)' invars2: after read nband '
!write(std_out,*)dtset%nband(1:nkpt*nsppol)
!stop
!ENDDEBUG
...
!van der Waals with Wannier functions (Silvestrelli's approach versions 1 and 2)
 if(dtset%vdw_xc==10.or.dtset%vdw_xc==11) then
   token = 'vdw_nfrag'
   call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),token,tread,'INT')
   if(tread==1)then
...
!DEBUG
!write(std_out,*)' invars2: before inkpts, vacuum=',vacuum(:)
!stop
!ENDDEBUG
...
!Default for pimass is amu
 dtset%pimass(1:ntypat)=dtset%amu(1:ntypat)
 call intagm(dprarr,intarr,jdtset,marr,ntypat,string(1:lenstr),token,tread,'DPR')
...
!Force ffalg 112 to debug omp sections, fftalga = 4 is broken.
!dtset%ngfft(7)=112
!if (usepaw==1) dtset%ngfftdg(7)=112
