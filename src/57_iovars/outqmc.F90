...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (DCA, XG, GMR, JYR, MKV, MT, FJ)
...
!!
!! CHILDREN
!!      leave_new,wrtout,xcomm_world,xredxcart
!!
!! SOURCE
...
!scalars
 ...
 integer :: iocc,isppol,jj,me,nband1,nband2,nelecs,nkgfull
 real(dp) :: norm
...
&     ' : Parallelization over k-points is not currently supported'
 ...
 if(mpi_enreg%paral_spin==1)then
   write(message,'(a,a,a)')' outqmc : ERROR -',ch10,&
&   ' : Parallelization over spinors is not currently supported'
   call wrtout(std_out,message,'PERS')
   call leave_new('PERS')
 end if
   ...
   do isppol=1,dtset%nsppol
     do iband=1,dtset%nband(ikpt+(isppol-1)*dtset%nkpt)
       if(occ(iocc)/=int(occ(iocc)))then
         write(message,'(a,a,a,i5,a,i1,a,i5,a,f11.8,a)')' outqmc : WARNING -',&
