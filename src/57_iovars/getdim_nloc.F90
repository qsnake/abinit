...
!!
!! INPUTS
!!  mixalch(npspalch,ntypalch)=alchemical mixing coefficients
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: npsp,npspalch,ntypalch,ntypat
...
!DEBUG
!write(std_out,*)' getdim_nloc : enter'
!write(std_out,*)' pspheads(1)%nproj(0:3)=',pspheads(1)%nproj(0:3)
!ENDDEBUG
...
!      If there is some mixing, must accumulate the projectors
       if(abs(mixalch(ipspalch,itypalch))>tol10)then
...
!DEBUG
!write(std_out,*)' getdim_nloc : exit'
!stop
!ENDDEBUG
