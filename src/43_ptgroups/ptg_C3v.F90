...
! List of irreducible representations.
 ...
 Irr(3)%dim = 2
 Irr(3)%nsym = 6
 ABI_ALLOCATE(Irr(3)%mat,(2,2,6))
 Irr(3)%mat(:,:,1) =  RESHAPE( (/1.0, 0.0, 0.0, 1.0/), (/2, 2/) )
 Irr(3)%mat(:,:,2) =  RESHAPE( (/-0.5+0.86603*j, 0.0+0.0*j, 0.0+0.0*j, -0.5-0.86603*j/), (/2, 2/) )
 Irr(3)%mat(:,:,3) =  RESHAPE( (/-0.5-0.86603*j, 0.0+0.0*j, 0.0+0.0*j, -0.5+0.86603*j/), (/2, 2/) )
 Irr(3)%mat(:,:,4) =  RESHAPE( (/0.0, 1.0, 1.0, 0.0/), (/2, 2/) )
 Irr(3)%mat(:,:,5) =  RESHAPE( (/0.0+0.0*j, -0.5+0.86603*j, -0.5-0.86603*j, 0.0+0.0*j/), (/2, 2/) )
 Irr(3)%mat(:,:,6) =  RESHAPE( (/0.0+0.0*j, -0.5-0.86603*j, -0.5+0.86603*j, 0.0+0.0*j/), (/2, 2/) )
