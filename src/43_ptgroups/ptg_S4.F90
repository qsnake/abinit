...
! List of irreducible representations.
 ...
 Irr(3)%dim = 1
 Irr(3)%nsym = 4
 ABI_ALLOCATE(Irr(3)%mat,(1,1,4))
 Irr(3)%mat(:,:,1) =  RESHAPE( (/1.0/), (/1, 1/) )
 Irr(3)%mat(:,:,2) =  RESHAPE( (/-1.0/), (/1, 1/) )
 Irr(3)%mat(:,:,3) =  RESHAPE( (/0.0+1.0*j/), (/1, 1/) )
 Irr(3)%mat(:,:,4) =  RESHAPE( (/0.0-1.0*j/), (/1, 1/) )

 Irr(4)%name = "E2"
 Irr(4)%dim = 1
 Irr(4)%nsym = 4
 ABI_ALLOCATE(Irr(4)%mat,(1,1,4))
 Irr(4)%mat(:,:,1) =  RESHAPE( (/1.0/), (/1, 1/) )
 Irr(4)%mat(:,:,2) =  RESHAPE( (/-1.0/), (/1, 1/) )
 Irr(4)%mat(:,:,3) =  RESHAPE( (/0.0-1.0*j/), (/1, 1/) )
 Irr(4)%mat(:,:,4) =  RESHAPE( (/0.0+1.0*j/), (/1, 1/) )
