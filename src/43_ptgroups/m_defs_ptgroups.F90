...
!!  cannote be #included here.  The size of the final module 
...
!! SOURCE
 ...
 type,public :: irrep_t
   ...
   complex(dpc),pointer :: trace(:)   SET2NULL 
   ! trace(nsym) 
...
!! SOURCE
 ...
 type,public :: point_group_t
   ...
   character(len=5),pointer :: class_names(:)   SET2NULL
   ! class_names(nclass) 
   ! The name of each class.

   type(irrep_t),pointer :: Irreps(:)          SET2NULL 
   ! Irreps(nclass)
