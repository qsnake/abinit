...
!! SOURCE
 ...
 public :: dst_init  ! Helper function that calculates the statistical parameters of a data distribution.
!!***
...
subroutine unit_matrix_cdp(matrix)
...
!!
!! SOURCE
function get_trace_int(matrix) result(trace)
...
function get_trace_rdp(matrix) result(trace)
...
!!
!! SOURCE
function get_trace_cdp(matrix) result(trace)
...
!!
!! SOURCE
function rdp2cdp_2D(rr) result(cc)
...
!!
!! SOURCE
function rdp2cdp_3D(rr) result(cc)
...
!!
!! SOURCE
function rdp2cdp_4D(rr) result(cc)
...
!!
!! SOURCE
function rdp2cdp_5D(rr) result(cc)
...
!!
!! SOURCE
function rdp2cdp_6D(rr) result(cc)
...
!!
!! SOURCE
function cdp2rdp_2D(cc) result(rr)
...
!!
!! SOURCE
function cdp2rdp_3D(cc) result(rr)
...
!!
!! SOURCE
function cdp2rdp_4D(cc) result(rr)
...
!!
!! SOURCE
function cdp2rdp_5D(cc) result(rr)
...
function imax_loc_int(iarr,mask)
...
!!
!! SOURCE
function imax_loc_rdp(arr,mask)
...
function imin_loc_int(arr,mask)
...
!!
!! SOURCE
function imin_loc_rdp(arr,mask)
...
& __FILE__,__LINE__)
 ...
 ABI_ALLOCATE(U,(npts,npts))
 ABI_ALLOCATE(S,(nfuncs))
 ABI_ALLOCATE(Vt,(nfuncs,nfuncs))
...
!!
!! INPUT
!!  nn=Number of elements to be analysed.
!!  arr(nn)=Array with the values.
!!
!! OUTPUT
!!  Dts<dts_t>=Data type storing the parameters of the data set.
!!
!! PARENTS
!!      m_shirley
!!
...
subroutine dst_init(Dst,nn,arr)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'dst_init'
!End of the abilint section
...
! *************************************************************************
 ...
 do ii=1,nn
   xx = arr(ii)
   Dst%max  = MAX(Dst%max, xx)
   Dst%min  = MIN(Dst%min, xx)
   Dst%mean = Dst%mean + xx
 end do

 Dst%mean  = Dst%mean/nn

 ! Two-pass algorithm for the variance (more stable than the single-pass one).
 x2_sum = zero
 do ii=1,nn
   xx     = arr(ii)
   x2_sum = x2_sum + (xx - Dst%mean)*(xx - Dst%mean)
 end do

 if (nn>1) then 
   Dst%stdev  = x2_sum/(nn-1)
   Dst%stdev = SQRT(ABS(Dst%stdev))
 else 
   Dst%stdev = zero
 end if
