...
!! SOURCE
 ...
 type,public :: lebedev_grid_t
   integer :: npts=0
   real(dp),pointer :: versor(:,:)  SET2NULL
   real(dp),pointer :: weight(:)    SET2NULL
...
! *********************************************************************
 ...
 ABI_ALLOCATE(xx,(npts))
 ABI_ALLOCATE(yy,(npts))
 ABI_ALLOCATE(zz,(npts))
 ABI_ALLOCATE(Lgrid%weight,(npts))

 call build_lebedev_grid(seq_idx,xx,yy,zz,Lgrid%weight)

 Lgrid%npts = lebedev_npts(seq_idx)
 ABI_ALLOCATE(Lgrid%versor,(3,Lgrid%npts))
...
! *********************************************************************
 ...
 Lgrid%npts=0
 if (associated(Lgrid%versor))  then
   ABI_DEALLOCATE(Lgrid%versor)
 end if
 if (associated(Lgrid%weight))  then
   ABI_DEALLOCATE(Lgrid%weight)
 end if
...
!arrays
 ...
 interface
   !double complex function cplx_func(vers,int_pars,real_pars,cplx_pars)
