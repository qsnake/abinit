...
!!  sum(3)=$\displaystyle \frac{1}{2\pi}\sum_{\vec G}
!!          \frac{|n(\vec G)|^2}{G^2}\left( 1-cos(GR) \right)$,
!! and for the true Coulomb interaction, $1/r$, as
!!  sum(2)=$\displaystyle \frac{1}{2\pi}\sum_{\vec G \neq 0}\frac{|n(\vec G)|^2}{G^2}$.
!! Also give
!!  sum(1)=$\displaystyle \frac{1}{2\pi}\sum_{\vec G \neq 0}
!!          \frac{|n(\vec G)|^2}{G^2}\left( 1-cos(GR) \right)$.
!!
!! INPUTS
...
!!
!! OUTPUT
!!  sum(1)=Hartree energy with cutoff interaction without $G=0$ term
!!  sum(2)=Hartree energy with full interaction without $G=0$ term
!!  sum(2)=Hartree energy with cutoff interaction including $G=0$ term
...
!arrays
 ...
 real(dp),intent(out) :: sum(3),vhartr(cplex*nfft)
...
!        MF use cutoff Coulomb interaction
         sum(re)=sum(re)+den*(1._dp-cos(2._dp*pi*sqrt(gs)*rcut))
...
!  End loop on i3
 ...
 sum(3)=sum(re)+(rhog(re,1)**2+rhog(im,1)**2)*0.5_dp*(2*pi*rcut)**2
...
!write(std_out,*)' hartre : zero vhartr'
!sum(:)=0.0_dp
!vhartr(:)=0.0_dp
!ENDDEBUG
