...
!!
!! COPYRIGHT
!! Copyright (C) 2002-2012 ABINIT group (SMazevet)
...
!BEGIN EXECUTABLE SECTION
 ...
 open(unit=111, file=trim(filnam_out_sig)//'_eps', status='replace')
 write(111,'(a)')'#energy (eV),sigma_1(Ohm-1cm-1),sigma_2(Ohm-1cm-1),epsilon_1,epsilon_2'
 open(unit=112, file=trim(filnam_out_sig)//'_abs', status='replace')
 write(112,'(a)')'#energy(eV),nomega,komega,refl.,abso.(cm-1)'
...
!  A special treatment of the case where eps2 is very small compared to eps1 is needed
   if(eps2**2 > eps1**2 * tol12)then
     ...
     abso(ip)=four_pi*fcti(ip)*ohmtosec/(nomega(ip)*Sp_Lt*ten)
     ...
     komega=sqrt(half*(-eps1 + sqrt(eps1**2 + eps2**2)))
     abso(ip)=two*sqrt(-eps1)*pole*ohmtosec/Sp_Lt/ten
   end if

   refl=((one-nomega(ip))**2 + komega**2)/ &
&   ((one+nomega(ip))**2 + komega**2)

   write(111,'(5e18.10)') Ha_eV*pole,fcti(ip)*Ohmcm,sigma2*Ohmcm,eps1,eps2
   write(112,'(5e18.10)') Ha_eV*pole,nomega(ip),komega,refl,abso(ip) 
