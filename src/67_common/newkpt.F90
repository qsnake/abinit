...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (DCA, XG, GMR, ZL, AR, MB)
...
!!  mpi_enreg2=informations about MPI parallelization, for the output wf file!!  mpw1=maximum allowed number of planewaves at any k, for the input wf file
...
!!      wffreadskipk,wrtout,xme_init,xproc_init
!!
!! SOURCE
...
!scalars
 ...
 integer,intent(in) :: ireadwf,mband2,mcg,mkmem1,mkmem2,mpw1,mpw2,nkpt1,nkpt2
 integer,intent(in) :: nspinor1,nspinor2,nsppol1,nsppol2,nsym,optorth,prtvol,restart
 integer,intent(in) :: sppoldbl,unkg2
...
!arrays
 ...
 integer,intent(in) :: nband2(nkpt2*nsppol2),ngfft(18),npwarr1(nkpt1)
 integer,intent(in) :: npwarr2(nkpt2),symrel(3,3,nsym)
...
!scalars
 ...
 integer :: mband1,mband_rd,mband_rw,mcg_aux,me1,me2,mgfft,my_nspinor1,my_nspinor2
 integer :: nb_band,nbd1,nbd1_rd,nbd2,nkpt_eff,nproc1,nproc2,npw1,npw2,nsp
...
!DEBUG
!write(std_out,*)' newkpt : paralbd=',mpi_enreg%paralbd
!if(restart==2)stop
!write(std_out,*)' newkpt : ireadwf,nkpt2= ',ireadwf,nkpt2
!write(std_out,*)' newkpt : npwarr2(:)=',npwarr2(:)
!write(std_out,*)' newkpt : mkmem1=',mkmem1
!write(std_out,*)' newkpt : set debug=1'
!debug=1
!write(std_out,*)' newkpt : cg array='
!do ikpt2=1,nkpt2
!nbd2=nband2(ikpt2)
!npw2=npwarr2(ikpt2)
!do iband=1,nbd2
...
!do ipw=1,1
!write(std_out,'(3i4,2es20.10)' )ikpt2,iband,ipw,cg(:,icg+ipw)
!end do
!icg=icg+npw2
!end do
!end do
...
!Init MPI data
 call xme_init(mpi_enreg1,me1)
 call xme_init(mpi_enreg2,me2)
 call xproc_init(mpi_enreg1,nproc1)
 call xproc_init(mpi_enreg2,nproc2)
...
&   '           and a non-spinor wf to start a spin-polarized wf.'
 ...
 my_nspinor1=max(1,nspinor1/mpi_enreg1%nproc_spin)
 mY_nspinor2=max(1,nspinor2/mpi_enreg2%nproc_spin)
...
!DEBUG
!write(std_out,*)' newkpt :me,restart before loop ',me,restart
!if(restart==2)stop
!ENDDEBUG
...
&         ' for ikpt=',ikpt2
         if(mpi_enreg2%paral_compil_kpt==1)then
!          This message might be overwritten in parallel
...
&                 ' for ikpt=',ikpt2,' by node ',iproc
           ...
           if((mpi_enreg2%paralbd==0) .or. (mpi_enreg2%paralbd>1))then
             write(message, '(a,i6,a,i8,a,i4,a,i4)' )&
&             'P newkpt: treating ',nbd2,' bands with npw=',npw2,&
...
!      End of restart==1
     ...
     if(mpi_enreg2%paral_compil_kpt==1)then
       test_cycle=0
       if(minval(abs(mpi_enreg2%proc_distrb(ikpt2,1:nbd2,isppol2)-me2))/=0) test_cycle=1
...
!    DEBUG
!    write(std_out,*)' newkpt : before randac, nbd1=',nbd1
!    stop
!    ENDDEBUG
...
!    have been placed already in cg, at the k-point location where they are needed
     ...
     if(formeig==0) ABI_ALLOCATE(occ_k,(mband_rw))
...
&       kg1,kg2_k,kptns1,kptns2,mband_rw,mband_rw,mcg,mcg,mgfft,mpi_enreg1,mpi_enreg2,mpw1,mpw2,&
&       nbd1_rd,nbd2,&
&       ngfft,nkpt1,nkpt2,npw1,npw2,nspinor1,nspinor2,nsym,&
&       occ_k,occ_k,optorth,restart,rprimd,sppoldbl,symrel,tnons)
     else
...
!    DEBUG
!    write(std_out,*)' newkpt : final content of cg,ikpt,icg=',ikpt2,icg
!    do iband=1,nbd2
!    do ipw=1,npw2
!    if(aux_stor==0)then
!    write(std_out,'(3i4,2es20.10)' )&
...
!    ENDDEBUG
     ...
     eigen(1+band_index:nbd2*(2*nbd2)**formeig+band_index)=&
...
!    occ(1+band_index:nbd2+band_index)=occ_k(1:nbd2)
     ...
     if(formeig==0) ABI_DEALLOCATE(occ_k)
...
!  as 1 always, since the transfer has been made in wfsinp .
   localrdwf=1
   call pareigocc(eigen,formeig,localrdwf,mpi_enreg2,mband2,nband2, &
&   nkpt2,nsppol2,occ,1)
 end if
...
!kg2_k(:,1:npw2)=kg2(:,1+ikg2:npw2+ikg2)
!do iband=1,nbd2
!write(std_out,'(a,4i4)' )' new band, ikpt2,iband,npw2,icg=',ikpt2,iband,npw2,icg
!do ipw=1,npw2
!write(std_out,'(4i4,2es20.10)' )ipw,kg2_k(:,ipw),cg(:,icg+ipw)
!end do
!icg=icg+npw2
!end do
!ikg2=ikg2+npw2
!end do
!write(std_out,*)' newkpt : exit'
!ENDDEBUG
