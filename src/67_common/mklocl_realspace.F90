...
!Local variables-------------------------------
 ...
 integer :: comm,countParSeconde,i1,i2,i3,ia,ia1,ia2,igeo,ii,ind,itypat,ix,iy,iz,jj
 integer :: kk,me,me_fft,n1,n2,n3,n_interpol,nproc,nproc_fft,tpsStart,tpsStop
...
!arrays
 ...
 real(dp),allocatable :: gridcart(:,:),gridred(:,:)
...
!  MT may-2011, do these lines correct the problem ?:
   call xcomm_init(mpi_enreg,comm,spaceComm_bandfft=mpi_enreg%comm_fft)
...
!    Compute Hartree's potential from rhor.
     ...
     call PSolver_hartree(dtset, entmp, mpi_enreg, rhor, rprimd, vhartr, wvl)


     ABI_ALLOCATE(gxyz,(3, dtset%natom))
!    calculate local part of the forces grtn (inspired from BigDFT routine)
     call local_forces_new(geocode,me, dtset%ntypat, dtset%natom, &
...
!    write(std_out,*) 'not implemented for the moment, wait a minute...'
!    stop
   end if
...
! *********************************************************************
 ...
 if(nproc<0)then
   write(std_out,*) ' nproc should not be negative '
   stop
 end if
...
!Arguments -------------------------------
  ...
  character(len=1), intent(in) :: geocode
  integer, intent(in) :: iproc,ntypes,nat,n1,n2,n3
  real(kind=8), intent(in) :: hxh,hyh,hzh
...
!Local variables -------------------------
  ...
  integer :: isx,isy,isz,iex,iey,iez,i1,i2,i3,j1,j2,j3,ind,iat,ityp,nloc,iloc
