...
!!
!! PARENTS
!!      berry_linemin,berryphase_new,cgwf,getcgqphase,make_grad_berry
!!      update_mmat
!!
!! CHILDREN
...
!Local variables -------------------------
!scalars
 integer :: count,iband,info,ipw,jband,jband1,jpw,my_nspinor,pwmax,pwmin
...
!inclusion of nspinor = 2 is at an early stage of development, this do-nothing code
!is a place holder to be updated later
!J Zwanziger 30 November 2011
 my_nspinor = nspinor

!Check if the values of ddkflag and job are compatible

 if ((job /= 0).and.(job /= 1).and.(job /= 10).and.(job /= 11).and.(job/=20)) then
   write(message,'(a,a,a,a,i3,a,a)')ch10,&
&   ' smatrix: BUG - ',ch10,&
&   '   job is equal to ',job,ch10,&
&   '   while only the values job = 0, 1, 10, 11 or 20 are allowed.'
   call wrtout(std_out,message,'PERS')
   call leave_new('PERS')
 end if

 if (ddkflag == 1) then
   if ((job == 0).or.(job == 10).or.(job==20)) then
     write(message,'(a,a,a,a,i3,a,a)')ch10,&
&     ' smatrix: BUG - ',ch10,&
&     '   job is equal to ',job,ch10,&
...
!      if (nspinor == 2 .and. &
!      ( (mod(iband,2)/=0 .and. mod(jband,2)==0) .or. &
!      &          (mod(iband,2)==0 .and. mod(jband,2)/=0) ) ) smat_k(:,iband,jband)=zero
         ...
         do iband = 1, nband_occ
           do ipw = 1, npw_k1
             ...
             if (jpw > 0) then
               ...
               wfr = cgq(1,icg1+(iband-1)*npw_k2+jpw)*pwnsfac_k(3,jpw)&
&               -cgq(2,icg1+(iband-1)*npw_k2+jpw)*pwnsfac_k(4,jpw)
               wfi = cgq(1,icg1+(iband-1)*npw_k2+jpw)*pwnsfac_k(4,jpw)&
&               +cgq(2,icg1+(iband-1)*npw_k2+jpw)*pwnsfac_k(3,jpw)

               cg1_k(1,(jband1-1)*npw_k1+ ipw) = &
&               cg1_k(1,(jband1-1)*npw_k1 + ipw) + &
&               smat_inv(1,iband,jband)*wfr + smat_inv(2,iband,jband)*wfi

               cg1_k(2,(jband1-1)*npw_k1 + ipw) = &
&               cg1_k(2,(jband1-1)*npw_k1 + ipw) - &
...
&               smat_inv(1,iband,jband)*wfi + smat_inv(2,iband,jband)*wfr
 ...
 end if         ! job > 0 .and. job < 20

 if(job == 20) then ! special case transfering cgq to cg1_k without use of S^{-1}, used in 
!  magnetic field case, see update_mmat.F90
   ...
   if (itrs == 10 .or. itrs == 11) then
     ...
     do jband = minbd, maxbd
       ...
       do ipw = 1, npw_k1
         ...
         if (jpw > 0) then
           wfr = cgq(1,icg1+(jband1-1)*npw_k2+jpw)*pwnsfac_k(3,jpw)&
&           -cgq(2,icg1+(jband1-1)*npw_k2+jpw)*pwnsfac_k(4,jpw)
           wfi = cgq(1,icg1+(jband1-1)*npw_k2+jpw)*pwnsfac_k(4,jpw)&
&           +cgq(2,icg1+(jband1-1)*npw_k2+jpw)*pwnsfac_k(3,jpw)

           cg1_k(1,(jband1-1)*npw_k1 + ipw) = wfr
           cg1_k(2,(jband1-1)*npw_k1 + ipw) = wfi
