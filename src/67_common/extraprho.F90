...
!!
!! CHILDREN
!!      atm2fft,atmlength,extrapwf,jellium,leave_new,rhoij_alloc,wrtout
!!
!! SOURCE
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: istep,mcg,mgfft,mqgrid,nfft,ntypat,usepaw
...
!arrays
 ...
 real(dp),intent(in) :: ylm(dtset%mpw*dtset%mkmem,psps%mpsang*psps%mpsang*psps%useylm),zion(ntypat),znucl(ntypat)
...
!arrays
 ...
 real(dp),allocatable :: deltarho(:),dummy_in(:),dummy_out(:),gauss(:,:),rhoijtmp(:,:),work1(:)
...
& gauss,gmet,gprimd,dummy_out,dummy_out,gsqcut,&
& mgfft,mpi_enreg,mqgrid,dtset%natom,nattyp,nfft,ngfft,ntypat,&
& optatm,optdyfr,optgr,optn,optn2,optstr,optv,dtset%paral_kgb,&
& pawtab,ph1d,qgrid,dummy3,dummy_in,dummy6,dummy6,&
& ucvol,usepaw,dummy_in,dummy2,dummy_in)
...
!---------------------------------------------------------------
 ...
 if (usepaw==1) then
   ...
   if (ind2==0) then
     ...
     call rhoij_alloc(pawrhoij(1)%cplex,nlmn,pawrhoij(1)%nspden,dtset%nspinor,dtset%nsppol,&
&     scf_history%pawrhoij(:,ind1new),dtset%typat,mpi_enreg=mpi_enreg)
     ABI_DEALLOCATE(nlmn)
   end if
   ...
   do iatom=1,dtset%natom
     ...
     if (hasmoved) then
       ABI_ALLOCATE(rhoijtmp,(lmn2_size,nspden))
       rhoijtmp=zero
...
!    (cannot use rhoij_copy here because update for single atom)
     ...
     scf_history%pawrhoij(iatom,ind1new)%rhoijp(1:cplex*nselect,1:nspden)=pawrhoij(iatom)%rhoijp(1:cplex*nselect,1:nspden)
