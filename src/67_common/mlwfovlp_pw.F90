...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2012 ABINIT group (BAmadon,FJollet,T Rangel)
...
!!
!! CHILDREN
!!      leave_new,leave_test,wrtout,xcomm_init,xme_init,xproc_init,xsum_mpi
!!
!! SOURCE
...
!write(std_out,*) "index for g points has been computed"
 call leave_test() 
 call xsum_mpi(invpwk,spaceComm,ierr)
...
!              if(me==1 .and. ikpt2==4)write(301,*)'ipw,iband2,index',ipw,iband2,index,cg_read(:,index)
       ...
       end if !MPI reading from a file
!      
       npw_k=npwarr(ikpt1)
       npw_k2=npwarr(ikpt2)
       do ig3=1,n3
...
!DEBUG
!write(std_out,*)' mlwfovlp_pw : exit'
!stop
!ENDDEBUG
