...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2012 ABINIT group (BAmadon,FJollet)
...
!Local variables---------------------------
!scalars
 integer :: iatom,icb,ikpt,ikpt1,intot,isppol,itypat,jj,mband_
...
!^^^^^^^^^^^^^^^^read wannier90.nnkp^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 if(lwanniersetup==0) then  !this part is not coded for nsppol>1
   ...
   open(unit=111,file=trim(filew90_nnkp),form='formatted',status='old')
   read(111,*)
   read(111,*) nntot , mband_, nwan(1)
...
&   trim(filew90_nnkp),'wannier90.nnkp has been read !'
   ...
   ' exclude bands is not given in this case (not implemented) : STOP',ch10
   call wrtout(std_out,message,'COLL')
   call leave_new('COLL')
...
!DEBUG
!write(std_out,*)' mlwfovlp_setup : exit'
!stop
!ENDDEBUG
