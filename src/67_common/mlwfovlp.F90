...
!!
!! COPYRIGHT
!! Copyright (C) 2005-2012 ABINIT group (BAmadon,FJollet,TRangel)
...
!
 if(MPI_enreg%paral_spin==1) then
   write (message,'(3a)')' mlwfovlp : ERROR- ',ch10,&
&   '  Parallelization over spinorial components not yet available !'
   call wrtout(std_out,message, 'COLL')
   call leave_new('COLL')
 end if
 if(mkmem==0) then
   write(std_out,*) "WARNING: mkmem==0 is not supported in mlwfovlp"
   stop
 end if
 if(nsppol==2) then
   if(spin==1) then
...
&     '   mlwfovlp:  Calculating matrices for spin-down polarization  ',ch10
   ...
   if(spin <0 .or. spin>2) then
     write(message, '(3a)' ) ch10,&
&     '   mlwfovlp:  spin variable should be equal to 0, 1 or 2 ',ch10
     call wrtout(ab_out,message,'COLL')
     call wrtout(std_out,  message,'COLL')
     stop
   end if
...
!    write(std_out,*) ikpt+(isppol-1)*nkpt,nkpt
     ...
     do iband=1,nband_k
       if(iband.gt.mband) then
         write(std_out,*)"error mband",iband,mband,nband_k
         stop
       end if
...
!    ----------------------------------------------------------------------------------------------
     ...
     if (have_wan2abi) then
       if(dtset%nspinor==2) then
         write(std_out,*) "WARNING: nspinor==2 is not supported in have_wan2abi, inside mlwfovlp"
         stop
       end if
...
!      all bands are inside the window!
       mwan=nwan(isppol) ; w90version=1.1 ; have_disentangled=.not.ALL(lwindow)
       if (have_disentangled) then
         ABI_ALLOCATE(ndimwin,(nkpt))
!        call read_chkunit(seed_name(isppol),nkpt,ndimwin,ierr)
!        if (ierr==-1) stop 'internal error, likely all bands are inside the window'
       end if
       if (.not.allocated(ndimwin))  then
         ABI_ALLOCATE(ndimwin,(0))
       end if

       call WrapInitWanData(WanData,mband,nkpt,nsppol,nntot,mwan,nwan(isppol),have_disentangled,eigen,&
&       band_in(:,isppol),lwindow(:,:,isppol),wann_centres(:,:,isppol),wann_spreads(:,isppol),spreadw,U_matrix(:,:,:,isppol),&
&       U_matrix_opt(:,:,:,isppol),Hdr,w90version,'Wannier90 Results')

       call DumpWanData(WanData,dtfil%fnameabo_wan,Dtset%accesswff)
       call DestroyWanData(WanData)
       ABI_DEALLOCATE(ndimwin)
     end if
...
!  collect output of  wannier90 from different processors
   ...
   call xsum_mpi(U_matrix,spaceComm,ierr)
   call xsum_mpi(U_matrix_opt,spaceComm,ierr)
   call xsum_mpi(lwindow,spaceComm,ierr)
   call xsum_mpi(wann_centres,spaceComm,ierr)
   call xsum_mpi(wann_spreads,spaceComm,ierr)


!  CALL SILVESTRELLI'S APPROACH TO EVALUATE vdW INTERACTION ENERGY USING MLWF!!
!  ----------------------------------------------------------------------------------------------
   if (dtset%vdw_xc==10.or.dtset%vdw_xc==11.and.rank==master) then ! vdw_xc==10 or 11 starts the vdW interaction using MLWFs
     ...
     call evdw_wannier(csix,corrvdw,mwan,natom,nsppol,nwan,dtset%vdw_nfrag,dtset%vdw_supercell,&
&     dtset%vdw_typfrag,dtset%vdw_xc,rprimd,wann_centres,wann_spreads,xcart)
     ABI_DEALLOCATE(csix)
   end if
