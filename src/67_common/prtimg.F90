...
!!
!! CHILDREN
!!      destroy_results_img,gather_results_img,metric,prtxvf,wrtout,xredxcart
!!
!! SOURCE
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: dynimage(nimage_tot),imgmov,iout,nimage,nimage_tot,prtvolimg
...
! ****************************************************************
 ...
 if (mpi_enreg%me_one_img/=0) return
 ...
 if (prtvolimg==1.or.prtvolimg==2) then
   ...
   if (test_img) then
     if (mpi_enreg%me==0)  then
       ABI_ALLOCATE(resimg_all,(nimage_tot))
     end if
...
!===== First option for the printing volume ===
 if (prtvolimg==1.and.mpi_enreg%me==0) then
   ...
   prtvel=0;if (imgmov==0.or.imgmov==9.or.imgmov==13) prtvel=1
...
!      Positions, forces and velocities
       ...
       iatfix_img=0
       call xredxcart(resimg_all(ii)%natom,1,resimg_all(ii)%rprim,xcart_img,resimg_all(ii)%xred)
