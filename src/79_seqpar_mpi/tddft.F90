...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2012 ABINIT group (XG, JYR, MB, MBELAND, SHAMEL)
...
!!
!! CHILDREN
!!      fourdp,fourwf,hartre,hdr_skip,int2char4,leave_new,leave_test,matr3inv
!!      mpi_barrier,mpi_bcast,mpi_gatherv,mpi_reduce,mpi_scatterv,rdnpw
!!      read_wfrspa,rwwf,sort_dp,sphereboundary,timab,wffkg,wrtout,xbarrier_mpi
!!      xcomm_world,xdefineoff,xexch_mpi,xsum_mpi,zhpev
!!
!! SOURCE
...
!Local variables-------------------------------
 ...
 integer :: nsp,nstate_k, nstate_occ, nstate_unocc, nexcit_pol(nsppol)
 ...
 real(dp) :: buffer,buffer_inv,diffeig,eigbnd,eigunocc,emax_win
 real(dp) :: factor,ff,flargest,fnext,fr_invsquare,fr_power
 real(dp) :: fnext1,fnext2
 real(dp) :: normint,product,saa,sab,sbb
 real(dp) :: sumx
 ...
 integer :: accesswff,action,me,nmaster,sender,source,sread,sskip
 integer :: formeig,headform,icg,icg0,ikg,nband_disk,nband_k_,option,optkg
...
!Init mpi_comm
 ...
 am_master=.true.
 master = 0
 nproc_loc = xcomm_size(spaceComm) !Init ntot proc max
 me_loc    = xcomm_rank(spaceComm) !Define who i am
...
!need to run in MPI I/O case
 if (wffnew%accesswff == IO_MODE_MPI ) then
   save_accesswff=wffnew%accesswff
   wffnew%accesswff = IO_MODE_FORTRAN
 else
!  Do not store value but set to have save_accesswff /= 1
   save_accesswff = IO_MODE_FORTRAN
 end if
...
!need to collect wavefunctions from processors to master
 ...
 accesswff=wffnew%accesswff

 if(am_master)then
#if defined HAVE_MPI
   ABI_ALLOCATE(cg_tmp,(2,mpw*nspinor*mband*nsppol))
#else
   if(mkmem==0)ABI_ALLOCATE(cg_tmp,(2,mpw*nspinor*mband*nsppol))
#endif
 end if
...
!  Define offsets, in case of MPI I/O
   call WffKg(wffnew,1)
   call xdefineOff(formeig,wffnew,mpi_enreg,dtset%nband,npwarr,nspinor,nsppol,nkpt)
 end if  !mkmem = 0
 icg=0
...
#if defined HAVE_MPI
       sread=-1
       if(minval(abs(mpi_enreg%proc_distrb(ikpt,1:nband_k_,isppol)-me))==0) sread=me
#endif
       if(sread==me)then
         headform=0 ; icg0=0 ; option=-2 ; optkg=1
         call rwwf(cg_disk,eig_dum,formeig,headform,&
&         icg0,ikpt,isppol,kg_disk,mband,mcg_disk,mpi_enreg,nband_k_,nband_disk,&
&         npw_k,nspinor,occ_dum,option,optkg,tim_rwwf,wffnew)
         if(nband_k_/=nband_disk)then
...
!        I am the master, and I receive the data
         if ( action==2.or.action==3) then
           ...
           if(mkmem/=0 .and. action==2)then
             call xexch_mpi(kg(:,1+ikg:npw_k+ikg),3*npw_k,source,kg_disk,nmaster,spaceComm,ierr)
             call xexch_mpi(cg(:,icg+1:icg+nband_k_*npw_k*nspinor),2*nband_k_*npw_k*nspinor &
&             ,source,cg_disk,nmaster,spaceComm,ierr)
           else
             call xexch_mpi(kg_disk,3*npw_k,source,kg_disk,nmaster,spaceComm,ierr)
             call xexch_mpi(cg_disk,2*nband_k_*npw_k*nspinor,source,cg_disk,nmaster,spaceComm,ierr)
           end if
...
#else
       if(mkmem==0)cg_tmp(:,icg+1:icg+nband_k_*npw_k*nspinor)=cg_disk(:,:)
#endif
     end if
...
!    if (mkmem/=0) then
     ...
     if (dtset%usewvl == 0) then
       sskip=0
       if(minval(abs(mpi_enreg%proc_distrb(ikpt,1:nband_k_,isppol)-me))==0)sskip=1
     end if
#endif
     if(sskip==1)then
...
!    rewind the kpgsph data file on unit unkg
     ...
     ikpt=1 ; isppol=1 ; nsp=nspinor
     call rdnpw(ikpt,isppol,nband_k(isppol),npw_k,nsp,0,dtfil%unkg)
!    Read k+g data
...
!need to run in MPI I/O case
 if ( save_accesswff == 1 )  wffnew%accesswff = IO_MODE_MPI
...
!DEBUG
!write(std_out,*) mkmem
!write(std_out,'(a)') 'After reading the wavefunction'
!call flush(6)
!call wrtout(std_out,message,'COLL')
!ENDDEBUG
...
!0, only 2   are in memory at the same time
 if (mkmem/=0) then
   ...
   ABI_ALLOCATE(wfrspa,(ndiel4,ndiel5,ndiel6,2))
 end if
...
&           (npw_k*nband_k(1)) : iband*npw_k+(isppol-1)*(npw_k*nband_k(1)))
           ...
           cwavef(:,1:npw_k)=cg(:,1+(iband-1)*npw_k+(isppol-1)* &
&           (npw_k*nband_k(1)) : iband*npw_k+(isppol-1)*(npw_k*nband_k(1)))
         end if
#endif
...
!        Possibility of writing to disk
         ...
         if (mkmem/=0) then
           ...
           fnametmp_tdwf_tag=trim(dtfil%fnametmp_tdwf)//tag
           write(std_out,*)'fileout : ',fnametmp_tdwf_tag
           open(tmp_unit,file=fnametmp_tdwf_tag,form='unformatted',status='unknown')
!          DEBUG
!          write(std_out,'(a)')' After opening tmp_unit '
!          END DEBUG
           write(tmp_unit)iband,isppol,eigen(iband+(isppol-1)*nband_k(1))
!          DEBUG
!          write(std_out,'(a)')' After writing haed '
!          END DEBUG
...
!          DEBUG
!          write(std_out,'(a)')' After writing wavefunction '
!          END DEBUG
...
#else
   if(mkmem==0)ABI_DEALLOCATE(cg_tmp)
#endif
 end if
...
&     iexcit,jexcit
   ...
   open(tmp_unit2, file=trim(dtfil%fnametmp_tdexcit),form='unformatted', recl=recl, &
&   access='DIRECT')
...
!Need to dispatch the elements to compute to the different processes
 ...
 tmpbuf=zero
 call MPI_Scatterv(excit_coords(1,1),counts,displs,MPI_INTEGER,tmpbuf,count, &
& MPI_INTEGER,0,spaceComm,ierr)
 excit_coords(:,1)=tmpbuf(:)
 tmpbuf=zero
 call MPI_Scatterv(excit_coords(1,2),counts,displs,MPI_INTEGER,tmpbuf,count, &
& MPI_INTEGER,0,spaceComm,ierr)
...
&         dtfil%fnametmp_tdwf,eigbnd,iband,isppol,1,ndiel4,ndiel5,ndiel6,wfrspa(:,:,:,1))
!        Read unoccupied band
         call read_wfrspa(index_state(iunocc2+nband_occ(isppol2)+(isppol2-1)*nband_k(1)),&
&         dtfil%fnametmp_tdwf,eigbnd,iband,isppol,2,ndiel4,ndiel5,ndiel6,wfrspa(:,:,:,2))
       end if
