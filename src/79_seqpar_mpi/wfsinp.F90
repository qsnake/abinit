...
#endif
!Arguments ------------------------------------
 integer, intent(in) :: exchn2n3d,formeig,headform0,localrdwf,mband,mban_dp_rd,mcg,mcg_disk,mkmem
 ...
 real(dp), intent(out) :: cg(2,mcg),cg_disk(2,mcg_disk)
...
! integer :: ipw ! commented out below
 integer :: isppol_trial,master,me,mgfft,my_nspinor,my_nspinor0
 ...
 integer :: read_cg,read_cg_disk,sender,spaceComm,tim_rwwf
...
&   '  formeig=',formeig,' , but the only allowed values are 0 or 1.'
 ...
 my_nspinor0=max(1,nspinor0/mpi_enreg%nproc_spin)

 if(mkmem==0)then
   write(std_out,*)' wfsinp : mkmem==0 not yet coded, stop'
   stop
 end if
 ...
 if(mkmem/=0) ABI_ALLOCATE(icg_k,(nkpt,nsppol))
...
!Compute the locations of the blocks in cg, eig and occ
 ...
 do isppol=1,nsppol
   ...
   do ikpt=1,nkpt
     ...
     if(minval(abs(mpi_enreg%proc_distrb(ikpt,1:nbd,isp)-me))/=0)test_cycle=1
       ...
       do ikassoc_trial=1,nkassoc
         ...
         if(localrdwf==1)then
           if(ikpt_trial/=0)then
             ...
             if(minval(abs(mpi_enreg%proc_distrb(ikpt_trial,1:nbd,isp)-me))/=0)ikpt_trial=0
           end if
         end if
#endif
         ...
         if(ikpt_trial/=0)then
           ...
           if(localrdwf==0)then
             do iproc=1,nproc_max
               ...
               nband_trial=nband(ikpt_trial+(isppol_trial-1)*nkpt)
               nbd=min(nband_trial,nbd_max);isp=min(isppol_trial,isp_max)
               if(minval(abs(mpi_enreg%proc_distrb(ikpt_trial,1:nbd,isp)-(iproc-1)))/=0)my_ikpt=0
               if(my_ikpt/=0)then
                 nband_trial=nband(ikpt_trial+(isppol_trial-1)*nkpt)
...
!      DEBUG
!      write(std_out,*)' me,ikpt,isppol,icg=',me,ikpt,isppol,icg
!      stop
!      if(ikpt0==2)stop
!      ENDDEBUG
...
!          The transfer of occ should be done.
         ...
         write(std_out,*)' wfsinp: transfer of first-order eigs not yet coded !'
       end if
...
!        Exclude rwwf time from wfsinp time
         call timab(726,1,tsec)
         if(mpi_enreg0%paralbd==0)tim_rwwf=4
         if(mpi_enreg0%paralbd==1)tim_rwwf=11
         if(mkmem==0)stop 'not allowed yet'
!        In particular, kg_k is not yet available

         if(master == me ) then
           call rwwf(cg_disk,eig_k,formeig,0,0,ikpt0,isppol0,kg_k,mban_dp_rd,mcg_disk,mpi_enreg0,nband_rdk,&
&           nband_rdk,npw0_k,my_nspinor0,occ_k,2,1,tim_rwwf,wffnow)
         end if
         call timab(726,2,tsec)
!        Fill the other blocks
       else if(mkmem/=0)then

         call timab(727,1,tsec)

!        DEBUG
!        if(me==0 .and. ikpt0==9)then
!        write(std_out,*)' wfsinp : cg array, before isppol_trial loop, ikpt0=',ikpt0
!        do ipw=1,15
!        write(std_out,'(i4,2es20.10)' )ipw,cg(:,ipw)
!        end do
!        end if
...
!                  write(std_out,*)' wfsinp: iband,ipw,cg_disk for ikpt_trial=2'
!                  write(std_out,*)' nban_dp_rdk,npw0_k=', nban_dp_rdk,npw0_k
!                  do iband=1,nban_dp_rdk
!                  do ipw=1,npw0_k
...
&                   eig0_k,eig_k,exchn2n3d,formeig,gmet0,gmet,icg_disk,icg_trial,&
&                   ikpt0,ikpt10,ikpt_trial,indkk,inplace,isppol_trial,&
&                   istwfk0,istwfk,kg0_k,kg_k,kptns0,kptns,nban_dp_rdk,nband_rdk,&
&                   mcg_disk,mcg,mgfft,mpi_enreg0,mpi_enreg,mpw0,mpw,nban_dp_rdk,nband_trial,&
...
!        DEBUG
!        write(std_out,*)' wfsinp : transferred cg'
!        if(ikpt0==2)stop
!        ENDDEBUG
...
!    DEBUG
!    if(me==0 .and. ikpt0==9)then
!    write(std_out,*)' wfsinp : cg array, ikpt0=',ikpt0
!    do ipw=1,15
!    write(std_out,'(i4,2es20.10)' )ipw,cg(:,ipw)
!    end do
!    end if
...
!ENDDEBUG
 ...
 if(mkmem/=0)ABI_DEALLOCATE(icg_k)
 ABI_DEALLOCATE(band_index_k)

!DEBUG
!write(std_out,*)' wfsinp : exit'
!write(std_out,*)cg(:,1)
!write(std_out,*)cg(:,2)
!stop
!ENDDEBUG
