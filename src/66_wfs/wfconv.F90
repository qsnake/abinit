...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2012 ABINIT group (XG,TD)
...
!!
!! CHILDREN
!!      envlop,getph,getspinrot,kpgsph,leave_new,mati3inv,ph1d3d,pw_orthon
!!      sphere,sphereboundary,timab,wrtout,xcomm_init,xsum_mpi
!!
!! SOURCE
...
!scalars
 ...
 integer,intent(in) :: ikpt2,inplace,isppol2,mband1,mband2,mcg1,mcg2,mgfft,mpw1
 integer,intent(in) :: mpw2,nbd1,nbd2,nkpt1,nkpt2,nspinor1,nspinor2,nsym
 integer,intent(in) :: optorth,restart,sppoldbl
...
!integer,save :: test_call=0
 ...
 integer :: istwf10_k,istwf1_k,istwf2_k,isym,itimrev,jsign,n1,n2,n3,n4
 integer :: n5,n6,nbremn,npwtot,nspinor_index,nspinor1_this_proc,nspinor2_this_proc
 integer :: order,ortalgo,seed,spaceComm
 real(dp) :: ai,ar,arg,bi,br,eig_tmp,spinrots,spinrotx
...
!DEBUG
!write(std_out,*)' wfconv : enter '
!write(std_out,*)' wfconv : isppol2',isppol2
!write(std_out,*)' wfconv : npw1,npw2,cg1(:,1)= ',npw1,npw2,cg1(:,1)
!test_call=test_call+1
!write(std_out,*)' wfconv : test_call= ',test_call
!if(nspinor2==2)stop
!if(ikpt2==2)write(std_out,*)' cg1(:,1)=', cg1(:,1)
!write(std_out,*)' cg1=',cg1(:,1+icg1)
!do ipw=1,mcg1
!write(std_out,*)ipw,cg1(1:2,ipw)
!end do
!ENDDEBUG
!DEBUG GZ
!read(17,'(a2)') direction
!write(std_out,*) 'direction',direction
!ENDDEBUG
...
!This line is the reason why ngfft_now has to be introduced
 ...
 fftalg=ngfft_now(7)
 if (mpi_enreg1%paral_fft == 1) then
   mpi_enreg1%num_group_fft=ikpt1 + (isppol2-1)*nkpt1
 else
   mpi_enreg1%num_group_fft=0
 end if
 if (mpi_enreg2%paral_fft == 1) then
   mpi_enreg2%num_group_fft=ikpt1 + (isppol2-1)*nkpt1
 else
   mpi_enreg2%num_group_fft=0
 end if

!Parallelization over spinors management
 nspinor1_this_proc=max(1,nspinor1/mpi_enreg1%nproc_spin)
 nspinor2_this_proc=max(1,nspinor2/mpi_enreg2%nproc_spin)
...
!DEBUG
!write(std_out,*)' wfconv : convert= ',convert
!stop
!ENDDEBUG
...
!    Convert 1D phase factors to 3D phase factors exp(i 2 pi (k+G).tnons )
   ...
   if(nspinor1==2 .and. nspinor2==2)ABI_ALLOCATE(wavefspinor,(2,2*npw2))
   ABI_ALLOCATE(gbound1,(2*mgfft+8,2))
   ABI_ALLOCATE(gbound2,(2*mgfft+8,2))
...
!  defined earlier.
   nspinor_index=mpi_enreg1%me_spin+1
   do iband=iband_first,iband_last,order
     do ispinor1=ispinor_first,ispinor_last,order
       ispinor=ispinor1
       if (mpi_enreg1%paral_spin==1) then
         if (ispinor1==nspinor_index) then
           ispinor=1
         else
           if (nspinor1==2.and.nspinor2==2) &
&           wavefspinor(:,(ispinor1-1)*npw2+1:ispinor1*npw2)=zero
           cycle
         end if
       end if
...
!        is doubled also
         if (mpi_enreg2%paral_spin==0) then
...
!      Take care of possible parallelization over spinors
       if (mpi_enreg2%paral_spin==1) then
         call xsum_mpi(wavefspinor,mpi_enreg2%comm_spin,ierr)
       end if
...
!        occ_k2(2*iband-1:2*iband )=occ_tmp
       ...
       write(std_out,*)' wfconv: not yet coded, 1 !'
     end if
   end if
   if(nspinor1==2 .and. nspinor2==1)then
     if(formeig==0)then
...
!        occ_k2(iband)=occ_tmp
       ...
       write(std_out,*)' wfconv: not yet coded, 2 !'
     end if
   end if
   ...
   if(nspinor1==2 .and. nspinor2==2)ABI_DEALLOCATE(wavefspinor)
...
!DEBUG
!write(std_out,*)' wfconv : after convert, nbd1,nbd2= ',nbd1,nbd2
!write(std_out,*)' wfconv : formeig= ',formeig
!if(test_call==19)stop
!if(ikpt2==2)write(std_out,*)' cg2(:,1)=', cg1(:,1)
!stop
!ENDDEBUG
...
!    No need for a difference for different k points and/or spin-polarization
     ...
     if (mpi_enreg1%paral_compil_fft == 1) then
       npwtot=npw2
       call xcomm_init(mpi_enreg1,spaceComm,spaceComm_bandfft=mpi_enreg1%commcart)
     ...
     do iband=(nbd1/nspinor1)*nspinor2+1,nbd2
       do ispinor2=1,nspinor2_this_proc
         ispinor=ispinor2;if (nspinor2_this_proc/=nspinor2) ispinor=mpi_enreg2%me_spin+1
...
!          ENDDEBUG seq==par
           ...
           if ( mpi_enreg2%paral_compil_fft /= 1) then
             seed=(iband-1)*npw2*nspinor2 + (ispinor-1)*npw2 + ipw
           else
!            DEBUG
!            ikpt_this_proc=mpi_enreg2%tab_kpt_distrib(ikpt2)
!            seed=(iband-1)*npwtot*nspinor2+ npwtot*(ispinor-1) &
!            &                 + mpi_enreg2%bandfft_kpt(ikpt_this_proc )%ind_kg_mpi_to_seq(ipw)
!            ENDDEBUG
...
!      XG : I do not know what happens for spin-orbit here :
       if(istwf2_k==2 .and. mpi_enreg2%me_g0==1) &
...
!    Multiply with envelope function to reduce kinetic energy
     ...
     nbremn=nbd2-nbd1
     call envlop(cg2,ecut2,gmet2,icgmod,kg2,kpoint2_sph,mcg2,nbremn,npw2,nspinor2_this_proc)
...
!DEBUG
!write(std_out,*)' wfconv : before pw_orthon= ',mcg2
!if(test_call==19)then
!do ipw=1+icg2,npw2*nspinor2_this_proc*nbd2+icg2
!write(std_out,*)ipw,cg2(1:2,ipw)
!end do
!stop
!end if
!ENDDEBUG
...
!Orthogonalize GS wfs
 if (optorth==1.and.formeig==0.and.mpi_enreg2%mode_para/='b') then
!  if (optorth==1.and.formeig==0) then
   ABI_ALLOCATE(dum,(2,0))
   ortalgo=0
   call pw_orthon (icg2,0,istwf2_k,mcg2,0,mpi_enreg2,npw2*nspinor2_this_proc,nbd2,ortalgo,dum,0,cg2)
   ABI_DEALLOCATE(dum)
 end if
...
!DEBUG
!write(std_out,*)' wfconv : exit, icg2= ',icg2
!if(test_call==19)stop
!stop
!write(std_out,*)'cg2=',cg2(:,1+icg2)
!do ipw=1,mcg2
!write(std_out,*)ipw,cg2(1:2,ipw)
!end do
!write(std_out,*)
