...
!************************************************************************
 ...
 nullify(self%oper)
 ABI_ALLOCATE(self%oper,(self%nw))
 do ifreq=1,self%nw
...
! *********************************************************************
 do ifreq=1,self%nw
  ...
  call nullify_oper(self%oper(ifreq))
 enddo
 if ( associated(self%oper))       then
   ABI_DEALLOCATE(self%oper)
 end if
...
&                 -  hu(cryst_struc%typat(iatom))%jpawu * ( charge_loc(iatom,isppol) - one / two )
             endif
           else if(opt_dc==2) then  ! AMF
             if(nspinor==2) then
               write(message,'(a,i4,i4,2x,e20.10)') "Double counting not implemented for SO"
               call wrtout(std_out,  message,'COLL')
               call leave_new('COLL')
             else
               self%hdc%matlu(iatom)%mat(m1,m1,isppol,ispinor,ispinor)=  &
...
subroutine rw_self(self,mpi_enreg,paw_dmft,prtopt,opt_rw,istep_iter)
...
!type
 ...
 type(paw_dmft_type), intent(inout) :: paw_dmft
 integer,intent(in) :: prtopt
 integer,intent(in),optional :: opt_rw,istep_iter

!local variables-------------------------------
 logical :: lexist
 complex(dpc), allocatable :: buffer(:)
 integer :: iall,iatom,iatu,ier,iexist2,ifreq,im,im1,ioerr,ispinor,isppol,istepiter,istep,istep_imp
 ...
 if(optrw==2.or.(optrw==1.and.myproc==master)) then
   ...
   do iatom=1,natom
     if(self%oper(1)%matlu(iatom)%lpawu.ne.-1) then
       ...
       ABI_ALLOCATE(s_r,(ndim,ndim))
       ABI_ALLOCATE(s_i,(ndim,ndim))
...
!          ===========================
           if(self%w_type=="real") then
             tmpfil = trim(paw_dmft%filapp)//'Self_ra-omega_iatom'//tag_at//'_isppol'//tag_is//'_ispinor'//tag_is2
           else
...
!&               ,im=1,ndim),im1=1,ndim)
               write(message,'(2x,101(e25.17,2x))')  self%omega(ifreq),&
&              ((self%oper(ifreq)%matlu(iatom)%mat(im,im1,isppol,ispinor,ispinor)&
&               ,im=1,ndim),im1=1,ndim)
               call wrtout(unitselffunc_arr(iall),message,'COLL')
...
!&              xtemp,(s_r(im),s_i(im),im=1,ndim)
               read(unitselffunc_arr(iall),*,iostat=ioerr) &
&              xtemp,((s_r(im,im1),s_i(im,im1),im=1,ndim),im1=1,ndim)
               if(ioerr<0) then
...
!                write(std_out,'(a4,2x,31(e15.8,2x))') xtemp,(s_r(im),s_i(im),im=1,ndim)
               ...
               do im=1,ndim
                do im1=1,ndim
                 self%oper(ifreq)%matlu(iatom)%mat(im,im1,isppol,ispinor,ispinor)&
&                 =cmplx(s_r(im,im1),s_i(im,im1),kind=dp)
                enddo
               enddo
             endif
           enddo ! ifreq
...
&            "#dc ",(self%hdc%matlu(iatom)%mat(im,im,isppol,ispinor,ispinor),im=1,ndim)
         ...
         !write(std_out,*) "8"
             read(unitselffunc_arr(iall),*,iostat=ioerr) &
&            chtemp,(s_r(im,1),s_i(im,1),im=1,ndim)
             if(ioerr<0) then
...
&               =cmplx(s_r(im,1),s_i(im,1),kind=dp)
             enddo
           endif
...
!  ===========================
 if(optrw==1) then
!   call xbarrier_mpi(spacecomm)
   ncount=natom*nsppol*nspinor*(self%nw+1)*(maxval(self%oper(1)%matlu(:)%lpawu)*2+1)**2
               ...
               do ifreq=1,self%nw
                 do im=1,ndim
                   do im1=1,ndim
                     ...
                     buffer(icount)=self%oper(ifreq)%matlu(iatom)%mat(im,im1,isppol,ispinor,ispinor)
                   enddo
                 enddo
               enddo
!               Double counting-------
...
!             self ---------------
             do ifreq=1,self%nw
               do im=1,ndim
                 do im1=1,ndim
                   icount=icount+1
                   self%oper(ifreq)%matlu(iatom)%mat(im,im1,isppol,ispinor,ispinor)=buffer(icount)
                 enddo
               enddo
             enddo
!             hdc  ---------------
         ...
         do isppol=1,nsppol
           do ispinor=1,nspinor
             do im=1,ndim
               if(nspinor==1) then
                 self%oper(ifreq)%matlu(iatom)%mat(im,im,isppol,ispinor,ispinor)= &
&                  paw_dmft%fixed_self(im,im,isppol,iatu)
               else
