...
#define ABI_FUNC 'newton'
 ...
 use interfaces_68_dmft, except_this_one => newton
...
!Local variables-------------------------------
 integer iter
 real(dp) Fx,Fxprime,Fxdouble,xold,x_before
 real(dp) :: nb_elec_x
 integer option,optalgo
 logical l_minus,l_plus
 real(dp) :: x_minus,x_plus
...
!    Apply stop criterion on Fx
     if((abs(Fx) .le. f_precision).and.(iter<=max_iter)) then
...
!    ==============================================
     if(x_input<x_minus.or.x_input>x_plus) then
       call compute_nb_elec(cryst_struc,green,mpi_enreg,paw_dmft,pawang,self,&
...
!!
!! SOURCE
subroutine function_and_deriv(cryst_struc,f_precision,green,iter,mpi_enreg,paw_dmft,pawang,self&
...
subroutine compute_nb_elec(cryst_struc,green,mpi_enreg,paw_dmft,pawang,self,&
...
! *********************************************************************
   ...
   call compute_green(cryst_struc,green,mpi_enreg,paw_dmft,pawang,0,self)
   call integrate_green(cryst_struc,green,mpi_enreg,paw_dmft,pawang,prtopt=0,opt_ksloc=-1)
