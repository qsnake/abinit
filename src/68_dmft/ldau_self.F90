...
!!
!! CHILDREN
!!      destroy_paw_ij,init_paw_ij,nullify_paw_ij,pawpupot,wrtout
!!
!! SOURCE
...
!&   1,1,cryst_struc%indsym,cryst_struc%natom,natpawu,&
!&   paw_dmft%nspinor,paw_dmft%nsppol,cryst_struc%nsym,cryst_struc%ntypat,paw_ij,pawang,3,&
!&   pawrhoij_dum,pawtab,cryst_struc%spinat,cryst_struc%symafm,0,10)

 do iatom=1,cryst_struc%natom
   lpawu=pawtab(cryst_struc%typat(iatom))%lpawu
   if(lpawu.ne.-1) then
     ldim=2*lpawu+1
     ABI_ALLOCATE(vpawu,(paw_ij(iatom)%cplex_dij,ldim,ldim))
...
!    ===============================
!    Begin loop over spin/spinors to compute V_{LDA+U}
     do idijeff=1,nsploop
!      ===============================
...
!      ===============================
       call pawpupot(idijeff,paw_ij(iatom),2,pawtab(cryst_struc%typat(iatom)),vpawu,vuks)
!      write(message,'(2a)') ch10," == The vpawu matrix is"
!      call wrtout(std_out,message,'COLL')
!      do im=1,ldim
!      write(message,'(12(1x,9(1x,f10.5)))') (vpawu(1,im,im1),im1=1,ldim)
!      call wrtout(std_out,message,'COLL')
!      write(message,'(12(1x,9(1x,f10.5)))') (vpawu(2,im,im1),im1=1,ldim)
!      call wrtout(std_out,message,'COLL')
!      end do
...
&             cmplx(vpawu(1,im,im1),vpawu(2,im,im1),kind=dp)
           end do
         end do
       end do
