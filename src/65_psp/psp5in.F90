...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (DCA, XG, GMR, FrD, FJ, MT)
...
!!
!! CHILDREN
!!      leave_new,psp1cc,psp5lo,psp5nl,spline,wrtout
!!
!! SOURCE
...
#define ABI_FUNC 'psp5in'
 ...
 use interfaces_65_psp, except_this_one => psp5in
...
!arrays
 ...
 real(dp),intent(out) :: ffspl(mqgrid,2,lnmax),rcpsp(mpssoang),rms(mpssoang)
 real(dp),intent(out) :: vlspl(mqgrid,2),xccc1d(n1xccc,6)
...
!this is not treated correctly by all machines - problems with SGI )
 pspso0=1
 read (tmp_unit,fmt=*,err=10,end=10) r1,al,pspso0
 10 continue
 if(pspso0/=1 .and. pspso0/=2)then
   write(message, '(6a,i4,2a)' ) ch10,&
&   ' psp5in: ERROR -',ch10,&
...
&   '  Action : check your pseudopotential input file.'
 ...
 do ipsang=1,(nso*lmax)+1
   read (tmp_unit,*) ll,e990(ipsang),e999(ipsang),nproj(ipsang),rcpsp(ipsang)
   write(message, '(i5,2f8.3,i5,f12.7,t47,a)' ) &
&   ll,e990(ipsang),e999(ipsang),nproj(ipsang),rcpsp(ipsang),&
&   'l,e99.0,e99.9,nproj,rcpsp'
   call wrtout(ab_out,message,'COLL')
   call wrtout(std_out,  message,'COLL')
   read (tmp_unit,*) rms(ipsang),ekb1(ipsang),ekb2(ipsang),epspsp(ipsang)
...
!  --NON SPIN-ORBIT
   do ipsang=1,lmax+1
     read (tmp_unit,*) ll
     read (tmp_unit,*) (vpspll(ii,ipsang),ii=1,mmax)
!    DEBUG
...
!  --SPIN-ORBIT
   ...
   read (tmp_unit,*) ll
   read (tmp_unit,*) (vpspll(ii,1),ii=1,mmax)
   vpspll_so(:,1)=0.0d0
   do ipsang=2,lmax+1
     read (tmp_unit,*) ll
     read (tmp_unit,*) (vpspll(ii,ipsang),ii=1,mmax)
     read (tmp_unit,*) ll
     read (tmp_unit,*) (vpspll_so(ii,ipsang),ii=1,mmax)
   end do
 end if
...
!    --NON SPIN-ORBIT
     do ipsang=1,lmax+1
       if (nproj(ipsang)/=0) then
         read (tmp_unit,*) ll
         if (ipsang/=ll+1) then
           write(message, '(a,a,a,a,a,a,a,a,a,2i6,a,a)' )ch10,&
&           ' psp5in: ERROR -',ch10,&
...
!        DEBUG
!        write(std_out,*) 'l=',ll,'OK CHECK'
!        ENDDEBUG
         read (tmp_unit,*) wfll(:,ipsang)
!        DEBUG
!        write(std_out,*) 'WF READED',ll,'OK'
!        write(std_out,*) 'WF',ipsang-1,wfll(1,ipsang),&
!        & wfll(2,ipsang),wfll(3,ipsang)
!        ENDDEBUG
       else
         wfll(:,ipsang)=0.0d0
       end if
...
&     qgrid,rad,vloc,vpspll_so,wfll_so)
     ...
     do ipsang=2,lmax+1
       if((ekb_sr(ipsang)*ekb_so(ipsang))<0.0) then
         write(std_out,*) 'BIG PROBLEM WITH THE SPIN ORBIT IN PSP5NL'
         stop
       end if
     end do
...
&           '  Action : check your pseudopotential input file.'
         ...
         read (tmp_unit,*) wfll(:,ipsang)
!        DEBUG
!        write(std_out,*) 'WF second',ipsang-1,wfll(1,ipsang),&
!        & wfll(2,ipsang),wfll(3,ipsang)
!        ENDDEBUG
       else
         wfll(:,ipsang)=0.0d0
