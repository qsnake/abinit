...
!!
!! PARENTS
!!      psp5in,psp6in,psp9in
!!
!! CHILDREN
!!      ctrap,leave_new,spline,wrtout
!!
!! SOURCE
...
!scalars
 ...
 integer :: iq,ir,lp1
 real(dp) :: arg,bes0,bes1,bes2,bes3,bessel,dvwf,qr,result,yp1,ypn,ztor1
...
!  Check that lmax is within allowed range
   if (lmax<0.or.lmax>3) then
     write(message, '(a,a,a,a,i12,a,a,a,a,a,a,a)' ) ch10,&
&     ' psp5nl: ERROR -',ch10,&
...
!      0 to r1 integral
       dvwf=(vpspll(1,lp1)-vloc(1))*wfll(1,lp1)*renorm(lp1)
       ztor1=(bes0(arg*rad(1))*rad(1)*dvwf)*rad(1)/3.0d0
!      integrand
       do ir=1,mmax
         dvwf=(vpspll(ir,lp1)-vloc(ir))*wfll(ir,lp1)*renorm(lp1)
         work1(ir)=rad(ir)*(rad(ir)*bes0(arg*rad(ir))*dvwf)
       end do
       call ctrap(mmax,work1,al,result)
...
!      where x=2 Pi qgrid(mqgrid) r
       ...
       if(qr<1.d-3)then
         ...
         bessel=bes0(qr)-2.d0*bes1(qr)/qr
       end if
...
!      where x=2 Pi qgrid(mqgrid) r
       ...
       if(qr<1.d-3)then
         ...
         bessel=bes1(qr)-3.0d0*bes2(qr)/qr
       end if
...
!      where x=2 Pi qgrid(mqgrid) r
       ...
       if(qr<1.d-3)then
         ...
         bessel=bes2(qr)-4.0d0*bes3(qr)/qr
       end if
