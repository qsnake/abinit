...
!!
!! COPYRIGHT
!!  Copyright (C) 2010-2012 ABINIT group (AHR)
...
!!
!! CHILDREN
!!      atmdata,leave_new,mkrdim,wrtout,xredxcart
!!
!! SOURCE
...
#define ABI_FUNC 'randomcellpos'
 ...
 use interfaces_42_geometry, except_this_one => randomcellpos
...
!Local variables-------------------------------
 ...
 real(dp) ::  rij(3), rijd(3), radiuscovi, amu, radiuscovj, dist, rati, ratj, angdeg(3)
...
!ENDDEBUG
 ...
 if(random_atpos==2 .and. npsp/=ntypat)then
   write(message, '(4a,i5,2a,i5,a,i5,4a)' ) ch10,&
&   ' randomcellpos : ERROR -',ch10,&
   ...
   select case (random_atpos)
     ...
     case (2)
       ...
       do
         ...
         call atmdata(amu,radiuscovi,symbol,znucl(typat(iatom)))
         do ii=1,iatom-1
...
!          convert reduced coordinates to cartesian coordinates
           call xredxcart(1,1,rprimd,rijd,rij)
           dist=dot_product(rijd,rijd)
           call atmdata(amu,radiuscovj,symbol,znucl(typat(ii)))
...
!          apply periodic boundary conditions
           ...
           rij = rij - ANINT ( rij )
           rij = rij + 0.5
           call xredxcart(natom,1,rprimd,rijd,rij)
           dist=dot_product(rijd,rijd)
...
!DEBUG
!write (std_out,*) ' randomcellpos : exit'
!stop
!ENDDEBUG
