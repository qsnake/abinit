...
!      Replace the second vector by the sum vector if the latter is smaller
 ...
 if (iiter >= maxiter) then
   write(message, '(a,i6,2a)') &
&   'the loop has failed to find a set of minimal vectors in ', &
&   maxiter, ' iterations. Exiting.', ch10
   MSG_BUG(message)
 end if
...
!  to replace each of the three vectors by minus the summ of the three vectors
 ...
 if (iiter >= maxiter) then
   write(message, '(a,i6,2a)') &
&   'the second loop has failed to find a set of minimal vectors in ', &
&   maxiter, ' iterations. Exiting.', ch10
   MSG_BUG(message)
 end if
