...
#define ABI_FUNC 'ioniondist'
 ...
 use interfaces_42_geometry, except_this_one => ioniondist
...
! *************************************************************************
 ...
 if (option==3.and.(.not.present(varlist))) then
   write(message,*)'ioniondist error: option=3 but no variable list provided for symmetrization'
...
!END DEBUG
 ...
 if (prtvoll>=3) then
   write(message,*)'ioniondist: ionic distances:'
   call  wrtout(std_out,message,'COLL')
 ...
 if (prtvoll==2) then
   write(message,*)'ioniondist: symmetrized matrix:'
   call  wrtout(std_out,message,'COLL')
