...
#define ABI_FUNC 'prt_cif'
 ...
 use interfaces_42_geometry, except_this_one => prt_cif
...
!arrays
 ...
 character(len=fnlen), intent(in) :: ciffname
...
!Local variables -------------------------------
!scalars
 integer :: unitcif, iostat
 integer :: iatom, isym
 integer :: sporder
 integer :: itypat, nat_this_type

 real(dp) :: amu, rcov, ucvol
...
!get unit for opening file
 unitcif = get_unit()

!open file in append mode
!xlf and other compilers refuse append mode
 open (unitcif, file=trim(ciffname), IOSTAT=iostat)
 if (iostat /= 0) then
   write(std_out,*) 'error opening file ', ciffname
   stop
 end if
...
!print reduced positions
 ...
 do iatom = 1, natom
   ...
   write (unitcif,'(2a,3E20.10)') '  ', symbol, xred(:,iatom)
 end do
...
!print chemical composition in simplest form
 ...
 do itypat = 1, ntypat
   ...
   call atmdata(amu,rcov,symbol,znucl(itypat))
   call int2char(nat_this_type, str_nat_type)
   chemformula = trim(chemformula) // symbol // trim(str_nat_type) // "  " 
 end do
