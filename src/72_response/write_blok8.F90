...
!!
!! PARENTS
!!      d3output,gstate,mblktyp1,mblktyp5
!!
!! CHILDREN
!!
!! SOURCE
...
!Count the number of elements
 nelmts=0
 do ii=1,msize
   if(ddb_blk%flg(ii,iblok)==1)nelmts=nelmts+1
 end do

!Write the block type and number of elements
 write(nunit,*)' '
 if (ddb_blk%typ(iblok) == 0) then
   write(nunit, '(a,i8)' )&
&   ' Total energy                 - # elements :',nelmts
 else if (ddb_blk%typ(iblok)==1) then
   write(nunit, '(a,i8)' )&
&   ' 2nd derivatives (non-stat.)  - # elements :',nelmts
 else if(ddb_blk%typ(iblok)==2) then
   write(nunit, '(a,i8)' )&
...
!  Write the phonon wavevector
   write(nunit, '(a,3es16.8,f6.1)' )' qpt',(ddb_blk%qpt(ii,iblok),ii=1,3),ddb_blk%nrm(1,iblok)
     ...
     do ipert2=1,mpert
       do idir2=1,3
         do ipert1=1,mpert
           do idir1=1,3
             ii=ii+1
             if(ddb_blk%flg(ii,iblok)==1)then
               write(nunit,'(4i4,2d22.14)')idir1,ipert1,idir2,ipert2,&
&               ddb_blk%val(1,ii,iblok),ddb_blk%val(2,ii,iblok)
             end if
           end do
         end do
       end do
...
&   ' qpt',(ddb_blk%qpt(ii,iblok),ii=1,3),ddb_blk%nrm(1,iblok)
   write(nunit, '(a,3es16.8,f6.1)' )&
&   '    ',(ddb_blk%qpt(ii,iblok),ii=4,6),ddb_blk%nrm(2,iblok)
   write(nunit, '(a,3es16.8,f6.1)' )&
&   '    ',(ddb_blk%qpt(ii,iblok),ii=7,9),ddb_blk%nrm(3,iblok)
...
!  Write total energy
   ...
   if (choice == 2) then
     write(nunit,'(2d22.14)')ddb_blk%val(1,1,iblok),ddb_blk%val(2,1,iblok)
   end if
