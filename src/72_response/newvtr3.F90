...
!!      ab6_mixing_eval_deallocate,flush_unit,fourdp,leave_new,metric,moddiel
!!      timab,wrtout,xcomm_init
!!
!! SOURCE
...
!Arguments-------------------------------
!scalars
 integer,intent(in) :: cplex,initialized,iscf,ispmix,istep,nfft
...
!scalars
 ...
 integer :: mpi_comm,old_paral_level,errid
...
&     '  pawrhoij()%cplex must be >=cplex !'
 ...
 if (usepaw==1) cplex_rhoij=pawrhoij(1)%cplex
...
!Copy current step arrays.
 call ab6_mixing_copy_current_step(mix, vresid0, errid, message, arr_respc = vrespc)
...
&             rhoijtmp(cplex_rhoij*klmn-dplex:cplex_rhoij*klmn,ispden)
   ...
   if(ispmix/=1) then
     write(message, '(a,a,a,a)' )ch10,&
&     ' newvtr3 : ERROR -',ch10,&
&     '  Mixing on reciprocal space not allowed with iscf=5 or 6.'
     call wrtout(std_out,message,'COLL')
     call leave_new('PERS')
   end if
