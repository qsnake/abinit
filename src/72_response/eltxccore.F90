...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: is2_in,n1,n1xccc,n2,n3,natom,nfft,ntypat
...
!scalars
 ...
 integer :: i1,i2,i3,iatom,ifft,is1,is2,ishift,ishift1,ishift2
 integer :: ishift3,ixp,jj,js,ka,kb,kd,kg,mu,nu
 real(dp) :: aa,bb,cc,cross,d2rss,dd,delta,delta2div6,deltam1,diff
 ...
 real(dp) :: term1,term2,xx,yy,zz
 character(len=500) :: message
!arrays
 integer,save :: idx(12)=(/1,1,2,2,3,3,3,2,3,1,2,1/)
 integer :: igrid(3),ii(mshift,3),irange(3),ngfft(3)
 real(dp) :: drm(3,3,6),lencp(3),rmet(3,3),rrdiff(mshift,3)
 real(dp) :: scale(3),tau(3),ts2(3),tt(3)
...
!              Vall(0) X Rhocore(2) term
               eltfrxc(is1,is2_in)=eltfrxc(is1,is2_in)+0.25_dp*&
...
&               vxc10_core(ifft)*drss1*term1/difmag
               eltfrxc(is2_in,is1)=eltfrxc(is2_in,is1)+0.25_dp*&
&               vxc10_core(ifft)*drss1*term1/difmag
...
&             +drm(:,3,is2_in)*rdiff3
             ...
             eltfrxc(js:js+2,is2_in)=eltfrxc(js:js+2,is2_in)&
