...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (XG, DCA, GMR, MM, AR, MV, MB, MT)
...
!!
!! CHILDREN
!!      appdig,clsopn,destroy_hamiltonian,dotprod_vn,hdr_skip,init_hamiltonian
...
!Arguments -------------------------------
!scalars
 integer,intent(in) :: cplex,idir,ipert,mpert,mpw,mpw1,nfft,nkpt,nkpt_rbz,nkxc,nspden,nsppol,nsym1
...
!Local variables-------------------------------
!scalars
 integer :: ban2tot,bantot,bdtot_index,ddkcase,formeig,ia,iatom,iband,icg,icg1,idir1
 integer :: ierr,ifft,ii,ikg,ikg1,ikpt,ikpt_dum,ilm,ipert1,ispden,isppol
 integer :: istwf_k,isym,jj,master,me,muig,n1,n2,n3,n3xccc,n4,n5,n6,nband_dum
 integer :: nband_k,nfftot,npw1_k,npw_k,nskip,nspinor_,option,spaceworld,t_iostat
 real(dp) :: arg,doti,dotr,wtk_k
 logical :: t_exist
...
!arrays
 ...
 real(dp) :: d2nl_elfd(2,3),d2nl_mgfd(2,3),kpoint(3),sumelfd(2),summgfd(2),tsec(2)
 real(dp),allocatable :: buffer1(:),buffer2(:),cg_dum(:,:),d2bbb_k(:,:,:,:),d2nl_k(:,:,:)
 real(dp),allocatable :: eig1_k(:),eig_k(:),eigen_dum(:),occ_dum(:),occ_k(:),rhodummy(:,:)
...
!  Check that ddk file exists
   ...
   if (t_iostat.ne.0) then
     write(unit=msg,fmt='(5a,i8)' ) '  Check for existence of file ',trim(fiwfddk),',',ch10,&
...
!    Note the use of unit numbers 21, 22 and 23
     ...
     write(msg, '(a,a)') '-open ddk wf file :',fiwfddk
...
&       ((ipert1==dtset%natom+2.or.ipert1==dtset%natom+5) &
...
!The following line prevents inclusion of the magnetic field perturbation
!in the second derivative output. Once the magnetic field is fully implemented,
!the following line will be removed and also all the tests updated.
 blkflg(:,dtset%natom+5,idir,ipert)=0
...
!3) Constant kleimann-Bylander energies are copied from psps to gs_hamk.
 call init_hamiltonian(gs_hamkq,psps,paw_ij,pawtab,dtset%nspinor,nspden,dtset%natom,dtset%ntypat,&
& dtset%typat,xred,nfft,dtset%mgfft,ngfft,rprimd,dtset%nloalg,ph1d=ph1d,use_gpu_cuda=dtset%use_gpu_cuda)
...
!LOOP OVER SPINS
 ...
 do isppol=1,nsppol
   ...
   if (nsppol/=1) then
     write(msg,*)' ****  In nstdy3 for isppol=',isppol
     call wrtout(std_out,msg,'COLL')
   end if
...
!        used in the ddk file : compute the number of k points to skip
         ...
         if(nskip/=0)then
           ABI_ALLOCATE(cg_dum,(2,1))
           ABI_ALLOCATE(eigen_dum,(0))
           ABI_ALLOCATE(kg_dum,(3,0))
           ABI_ALLOCATE(occ_dum,(0))
...
!            Skip the records whose information is not needed (in case of parallelism)
           ...
           ABI_DEALLOCATE(cg_dum)
           ABI_DEALLOCATE(eigen_dum)
...
!    Continue to initialize the Hamiltonian at k+q
     ...
     if (ipert/=dtset%natom+3.and.ipert/=dtset%natom+4) gs_hamkq%kpoint(:)=gs_hamkq%kpoint(:)+dtset%qptn(:)
     do ia=1,dtset%natom
       iatom=atindx(ia)
...
!    Note that nstwf3 is called with kpoint, while kpt is used inside vtowfk3
     call nstwf3(cg,cg1,ddkfil,dtfil,dtset,d2bbb_k,d2nl_k,eig_k,eig1_k,gs_hamkq,&
&     icg,icg1,idir,ikpt,ipert,isppol,istwf_k,kg_k,kg1_k,kpoint,mpert,&
&     mpi_enreg,psps%mpsang,mpw,mpw1,nband_k,npw_k,npw1_k,nsppol,&
&     occ_k,psps,rmet,wffddk,wffnow,wfftgs,wtk_k,ylm_k,ylm1_k)
...
!  End loop over spins
 ...
 if(mpi_enreg%paral_compil_kpt==1)then
   ...
   call leave_test()
   write(msg,*) ' nstdy3: loop on k-points and spins done in parallel'
   call wrtout(std_out,msg,'COLL')
...
!Do the transform
 ...
 call sygra3(dtset%natom,d2nl(:,:,:,idir,ipert),work1,indsy1,ipert,nsym1,dtset%qptn,symrc1)
...
!        (core-correction contribution only !)
         if(psps%n1xccc/=0)then
           option=0
           call mkvxc3(cplex,kxc,mpi_enreg,nfft,ngfft,nkxc,nspden,n3xccc,&
