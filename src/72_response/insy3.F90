...
!! syuse= flag to use the symmetries or not. If 0 do not use, if 1, use.
...
!!
!! PARENTS
!!      irred_perts,loper3,m_io_gkk,read_gkk
!!
!! CHILDREN
!!      stresssym,wrtout
!!
...
!Arguments -------------------------------
!scalars
 integer,intent(in) :: idir,ipert,natom,nsym,rfmeth,syuse
...
!          All the tests have been successful !
 ...
 if (nsym1<1) then
   write(msg,'(a,i0,a)')&
&   ' The number of selected symmetries should be > 0, while it is nsym=',nsym,'.'
   MSG_BUG(msg)
 end if
 ...
 if (nsym1/=1) then
   ...
   write(msg,'(a,i5,a)')' insy3 : found ',nsym1,' symmetries that leave the perturbation invariant :'
   call wrtout(std_out,msg,'COLL')
 else
   write(msg,'(a,a)')' The set of symmetries contains',' only one element for this perturbation.'
   call wrtout(ab_out,msg,'COLL')
   write(msg,'(a)')' insy3 : only one element in the set of symmetries for this perturbation :'
   call wrtout(std_out,msg,'COLL')
 end if
