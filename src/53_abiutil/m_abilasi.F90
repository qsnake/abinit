...
!!  thus reducing the number of input arguments that has to be passed.
...
!!  3) On certain networks, xsum_mpi might crash due to the size of the MPI packet.
...
! Procedures for complex non-symmetric matrices
 ...
 integer,private,parameter :: SLK_BLOCK_SIZE=40  
...
!!
!! CHILDREN
!!      destruction_matrix_scalapack,end_scalapack,init_matrix_scalapack
!!      init_scalapack,slk_matrix_from_global_dpc_2d
!!      slk_matrix_to_global_dpc_2d,slk_zdhp_invert,wrtout,xsum_mpi,zpotrf
!!      zpotri
!!
!! SOURCE
...
!************************************************************************
 ...
 if (istat/=0) stop 'wrap_CHEEV 2: out of memory!'
      
 ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
 istat = ABI_ALLOC_STAT
 if (istat/=0) stop 'wrap_CHEEV 1: out of memory!'
...
!Local variables ------------------------------
!scalars
 integer :: lwork,info,istat,nprocs
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  ABI_ALLOCATE(work,(lwork))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEEV 2: out of memory!'

  ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEEV 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  if (cplex==1) then  ! Real symmetric case.
   ...
   ABI_ALLOCATE(work_real,(lwork))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYEV_ZHEEV 2: out of memory!'
...
&    info," off-diagonal elements of an intermediate tridiagonal form did not converge to zero. "
   ...
   ABI_ALLOCATE(work_cplx,(lwork))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYEV_ZHEEV 2: out of memory!'

   ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYEV_ZHEEV 1: out of memory!'
...
!************************************************************************
 ...
 ABI_ALLOCATE(work,(MAX(1,2*n-1)))
 istat = ABI_ALLOC_STAT
 if (istat/=0) stop 'wrap_CHPEV 2: out of memory!'

 ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
 istat = ABI_ALLOC_STAT
 if (istat/=0) stop 'wrap_CHPEV 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  ABI_ALLOCATE(work,(MAX(1,2*n-1)))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHPEV 2: out of memory!'

  ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHPEV 1: out of memory!'
...
!Local variables ------------------------------
!scalars
 integer :: lwork,info,istat,nprocs,ii
 logical :: use_scalapack
 character(len=500) :: msg
!arrays
 real(dp),allocatable :: rwork(:)
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  ABI_ALLOCATE(work,(lwork))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEGV 2: out of memory!'

  ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEGV 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  if (cplex==1) then ! Real symmetric case.
   ...
   ABI_ALLOCATE(work_real,(lwork))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYGV_ZHEGV 2: out of memory!'
...
&    " The factorization of B could not be completed and no eigenvalues or eigenvectors were computed."
   ...
   ABI_ALLOCATE(rwork,(MAX(1,3*n-2)))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYGV_ZHEGV 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.) ! Standard LAPACK call.
  ...
  ABI_ALLOCATE(work,(lwork))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEEVX 2: out of memory!'

  ABI_ALLOCATE(rwork,(7*n))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEEVX 1: out of memory!'

  ABI_ALLOCATE(iwork,(5*n))
  ABI_ALLOCATE(ifail,(n))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEEVX 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.) ! Standard LAPACK call.
  ...
  if (cplex==1) then      ! Real symmetric case
   ...
   ABI_ALLOCATE(work_real,(lwork))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYEVX_ZHEEVX 2: out of memory!'

   ABI_ALLOCATE(iwork,(5*n))
   ABI_ALLOCATE(ifail,(n))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYEVX_ZHEEVX 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.) ! Standard LAPACK call.
  ...
  ABI_ALLOCATE(work,(lwork))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEGVX 2: out of memory!'

  ABI_ALLOCATE(rwork,(7*n))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZHEGVX 1: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.) ! Standard LAPACK call.
  ...
  if (cplex==1) then  ! Real symmetric case 
   ...
   ABI_ALLOCATE(work_real,(lwork))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYGVX_ZHEGVX 2: out of memory!'

   ABI_ALLOCATE(iwork,(5*n))
   ABI_ALLOCATE(ifail,(n))
   istat = ABI_ALLOC_STAT
   if (istat/=0) stop 'wrap_DSYGVX_ZHEGVX 1: out of memory!'
...
!************************************************************************
 ...
 ABI_ALLOCATE(work,(lwork))
 istat = ABI_ALLOC_STAT
 if (istat/=0) stop 'wrap_CGEEV 1: out of memory!'

 ABI_ALLOCATE(rwork,(2*n))
 istat = ABI_ALLOC_STAT
 if (istat/=0) stop 'wrap_CGEEV 2: out of memory!'
...
!************************************************************************
 ...
 CASE (.FALSE.)
  ...
  ABI_ALLOCATE(work,(lwork))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZGEEV 1: out of memory!'

  ABI_ALLOCATE(rwork,(2*n))
  istat = ABI_ALLOC_STAT
  if (istat/=0) stop 'wrap_ZGEEV 2: out of memory!'
