...
MODULE m_atom
 ...
 use m_radmesh,       only : nullify_radmesh, init_radmesh, print_radmesh, destroy_radmesh, &
&                            isame_radmesh, simp_gen, ifromr ,copymesh, compmesh
...
!arrays
  ...
  integer, pointer :: klm_diag(:)  SET2NULL
   ! klm_diag(lmn2_size)
   ! 1 il==jl and im==jm, 0 otherwise.

  integer, pointer :: klmntomn(:,:)  SET2NULL
  ...
  integer, pointer :: kln2ln(:,:)  SET2NULL
   ! kln2ln(6,ln2_size)
...
!!****f* m_atom/my_nullify_atom
!! NAME
!!  my_nullify_atom
!!
!! FUNCTION
!!  Nullify all pointers in a structure of type atom_type.
!!
!! SIDE EFFECTS
...
subroutine my_nullify_atom(Atm)
...
#undef ABI_FUNC
#define ABI_FUNC 'my_nullify_atom'
...
!************************************************************************
 ...
 nullify(Atm%indln   )
 nullify(Atm%indklmn )
 nullify(Atm%klm_diag)
 nullify(Atm%klmntomn)
 nullify(Atm%kln2ln  )
 nullify(Atm%mode    )
...
end subroutine my_nullify_atom
!!***
...
!************************************************************************
 ...
 ! integers
 if (associated(Atm%indlmn  ))   then
   ABI_DEALLOCATE(Atm%indlmn)
 end if
 if (associated(Atm%indln   ))   then
   ABI_DEALLOCATE(Atm%indln)
 end if
 if (associated(Atm%indklmn ))   then
   ABI_DEALLOCATE(Atm%indklmn)
 end if
 if (associated(Atm%klm_diag))   then
   ABI_DEALLOCATE(Atm%klm_diag)
 end if
 if (associated(Atm%klmntomn))   then
   ABI_DEALLOCATE(Atm%klmntomn)
 end if
 if (associated(Atm%kln2ln  ))   then
   ABI_DEALLOCATE(Atm%kln2ln)
 end if
 if (associated(Atm%mode    ))   then
   ABI_DEALLOCATE(Atm%mode)
 end if

 !real
 if (associated(Atm%eig))   then
   ABI_DEALLOCATE(Atm%eig)
 end if
 if (associated(Atm%occ))   then
   ABI_DEALLOCATE(Atm%occ)
 end if
 if (associated(Atm%phi))   then
   ABI_DEALLOCATE(Atm%phi)
 end if
...
! ************************************************************************
 ...
 !@atom_type
 call my_nullify_atom(Atm)

 unt = get_unit(); ierr=0
 open(unt,file=filename,form='formatted',status='old',iostat=ierr)
 if (ierr/=0) then
  msg = " Error opening file"//TRIM(filename)//" as formatted-old."
  MSG_WARNING(msg)
  ierr=1; RETURN
 end if
...
&  ' the current version of Abinit.'
 ...
 ! 6)
 ABI_ALLOCATE(orbitals,(Atm%ln_size))
...
&  " Truncating radial mesh for core orbitals using new rcore = ",Atm%rcore," (",my_rcore,")"
 ...
 do imsh=1,nmesh
  call print_radmesh(Radmesh(imsh),prtvol=prtvol)
 end do
 ...
 ABI_ALLOCATE(Atm%indln,(2,Atm%ln_size))
 ABI_ALLOCATE(Atm%eig,(Atm%ln_size,Atm%nsppol))
 ABI_ALLOCATE(Atm%occ,(Atm%ln_size,Atm%nsppol))
 Atm%occ = zero
 ...
 do isppol=1,Atm%nsppol
  do iln=1,Atm%ln_size
   ...
   if (iln==1.and.isppol==1) then
    ...
    ABI_ALLOCATE(Atm%phi,(Atm%mesh_size,Atm%ln_size,Atm%nsppol))
...
! -------------------- END OF READING -------------------------------
 ...
 if ( rcut < tol16) then
  ...
  call copymesh(Radmesh(imainmesh),Atmrad) !this is needed, check psp7in since there's a problem with pawrad
  !call compmesh(Atmrad,Atm%rcore) ! check this
  call compmesh(Atmrad,-one) ! check this
 else
  ! * Truncate orbitals and radial mesh within rcut.
  msz = MIN(ifromr(Radmesh(imainmesh),rcut)+6, Radmesh(imainmesh)%mesh_size) ! add six more points
...
&  " Truncating radial mesh for core orbitals ",Radmesh(imainmesh)%rad(msz),"(",my_rcore,")"
  ...
  ABI_ALLOCATE(phitmp,(msz,Atm%ln_size,Atm%nsppol))
  phitmp = Atm%phi(1:msz,:,:)

  ABI_DEALLOCATE(Atm%phi)
  ABI_ALLOCATE(Atm%phi,(msz,Atm%ln_size,Atm%nsppol))
  Atm%phi = phitmp
  ...
  call init_radmesh(Atmrad,mesh_size,mesh_type,rstep,lstep,-one)
  !call init_radmesh(Atmrad,mesh_size,mesh_type,rstep,lstep,Atm%rcore)
 ...
 call print_radmesh(Atmrad,header="Final mesh",prtvol=prtvol)

 ABI_ALLOCATE(radens,(Atm%mesh_size,Atm%nspden))
 ...
 ! * Setup of indlmn
 ABI_ALLOCATE(Atm%indlmn,(6,Atm%lmn_size))
 ...
 lcutdens=HUGE(1)
 ABI_ALLOCATE(Atm%indklmn,(6,Atm%lmn2_size))
 ABI_ALLOCATE(Atm%klm_diag,(Atm%lmn2_size))

 call make_indklmn(lcutdens, Atm%lmn_size, Atm%lmn2_size, Atm%indlmn, Atm%indklmn, Atm%klm_diag)

 ! * Setup of klmntomn.
 ABI_ALLOCATE(Atm%klmntomn,(4,Atm%lmn2_size))
...
!************************************************************************
 ...
 ABI_ALLOCATE(phi2nl,(Atm%mesh_size))
...
!scalars
 ...
 type(atom_type),intent(in) :: Atm
 type(pawrad_type),intent(in) :: Atmesh,Radmesh2
!arrays
...
!************************************************************************
 ...
 call isame_radmesh(Atmesh,Radmesh2,hasameq,whichdenser)
 ...
 if (do_spline==1) then
  ...
  ABI_ALLOCATE(ff_spl,(my_mesh_size,nphi))

  size4spl =  Radmesh2%mesh_size
  rad4spl  => Radmesh2%rad
  ABI_ALLOCATE(der,(size4spl))
  ABI_ALLOCATE(ypp,(size4spl))
 ...
 ABI_ALLOCATE(func,(my_mesh_size))
 overlap = zero
