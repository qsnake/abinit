...
!!
!! COPYRIGHT
!! Copyright (C) 2000-2012 ABINIT group (MT, FDahm)
...
!scalars
#if defined HAVE_GPU_CUDA
 integer :: dimekb1_max,dimekb2_max,dimffnl_max,ikpt,npw_max_loc,npw_max_nonloc
#endif
...
!  Compute max of total planes waves
   ...
   if(dtset%paral_kgb==1) then
     do ikpt=1,dtset%nkpt
       npw_max_loc = max(npw_max_loc,mpi_enreg%bandfft_kpt(ikpt)%npw_tot)
     end do
   else
...
!  Compute max of total planes waves
   ...
   if(dtset%paral_kgb==1) then
     do ikpt=1,dtset%nkpt
       npw_max_nonloc = max(npw_max_nonloc,mpi_enreg%bandfft_kpt(ikpt)%ndatarecv)
     end do
   else
...
&   dimffnl_max,dimekb1_max,dimekb2_max,dimffnl_max)
 end if
 call leave_test()
#else
 if (.false.) then
