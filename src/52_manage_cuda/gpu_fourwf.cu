												...
												int *use_ndo,double *fofginb)*/
{
  ...
  if((n1!=(*n4)) || (n2!=(*n5)) || (n3!=(*n6))){
    printf("FFT SIZE ERROR: \n when gpu mode is on the fft grid must'nt be augmented\n");
  ...
  if(*option!=3){
    ...
    cufft_state = FFTEXECC2C(plan_fft,(cufftDoubleComplex *)work_gpu,(cufftDoubleComplex *)fofr_gpu,CUFFT_INVERSE);
  ...
  if(*option==2 || *option==3){
    ...
    cufft_state = FFTEXECC2C(plan_fft,(cufftDoubleComplex *)fofr_gpu,(cufftDoubleComplex *)work_gpu,CUFFT_FORWARD);
    if(cufft_state!=CUFFT_SUCCESS){
