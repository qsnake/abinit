...
//
// COPYRIGHT
// Copyright (C) 1998-2012 ABINIT group (MT,FDahm)
...
//
// PARENTS
//      bloch_interp,cgwf,dyfnl3,eig1fixed,eltfrnl3,energy,forstrnps,getgh1c
			    ...
			    double *pi,double *ucvol,double *vectin,double *vectout)
{
  ...
  if(((*choice)!=0)&&((*choice)!=1)&&((*choice)!=2)&&((*choice)!=3)&&((*choice)!=23)){
    ...
    printf(" Try to change your input files with with correct optforces & optstress ... \n");
    fflush(stdout);
    leave_new_("COLL");
  }
...
extern "C" void gpu_update_ham_data_(double *enl,double *sij,double *gprimd,int *dimenl1,int *dimenl2,int *nspinor,int *ntypat,int *use_paw){
...
extern "C" void gpu_update_ffnl_ph3d_(double *ph3din,double *ffnlin,
				      int *npwin,int *dimffnlin,int *lmnmax,int *ntypat,int *natom){
