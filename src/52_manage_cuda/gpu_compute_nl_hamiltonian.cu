...
//
// COPYRIGHT
// Copyright (C) 1998-2012 ABINIT group (FDahm)
...
__device__ static inline void reduce_complex_64(double *sh_vect_x,double *sh_vect_y){
...
__device__ static inline  void reduce_double_32(double *sh_vect){
...
__global__ void kernel_compute_forces(double *rdlmn,double *enlout,
  ...
  //Write results
  if(threadIdx.x==0)
    enlout[decalage_enlout + ialpha + 3*iatom] = 2*ucvol*enl_sh_sum[0];
...
__global__ void kernel_stress_convert(double *gprimd,double *d_enlk,double *enlout,double ucvol1){
  ...
  if(threadIdx.x==0){
    if(blockIdx.x < 3 )
      enlout[blockIdx.x] = ucvol1*(2*sh_enl[0] - d_enlk[0]);
    else
      enlout[blockIdx.x] = ucvol1*2*sh_enl[0];
  }
}
...
extern "C" void gpu_compute_nl_hamiltonian_(double2 *proj_gpu,double2 *dproj_gpu,
					    ...
					    const double *four_pi_by_ucvol,const double *lambda,const double*ucvol
					    ){
  ...
  if( ((*choice)==1) && (*signs==2)) {
	...
	block.x /= 2;
      ...
      double ucvol1 = 1./(*ucvol);
      kernel_stress_convert<<<6,9>>>(gprimd_gpu,d_enlk_gpu,enlout_gpu,ucvol1);
