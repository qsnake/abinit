...
!!
!! CHILDREN
!!      dsyev,dsysv,fcart2fred,make_bonds_new
!!
!! SOURCE
...
#define ABI_FUNC 'prec_simple'
 use interfaces_45_geomoptim, except_this_one => prec_simple
...
& 0.9491, 1.7190, 2.3185, 2.5206, 2.5110, 0.0000 /), (/ 6, 6/) )
 ...
 write(fmt,'(a1,i2,a5)') '(',3*ab_mover%natom,'f8.3)'
...
!  For all bonds compute the 3x3 matrix 
!  and fill also the big matrix
...
!  Visualize the matrix
   ...
   ABI_ALLOCATE(work,(1))
   lwork=-1
   call DSYEV('V', 'U', 3*ab_mover%natom, matrix_tmp, 3*ab_mover%natom, w , work,&
&   lwork, info )
   lwork=work(1)
...
& 3*ab_mover%natom, ipiv, B, 3*ab_mover%natom, work, lwork, info )
 ...
 forstr_precon%fcart=reshape(B,(/ 3, ab_mover%natom /) )
 call fcart2fred(forstr_precon%fcart,forstr_precon%fred,rprimd,ab_mover%natom)
