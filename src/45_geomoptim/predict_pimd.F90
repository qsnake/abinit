...
!!
!! COPYRIGHT
!! Copyright (C) 2010-2012 ABINIT group (GG)
...
!!
!! CHILDREN
!!      gather_array_img,pimd_langevin_npt,pimd_langevin_nvt
!!      pimd_nosehoover_npt,pimd_nosehoover_nvt,scatter_array_img,xcast_mpi
!!      xgather_mpi
!!
!! SOURCE
...
!scalars
 ...
 real(dp) :: volume
!arrays
 real(dp) :: rprimd(3,3),rprimd_next(3,3),rprimd_prev(3,3)
 real(dp),allocatable :: mpibuf(:),mpibuffer(:,:,:),mpibuffer_all(:,:,:)
 real(dp),allocatable :: etotal(:),forces(:,:,:),vel(:,:,:)
...
!  Parallelism: Gather positions/forces/velocities/energie from all images
   if (mpi_enreg%paral_img==1) then
     ABI_ALLOCATE(mpibuffer,(12,natom,nimage))
     do ii=1,nimage
     ...
     if (mpi_enreg%me_img==0)  then
       ABI_ALLOCATE(mpibuffer_all,(12,natom,nimage_tot))
     end if
     ...
     if (mpi_enreg%me_img/=0) ABI_ALLOCATE(etotal,(0))
     do ii=1,nimage
       mpibuf(ii)=results_img(ii,1)%results_gs%etotal
     end do
     call xgather_mpi(mpibuf,nimage,etotal,nimage,0,mpi_enreg%comm_img,ierr)
     ABI_DEALLOCATE(mpibuf)
     if (mpi_enreg%me_img/=0) ABI_DEALLOCATE(etotal)

!    No parallelism: simply copy positions/forces/velocities
   else
     do ii=1,nimage
...
!    Take acell and rprim from 1st image
     do ii=1,3
       rprimd     (1:3,ii)=results_img(1,1)%acell(ii)*results_img(1,1)%rprim(1:3,ii)
       rprimd_prev(1:3,ii)=results_img(1,2)%acell(ii)*results_img(1,2)%rprim(1:3,ii)
       rprimd_next(1:3,ii)=rprimd_prev(1:3,ii)
     end do
...
&             rprimd,nimage_tot,vel,volume,xred,xred_next,xred_prev)
           case(2)  !NPT
             call pimd_langevin_npt(etotal,forces,itimimage,natom,pimd_param,prtvolimg,&
&             rprimd,rprimd_next,rprimd_prev,nimage_tot,vel,volume,xred,xred_next,xred_prev)
         end select
...
!  use (9      ) to store rprimd_next
   ABI_ALLOCATE(mpibuffer,(9,natom+1,nimage))
   if (mpi_enreg%paral_img==1) then
     if (mpi_enreg%me_img==0) then
       ABI_ALLOCATE(mpibuffer_all,(9,natom+1,nimage_tot))
       do ii=1,nimage_tot
...
!Send results to all procs treating the same image
 call xcast_mpi(mpibuffer,0,mpi_enreg%comm_one_img,ierr)
...
!Store results in final place
 ...
 if (pimd_param%optcell/=0) then
   do ii=1,nimage
     ...
     results_img(ii,1)%rprim(1:3,1)=mpibuffer(1:3,natom+1,ii)
     results_img(ii,1)%rprim(1:3,2)=mpibuffer(4:6,natom+1,ii)
     results_img(ii,1)%rprim(1:3,3)=mpibuffer(7:9,natom+1,ii)
     do jj=1,3
       results_img(ii,1)%acell(jj)=sqrt(results_img(ii,1)%rprim(1,jj)**2 &
&       +results_img(ii,1)%rprim(2,jj)**2 &
&       +results_img(ii,1)%rprim(3,jj)**2)
     end do
     results_img(ii,1)%rprim(1:3,1)=results_img(ii,1)%rprim(1:3,1)/results_img(ii,1)%acell(1)
     results_img(ii,1)%rprim(1:3,2)=results_img(ii,1)%rprim(1:3,2)/results_img(ii,1)%acell(2)
     results_img(ii,1)%rprim(1:3,3)=results_img(ii,1)%rprim(1:3,3)/results_img(ii,1)%acell(3)
   end do
 end if
 ABI_DEALLOCATE(mpibuffer)
