...
!! Gsph_c<Gvectors_type>= info on the G-sphere for Sigma_x
...
!!
!! CHILDREN
!!      calc_coh,calc_wfwfg,clib_progress_bar,cprj_alloc,cprj_copy,cprj_free
!!      destroy_paw_pwij,epsm1_symmetrizer,findqg0,get_bz_item,get_epsm1
!!      get_gftt,gsph_fft_tabs,hermitianize,init_paw_pwij,paw_rho_tw_g
!!      paw_symcprj,print_little_group,rho_tw_g,rotate_fft_mesh
!!      sigma_distribution,symmetrize_me,timab,wfd_change_ngfft,wfd_get_cprj
!!      wfd_get_ur,wrtout,xbarrier_mpi,xgemv,xsum_mpi
!!
!! SOURCE
...
!Local variables ------------------------------
!scalars
 integer,parameter :: level=41,tim_fourdp=2
 integer :: iab,ib,ib1,ib2,ierr,ig,ii,iik,itim_q,i1,i2
 integer :: ik_bz,ik_ibz,io,isym_q,iq_bz,iq_ibz,spin,istat,isym,jb,is_idx
...
&         " band classification encountered the following problem: ",ch10,TRIM(QP_sym(spin)%err_msg)
 ...
 call sigma_distribution(Wfd,Kmesh,Ltg_k,Qmesh,nsppol,can_symmetrize,kgw,Sigp%mg0,my_nbks,&
&  proc_distrb,got,bks_mask,global=.TRUE.)
 ...
 if (mod10==SIG_COHSEX) then  ! Distribute the COHSEX terms, taking into account the symmetries of the Sigma_ij matrix.
   ABI_ALLOCATE(coh_distrb,(ib1:ib2,ib1:ib2,Kmesh%nbz,nsppol))
 ...
 ABI_ALLOCATE(rhotwg_ki,(Sigp%npwc*nspinor,minbnd:maxbnd))
 rhotwg_ki=czero_gw
 ABI_ALLOCATE(rhotwg   ,(Sigp%npwc*nspinor))
 ABI_ALLOCATE(rhotwgp  ,(Sigp%npwc*nspinor))
 ...
 sigctmp=czero_gw
 ABI_ALLOCATE(sigc_ket,(Sigp%npwc*nspinor,nomega_sigc))

 if (mod10==SIG_COHSEX)  then
   ABI_ALLOCATE(wf1swf2_g,(gwc_nfftot*nspinor))
 end if
 ...
 ABI_ALLOCATE(sigsex,(Sigp%npwc))

 !allocate(sigcme_tmp(nomega_sigc,ib1:ib2,ib1:ib2,nsppol*Sigp%nsig_ab))
 sigcme_tmp=czero

 ABI_ALLOCATE(sigc,(2,nomega_sigc,ib1:ib2,ib1:ib2,nsppol*Sigp%nsig_ab))
 sigc=czero
 !
...
!   end if
 ...
 ABI_ALLOCATE(epsm1_qbz,(Sigp%npwc,Sigp%npwc,1))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,"out-of-memory in epsm1_qbz")

 ABI_ALLOCATE(igfftcg0,(Gsph_c%ng))
 !
 ...
 do spin=1,nsppol
   ...
   do ik_bz=1,Kmesh%nbz
     ...
     if (Er%mqmem==0) then ! Read q-slice of epsilon^{-1}|chi0 in Er%epsm1(:,:,:,1) (much slower but less memory).
       call get_epsm1(Er,Vcp,0,0,accesswff,xmpi_self,iqibzA=iq_ibz)
     end if
     !
     ...
     do ib=1,Sigp%nbnds
       ...
       call wfd_get_ur(Wfd,ib,ik_ibz,spin,wfr_sum)
...
&                                        spinor_comm=Wfd%MPI_enreg%comm_spin)
             end if
             !ovlp(2) = -ovlp(1)
...
&              (wtqp+wtqm)*DBLE(sigctmp(:,iab)) + (wtqp-wtqm)*j_gw*AIMAG(sigctmp(:,iab))
 ...
 do spin=1,nsppol
   if (can_symmetrize(spin)) then
     ...
     if (Sigp%gwcalctyp >= 20) then
       call symmetrize_me(QP_sym(spin),ib1,ib2,sigc(:,1,:,:,spin),sym_cme(1,:,:))
     end if
     !
