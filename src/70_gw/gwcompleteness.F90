...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2012 ABINIT group (FB, MG)
...
!!  Gsph_FFT=<Gvectors_type>=Info on the largest G-sphere contained in the FFT box used for wavefunctions.
...
& nfftot,ngfft,igfft0,Gsph_FFT,Ltg_q,green_enhigh_w,wfwfg,chi0)
 ...
 use m_gsphere,        only : gvectors_type, gsph_gmg_idx, gsph_gmg_fftidx
...
!************************************************************************
 ...
 CASE (1) ! Symmetrize the integrand in the full BZ.
 ...
 if (outofbox_wfn/=0) then 
   ...
   if (enough<=50) then 
     ...
     MSG_WARNING(msg)
     if (enough==50) call wrtout(std_out,' ========== Stop writing Warnings ==========','COLL') 
   end if
 end if
...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2012 ABINIT group (FB)
!! This file is distributed under the terms of the
!! GNU General Public License, see ~abinit/COPYING
!! or http://www.gnu.org/copyleft/gpl.txt .
!! For the initials of contributors, see
...
!
!
!call rho_tw_g(paral_kgb,nspinor,npwvec,nr,ngfft,map2sphere,use_padfft,igfftg0,gbound,&
