...
!!
!! COPYRIGHT
!!  Copyright (C) 2008-2012 ABINIT group (MG, GMR, VO, LR, RWG, RS)
...
!!***
 ...
 ! Flags giving the status of the pointers defined in ppmodel_type
...
!!  Procedures that should be modified to reflect the changes done in the datatype have the tag @ppmodel_type.
!!
!! SOURCE

 type,public :: ppmodel_type
...
!arrays
 ...
 complex(gwpc),pointer :: bigomegatwsq(:,:),omegatw(:,:),eigpot(:,:)
! *********************************************************************
 ...
 bigomegatwsq => PPm%bigomegatwsq(iq_curr)%value
 omegatw      => PPm%omegatw(iq_curr)%value
 !
 ! Symmetrize the PPM parameters ----------------------------------------------
 SELECT CASE (PPm%model)

 CASE (PPM_GODBY_NEEDS, PPM_HYBERTSEN_LOUIE) ! * Plasmon pole frequencies otq are invariant under symmetry 
   do jj=1,PPm%npwc
     isg2 = grottb(jj)
     do ii=1,PPm%npwc
       isg1 = grottb(ii)
 ...
 CASE DEFAULT 
   write(msg,'(a,i0)')' Wrong value for PPm%model = ',PPm%model
   MSG_BUG(msg)
 END SELECT
 !
 ! * Take into account time-reversal symmetry.
 if (itim_q==2) botsq=CONJG(botsq)
...
!!
!! SIDE EFFECTS
!!  PPm<PPmodel_type>=All dynamic memory is released.
!!
!! PARENTS
...
! *********************************************************************
 ...
 if (associated(PPm%bigomegatwsq_qbz)) then
   ...
   case (PPM_ISALLOCATED)
     ABI_DEALLOCATE(PPm%bigomegatwsq_qbz%value)
   case (PPM_ISPOINTER)
     nullify(PPm%bigomegatwsq_qbz)
   end select
 end if
 ...
 if (associated(PPm%omegatw_qbz)) then
   ...
   case (PPM_ISALLOCATED)
     ABI_DEALLOCATE(PPm%omegatw_qbz%value)
   case (PPM_ISPOINTER)
     nullify(PPm%omegatw_qbz)
   end select
 end if
 ...
 if (associated(PPm%eigpot_qbz)) then
   ...
   case (PPM_ISALLOCATED)
     ABI_DEALLOCATE(PPm%eigpot_qbz%value)
   case (PPM_ISPOINTER)
     nullify(PPm%eigpot_qbz)
   end select
 end if
 ...
 ABI_DEALLOCATE(PPm%bigomegatwsq)
 ABI_DEALLOCATE(PPm%omegatw)
 ABI_DEALLOCATE(PPm%eigpot)

#else
 if (associated(PPm%bigomegatwsq)) then 
   do iq_ibz=1,dim_q
     ABI_DEALLOCATE(PPm%bigomegatwsq(iq_ibz)%value)
   end do
   ABI_DEALLOCATE(PPm%bigomegatwsq)
 end if
 !
 if (associated(PPm%omegatw)) then 
   do iq_ibz=1,dim_q
     ABI_DEALLOCATE(PPm%omegatw(iq_ibz)%value)
   end do
   ABI_DEALLOCATE(PPm%omegatw)
 end if
 !
 if (associated(PPm%eigpot)) then 
   do iq_ibz=1,dim_q
     ABI_DEALLOCATE(PPm%eigpot(iq_ibz)%value)
   end do
   !call ragged_free(PPm%eigpot)
   ABI_DEALLOCATE(PPm%eigpot)
 end if
#endif

 ! logical flags must be deallocated here.
 if (associated(PPm%keep_q))  then
   ABI_DEALLOCATE(PPm%keep_q)
 end if
 if (associated(PPm%has_q))   then
   ABI_DEALLOCATE(PPm%has_q)
 end if
...
!!****f* m_ppmodel/ppm_table_allocate
!! NAME
!!  ppm_table_allocate
!!
!! FUNCTION
...
!! OUTPUT
!!  istat=Status error
!!  msg_err=String with the error message.
!!
!! SIDE EFFECTS
!!  PPm<ppmodel_type>=PPm tables are allocated.
!!
!! PARENTS
!!      m_ppmodel
!!
...
subroutine ppm_table_allocate(PPm,iq_ibz,msg_err,istat)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'ppm_table_allocate'
!End of the abilint section
...
!Arguments ------------------------------------
 ...
 character(len=*),intent(out) :: msg_err
 type(PPmodel_type),intent(inout) :: PPm

! *********************************************************************

 !@ppmodel_type
 ABI_ALLOCATE(PPm%bigomegatwsq(iq_ibz)%value,(PPm%npwc,PPm%dm2_botsq))
 istat = ABI_ALLOC_STAT
 if (istat/=0) then 
   msg_err = "out of memory bigomegatwsq"
   RETURN
 end if
 !
 ABI_ALLOCATE(PPm%omegatw(iq_ibz)%value,(PPm%npwc,PPm%dm2_otq))
 istat = ABI_ALLOC_STAT
 if (istat/=0) then 
   msg_err = "out of memory omegatw"
   RETURN
 end if
 !
 ABI_ALLOCATE(PPm%eigpot(iq_ibz)%value,(PPm%dm_eig,PPm%dm_eig))
 istat = ABI_ALLOC_STAT
 if (istat/=0) then 
   msg_err = "out of memory eigpot"
 end if

 PPm%has_q(iq_ibz) = PPM_TAB_ALLOCATED

end subroutine ppm_table_allocate
!!***
...
!!
!! SIDE EFFECTS
!!  PPm<ppmodel_type>=PPm tables are deallocated.
!!
!! PARENTS
!!      m_ppmodel
!!
...
! *********************************************************************
 ...
 !@ppmodel_type
 if (associated(PPm%bigomegatwsq)) then
   if (associated(PPm%bigomegatwsq(iq_ibz)%value)) then
    ABI_DEALLOCATE(PPm%bigomegatwsq(iq_ibz)%value)
    !call ragged_free(PPm%bigomegatwsq)
   end if
 end if
 !
 if (associated(PPm%omegatw)) then 
   if (associated(PPm%omegatw(iq_ibz)%value)) then 
    ABI_DEALLOCATE(PPm%omegatw(iq_ibz)%value)
    !call ragged_free(PPm%omegatw)
   end if
 end if
 !
 if (associated(PPm%eigpot)) then 
   if (associated(PPm%eigpot(iq_ibz)%value)) then 
    ABI_DEALLOCATE(PPm%eigpot(iq_ibz)%value)
    !call ragged_free(PPm%eigpot)
   end if
 end if
...
!!
!! SIDE EFFECTS
!!  PPm<PPmodel_type>=Arrays needed to store the ppmodel parameters are allocated with 
...
subroutine ppm_init(PPm,mqmem,nqibz,npwe,ppmodel,drude_plsmf)
...
!Arguments ------------------------------------
 integer,intent(in) :: mqmem,nqibz,npwe,ppmodel
 real(dp),intent(in) :: drude_plsmf
 type(PPmodel_type),intent(out) :: PPm

!Local variables-------------------------------
!scalars
 integer :: dim_q,istat,iq_ibz
 logical :: ltest
 character(len=500) :: msg      
! *********************************************************************
...
! ABI_CHECK(ltest,'Wrong value for mqmem')
 ...
 ABI_ALLOCATE(PPm%keep_q,(nqibz))
 PPm%keep_q=.FALSE.; if (PPm%mqmem>0) PPm%keep_q=.TRUE.

 ABI_ALLOCATE(PPm%has_q,(nqibz))
 PPm%has_q=PPM_NOTAB       
 !
 ! Full q-mesh is stored or out-of-memory solution.
 dim_q=PPm%nqibz; if (PPm%mqmem==0) dim_q=1
                                                    
 ABI_ALLOCATE(PPm%bigomegatwsq,(dim_q))
 ABI_ALLOCATE(PPm%omegatw,(dim_q))
 ABI_ALLOCATE(PPm%eigpot,(dim_q))
 ...
 CASE (PPM_ENGEL_FARID)
   ...
   !
 CASE DEFAULT
   write(msg,'(a,i0)')' Wrong PPm%model = ',PPm%model
   MSG_BUG(msg)
 END SELECT
 !
 ...
 do iq_ibz=1,dim_q
   ...
   ABI_ALLOCATE(PPm%bigomegatwsq(iq_ibz)%value,(PPm%npwc,PPm%dm2_botsq))
   istat = ABI_ALLOC_STAT
   ABI_CHECK(istat==0,"out of memory bigomegatwsq")
   !
   ABI_ALLOCATE(PPm%omegatw(iq_ibz)%value,(PPm%npwc,PPm%dm2_otq))
   istat = ABI_ALLOC_STAT
   ABI_CHECK(istat==0,"out of memory omegatw")
   !
   ABI_ALLOCATE(PPm%eigpot(iq_ibz)%value,(PPm%dm_eig,PPm%dm_eig))
   istat = ABI_ALLOC_STAT
   ABI_CHECK(istat==0,"out of memory eigpot")
   !%endif
#else
   call ppm_table_allocate(PPm,iq_ibz,msg,istat)
   ABI_CHECK(istat==0,msg)
#endif
   !%end if
 end do
...
!!
!! INPUTS
!!  Cryst<crystal_structure>=Info on the unit cell and crystal symmetries.
...
!arrays
 ...
 complex(gwpc),target,intent(in) :: epsm1(:,:,:,:)
...
!scalars  
 real(dp) :: qpt(3)
 complex(gwpc),pointer :: bigomegatwsq(:,:),omegatw(:,:),eigpot(:,:),omegatw_1(:)
 complex(gwpc),pointer :: epsm1_ggw(:,:,:),epsm1_gg(:,:)
! *************************************************************************
 ...
 CASE (PPM_GODBY_NEEDS) 
   ...
   do iq_ibz=1,nqiA
     epsm1_ggw    => epsm1(:,:,:,iq_ibz)

     bigomegatwsq => PPm%bigomegatwsq(iq_ibz)%value
     omegatw      => PPm%omegatw(iq_ibz)%value

     call cppm1par(npwe,nomega,omega,PPm%drude_plsmf,epsm1_ggw,omegatw,bigomegatwsq)
   end do
 ...
 CASE (PPM_HYBERTSEN_LOUIE)
   ...
   do iq_ibz=1,nqiA
     ...
     if (single_q) qpt=Qmesh%ibz(:,iqiA)
     epsm1_gg     => epsm1(:,:,1,iq_ibz)

     bigomegatwsq => PPm%bigomegatwsq(iq_ibz)%value
     omegatw      => PPm%omegatw(iq_ibz)%value

     call cppm2par(qpt,npwe,epsm1_gg,ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,Cryst%gmet,bigomegatwsq,omegatw)
   end do
   !
   ...
   if (PPm%force_plsmf>tol6) then ! Integrate the real-space density
      ...
      do iq_ibz=1,PPm%nqibz
        PPm%bigomegatwsq(iq_ibz)%value = PPm%force_plsmf * PPm%bigomegatwsq(iq_ibz)%value
        PPm%omegatw(iq_ibz)%value      = PPm%force_plsmf * PPm%omegatw(iq_ibz)%value
      end do
 ...
 CASE (PPM_LINDEN_HORSH) ! TODO Check better double precision, this routine is in a messy state
   ...
   do iq_ibz=1,nqiA
     ...
     bigomegatwsq => PPm%bigomegatwsq(iq_ibz)%value
     omegatw_1    => PPm%omegatw(iq_ibz)%value(:,1)
     eigpot       => PPm%eigpot(iq_ibz)%value

     call cppm3par(qpt,npwe,epsm1_gg,ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,bigomegatwsq,omegatw_1,eigpot) 
   end do
 ...
 CASE (PPM_ENGEL_FARID)  ! TODO Check better double precision, this routine is in a messy state
   ...
   do iq_ibz=1,nqiA
     ...
     call cppm4par(qpt,npwe,epsm1_gg,ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,bigomegatwsq,omegatw_1)
   end do
   ...
   do io=1,nomega
     ...
     do ig2=ig2_min,ig2_max
       do ig1=ig1_min,ig1_max
        !den = omega(io)**2-REAL(PPm%omegatw(iqibz)%value(ig1,ig2)**2)
        !if (den**2<zcut**2) den = omega(io)**2-REAL( (PPm%omegatw(iqibz)%value(ig1,ig2)-delta)**2 )
        den = omega(io)**2 - REAL( (PPm%omegatw(iqibz)%value(ig1,ig2)-delta)**2 )
        em1ggp = PPm%bigomegatwsq(iqibz)%value(ig1,ig2)/den
   ...
   do io=1,nomega
     ...
     do ig2=ig2_min,ig2_max
       do ig1=ig1_min,ig1_max
         ...
         em1ggp=czero
         do idm=1,PPm%npwc
           !den=omega(io)**2-(PPm%omegatw(iqibz)%value(ig1,ig2)-delta)**2
           !em1w(io)=em1w(io)+eigvec(ig1,idm,iqibz)*conjg(eigvec(ig2,idm,iqibz))*bigomegatwsq(ig1,ig2,iqibz)/den
           ug1 =PPm%eigpot(iqibz)%value(ig1,idm)
           ug2 =PPm%eigpot(iqibz)%value(ig2,idm)
           otw =PPm%bigomegatwsq(iqibz)%value(idm,1)*PPm%omegatw(iqibz)%value(idm,1)
           zzpq=PPm%bigomegatwsq(iqibz)%value(idm,1)
   ...
   do io=1,nomega
     ...
     do ig2=ig2_min,ig2_max
       ...
       do ig1=ig1_min,ig1_max
         ...
         chig1g2=czero 
         do idm=1,PPm%npwc
           otw =PPm%omegatw(iqibz)%value(idm,1)
           bot1=PPm%bigomegatwsq(iqibz)%value(ig1,idm)
           bot2=PPm%bigomegatwsq(iqibz)%value(ig2,idm)
...
! *************************************************************************
 ...
 ABI_ALLOCATE(em1q,(PPm%npwc,PPm%npwc,nomega))
 ...
 do iomega=1,nomega
   ...
   if (ABS(REAL(omega(iomega)))>0.00001) then
     ...
     lwork=4*2*PPm%npwc
     ABI_ALLOCATE(wwc,(PPm%npwc))
     ABI_ALLOCATE(work,(lwork))
     ABI_ALLOCATE(rwork,(PPm%npwc))
     ABI_ALLOCATE(bwork,(PPm%npwc))
     ABI_ALLOCATE(vs,(PPm%npwc,PPm%npwc))
     istat = ABI_ALLOC_STAT
     ABI_ALLOCATE(Afull,(PPm%npwc,PPm%npwc))
     istat = ABI_ALLOC_STAT
     Afull=em1q(:,:,iomega)
     ...
     ! === Hermitian Case ===
     lwork=2*PPm%npwc-1
     ABI_ALLOCATE(ww,(PPm%npwc))
     ABI_ALLOCATE(work,(lwork))
     ABI_ALLOCATE(rwork,(3*PPm%npwc-2))
     ABI_ALLOCATE(eigvec,(PPm%npwc,PPm%npwc))
     ABI_ALLOCATE(Adpp,(PPm%npwc*(PPm%npwc+1)/2))
     istat = ABI_ALLOC_STAT
     ABI_CHECK(istat==0,"out of memory in Adpp")
...
subroutine cppm2par(qpt,npwc,epsm1,ngfftf,gvec,gprimd,rhor,nfftf,gmet,bigomegatwsq,omegatw)
...
!scalars
 ...
 integer :: ig,igp,istat,nimwp
 integer :: ngfft1,ngfft2,ngfft3,gmgp_idx,ierr
 real(dp) :: lambda,phi
...
!*************************************************************************
 ...
 ABI_ALLOCATE(rhog,(nfftf))
 ngfft1=ngfftf(1) 
 ngfft2=ngfftf(2) 
 ngfft3=ngfftf(3)

 ABI_ALLOCATE(tmp_rhor,(nfftf))
 ...
 ABI_ALLOCATE(rhogg,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,"out of memory rhogg")
...
&   ' Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_ALLOCATE(qplusg,(npwc))
 ABI_ALLOCATE(temp,(npwc,npwc))
 ABI_ALLOCATE(omegatwsq,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'out of memory in omegatwsq')
...
&  '            omega twiddle min location = ',MINLOC(ABS(omegatw))
 ...
 ABI_DEALLOCATE(omegatwsq)
 ABI_DEALLOCATE(rhogg)
 ABI_DEALLOCATE(temp)
...
!scalars
 integer,parameter :: paral_kgb0=0
 integer :: idx,ierr,ig,igp,ii,istat,jj,ngfft1,ngfft2,ngfft3,gmgp_idx
...
!*************************************************************************
 ...
 ABI_ALLOCATE(rhogg,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,"out of memory in rhogg")
 !
...
&   ' Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_ALLOCATE(mm,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'out of memory in mm')
...
&        (gppq(1)*b1(ii)+gppq(2)*b2(ii)+gppq(3)*b3(ii))
 ...
 ABI_ALLOCATE(eigval,(npwc))
 ABI_ALLOCATE(eigvec,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'eigvec out of memory')

 ABI_ALLOCATE(zz,(npwc))
 istat = ABI_ALLOC_STAT
 zz=czero
 ...
 ABI_ALLOCATE(matr,(npwc*(npwc+1)/2))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'matr of memory')
 ...
 ABI_ALLOCATE(zhpev2,(3*npwc-2))
 ABI_ALLOCATE(zhpev1,(2*npwc-1))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'zhpev1 out-of-memory') 
...
!scalars
 integer,parameter :: paral_kgb0=0
 integer :: ierr,ig,igp,ii,istat,ngfft1,ngfft2,ngfft3,gmgp_idx
...
!*************************************************************************
 ...
 ABI_ALLOCATE(rhogg,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'rhogg out of memory')
 !
...
&   ' Enlarge the FFT mesh to get rid of this problem. '
 ...
 ABI_ALLOCATE(mm,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'mm out of memory')
...
&        (gppq(1)*b1(ii)+gppq(2)*b2(ii)+gppq(3)*b3(ii))
 ...
 ABI_ALLOCATE(chitmp,(npwc,npwc))
 ABI_ALLOCATE(chi,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_CHECK(istat==0,'cppm4par out of memory')
 ...
 ABI_ALLOCATE(eigvec,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 ABI_ALLOCATE(mtemp,(npwc,npwc))
 ABI_ALLOCATE(chitmps,(npwc,npwc))
 istat = ABI_ALLOC_STAT
 !
 ...
 eigvec(:,:)=chitmps(:,:)
 ABI_DEALLOCATE(mtemp)
 ABI_DEALLOCATE(chitmps)
 !
 ! === Calculate the plasmon pole parameters ===
 ABI_ALLOCATE(tmp1,(npwc))
 ABI_ALLOCATE(zz2,(npwc,npwc))
 istat = ABI_ALLOC_STAT
...
!scalars
 ...
 real(dp) :: den,ff,omegame0i_io,otw,one_m_theta_mu_minus_e0i
 real(dp) :: twofm1,twofm1_zcut,twofm1_zcut2
 complex(gwpc) :: ct,num,numf,rhotwgdp_igp
 logical :: fully_occupied,totally_empty
 character(len=500) :: msg
!arrays
 complex(gwpc),allocatable :: ket_ig(:),rhotwgdpcc(:) 
...
!*************************************************************************
 ...
 CASE (PPM_GODBY_NEEDS, PPM_HYBERTSEN_LOUIE)
   ...
   do ispinor=1,nspinor
     ...
     if (.not.totally_empty) then  ! \Bomega^2_{G1G2}/\omegat_{G1G2} M_{G1,G2}. \theta(\mu-e_s) / (\omega+\omegat_{G1G2}-e_s-i\delta)
       twofm1_zcut=zcut
         ...
         do igp=1,npwc
           ...
           do ig=1,npwc
             ...
             if (den**2>zcut**2) then
               ket_ig(ig)=ket_ig(ig) + num/(den*otw)
             else
               ket_ig(ig)=ket_ig(ig) + &
&               num*CMPLX(den,twofm1_zcut)/((den**2+twofm1_zcut2)*otw)
             end if
           end do !ig
         end do !igp
         ket(spadc+1:spadc+npwc,ios)=ket(spadc+1:spadc+npwc,ios)+ket_ig(1:npwc)*theta_mu_minus_e0i
       end do !ios
     end if !not totally empty

     if (.not.(fully_occupied)) then ! \Bomega^2_{G1G2}/\omegat_{G1G2} M_{G1,G2}. \theta(e_s-\mu) / (\omega-\omegat_{G1G2}-e_s+i\delta)
       twofm1_zcut=-zcut
       twofm1_zcut2=twofm1_zcut**2
       one_m_theta_mu_minus_e0i=one-theta_mu_minus_e0i
       do ios=1,nomega
         omegame0i_io=omegame0i(ios)
         ket_ig(:)=zero
         do igp=1,npwc
...
&               num*CMPLX(den,twofm1_zcut)/((den**2+twofm1_zcut2)*otw)
         ...
         ket(spadc+1:spadc+npwc,ios)=ket(spadc+1:spadc+npwc,ios)+ket_ig(1:npwc)*one_m_theta_mu_minus_e0i
       end do !ios
     end if !not fully occupied

   end do !ispinor

   ABI_DEALLOCATE(ket_ig)
   ...
   do ios=1,nomega
     ...
     do ii=1,npwc ! Loop over the DM bands
       ...
       if (den**2>zcut**2) then
         ...
         ct=ct+numf*CMPLX(den,twofm1_zcut)/((den**2+twofm1_zcut2))
       end if

     end do !ii DM bands
     sigcme(ios)=ct*half
   end do !ios
   ABI_DEALLOCATE(rhotwgdpcc)
...
!Local variables-------------------------------
!scalars
 integer :: iq_ibz,itim_q,isym_q,iq_curr,istat
 logical :: q_isirred
 character(len=500) :: msg
!arrays
 real(dp) :: qbz(3)
 complex(gwpc),pointer :: botsq(:,:),otq(:,:),eig(:,:)
! *********************************************************************
 ...
 ! =======================================================
 !
 if (PPm%has_q(iq_ibz)==PPM_NOTAB) then ! Allocate the tables for this q_ibz.ABI_ALLOCATE(iq_ibz,)
   call ppm_table_allocate(PPm,iq_ibz,msg,istat)
   ABI_CHECK(istat==0,msg)
 end if

 if (PPm%has_q(iq_ibz)==PPM_TAB_ALLOCATED) then ! Calculate the ppmodel tables for this q_ibz
   call new_setup_ppmodel(PPm,iq_ibz,Cryst,Qmesh,npwe,nomega,omega,epsm1_ggw,nfftf,Gsph%gvec,ngfftf,rhor_tot) !Optional
 end if
 !
 !
 if (q_isirred) then ! Symmetrization is not needed.
   ...
   if (PPm%bigomegatwsq_qbz_stat==PPM_ISPOINTER) then 
      nullify(PPm%bigomegatwsq_qbz)
      ABI_ALLOCATE(PPm%bigomegatwsq_qbz%value,(PPm%npwc,PPm%dm2_botsq))
      istat = ABI_ALLOC_STAT
      ABI_CHECK(istat==0,"out of memory bigomegatwsq")
   ...
   if (PPm%omegatw_qbz_stat==PPM_ISPOINTER) then 
      nullify(PPm%omegatw_qbz)
      ABI_ALLOCATE(PPm%omegatw_qbz%value,(PPm%npwc,PPm%dm2_otq))
      istat = ABI_ALLOC_STAT
      ABI_CHECK(istat==0,"out of memory omegatw")
   ...
   if (PPm%eigpot_qbz_stat==PPM_ISPOINTER) then 
     nullify(PPm%eigpot_qbz)
     ABI_ALLOCATE(PPm%eigpot_qbz%value,(PPm%dm_eig,PPm%dm_eig))
     istat = ABI_ALLOC_STAT
     ABI_CHECK(istat==0,"out of memory eigpot")
   ...
   botsq => PPm%bigomegatwsq_qbz%value
   otq   => PPm%omegatw_qbz%value
   eig   => PPm%eigpot_qbz%value

   call ppm_get_qbz(PPm,Gsph,Qmesh,iq_bz,botsq,otq,eig) 
   !
...
! *************************************************************************
 ...
 CASE (PPM_HYBERTSEN_LOUIE)
   ...
   call cppm2par(qpt,npwe,epsm1_ggw(:,:,1),ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,Cryst%gmet,bigomegatwsq,omegatw)
   !
   ! Quick-and-dirty change of the plasma frequency. Never executed in standard runs.
   if (PPm%force_plsmf>tol6) then ! Integrate the real-space density
      n_at_G_zero = SUM(rhor_tot(:))/nfftf
 ...
 CASE (PPM_LINDEN_HORSH) ! TODO Check better double precision, this routine is in a messy state
   ...
   call cppm3par(qpt,npwe,epsm1_ggw(:,:,1),ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,bigomegatwsq,omegatw_1,eigpot) 
 ...
 CASE (PPM_ENGEL_FARID)  ! TODO Check better double precision, this routine is in a messy state
   ...
   call cppm4par(qpt,npwe,epsm1_ggw(:,:,1),ngfftf,gvec,Cryst%gprimd,rhor_tot,nfftf,bigomegatwsq,omegatw_1)
...
!scalars
 integer :: ig,igp,ii,ios,ispinor,spadc,spadx
 real(dp) :: den,ff,omegame0i_io,one_m_theta_mu_minus_e0i
 real(dp) :: otw,twofm1,twofm1_zcut,twofm1_zcut2
 complex(gwpc) :: ct,num,numf,rhotwgdp_igp
 logical :: fully_occupied,totally_empty
 character(len=500) :: msg
!arrays
...
!*************************************************************************
 ...
 CASE (PPM_GODBY_NEEDS, PPM_HYBERTSEN_LOUIE)
   ...
   ABI_ALLOCATE(ket_ig,(npwc))
   botsq  => PPm%bigomegatwsq_qbz%value !(1:npwc,1:PPm%dm2_botsq)
   otq    => PPm%omegatw_qbz%value      !(1:npwc,1:PPm%dm2_otq)
...
&               num*CMPLX(den,twofm1_zcut)/((den**2+twofm1_zcut2)*otw)
 ...
 CASE (PPM_LINDEN_HORSH,PPM_ENGEL_FARID)
   ...
   botsq  => PPm%bigomegatwsq_qbz%value !(1:npwc,1:PPm%dm2_botsq)
   otq    => PPm%omegatw_qbz%value      !(1:npwc,1:PPm%dm2_otq)
   eig    => PPm%eigpot_qbz%value       !(1:PPm%dm_eig,1:PPm%dm_eig)
   !
   ! * rho-twiddle(G) is formed, introduce rhotwgdpcc, for speed reason
   ABI_ALLOCATE(rhotwgdpcc,(npwx))
