...
!!
!! COPYRIGHT
!!  Copyright (C) 2008-2012 ABINIT group (DRH)
...
!!
!! CHILDREN
!!      bstruct_clean,bstruct_init,destroy_bz_mesh_type,destroy_crystal
!!      init_crystal_from_hdr,init_kmesh,initmpi_seq,listkk,metric
!!      nullify_bz_mesh,rdgw,rdqps,sort_dp,update_cprj,wrtout,xcomm_init
!!      xmaster_init,xme_init,xproc_init,zgemm
!!
!! SOURCE
...
& nkpt,npwarr,nspden,nsppol,ntypat,Hdr,Pawtab,rprimd,MPI_enreg)
 ...
 use m_bz_mesh,        only : bz_mesh_type, init_kmesh, destroy_bz_mesh_type, nullify_bz_mesh
...
!scalars
 ...
 integer :: iatom,iband,icg,icg_shift,ii,ipw,isppol,my_nspinor,nband_k,ord_iband
 ...
 integer :: master,nprocs,spaceComm,rank,ios
...
&  '  No internal check that can verify this is presently possible.',ch10
 ...
 ABI_CHECK(mkmem/=0,'mkmem==0 not implemented')
 !
 ! MPI initialization
 !
 ...
 sppoldbl=1 ; timrev=1 ; my_nspinor=max(1,Dtset%nspinor/MPI_enreg%nproc_spin)
...
&    '  Action : correct input data'
 ...
 call init_crystal_from_hdr(Cryst,Hdr,gw_timrev)
 call nullify_bz_mesh(Kibz_mesh)
 call init_kmesh(Kibz_mesh,Cryst,nkibz,kibz,Dtset%kptopt)
 wtk_ibz=Kibz_mesh%wt
 call destroy_crystal(Cryst)
 call destroy_bz_mesh_type(Kibz_mesh)

 ABI_ALLOCATE(ibz2bz,(nkibz,6))
...
&     '  Action : correct input data'
 ...
 ABI_ALLOCATE(npwarr_ibz,(nkibz))
 ABI_ALLOCATE(istwfk_ibz,(nkibz))
 ABI_ALLOCATE(nband_ibz,(nkibz*nsppol))
 ...
 ABI_ALLOCATE(doccde_ibz,(bantot_ibz))
 ABI_ALLOCATE(eigen_ibz,(bantot_ibz))
 ABI_ALLOCATE(occfact_ibz,(bantot_ibz))
...
&  nkibz,npwarr_ibz,nsppol,Dtset%nspinor,Dtset%tphysel,Dtset%tsmear,Dtset%occopt,occfact_ibz,wtk_ibz)
 ...
 ABI_DEALLOCATE(kibz)
 ABI_DEALLOCATE(wtk_ibz)
 ABI_DEALLOCATE(ibz2bz)
 ABI_DEALLOCATE(npwarr_ibz)
 ABI_DEALLOCATE(istwfk_ibz)
 ABI_DEALLOCATE(nband_ibz)
 ABI_DEALLOCATE(doccde_ibz)
 ABI_DEALLOCATE(eigen_ibz)
 ABI_DEALLOCATE(occfact_ibz)
 ...
 ABI_ALLOCATE(m_lda_to_qp,(mband,mband,dtset%nkptgw,nsppol))
 m_lda_to_qp=czero
 do iband=1,mband
 ...
 if (.not.g0w0_exists) then ! read QPS file (default behavior).
   input = from_QPS_FILE
   ABI_ALLOCATE(prev_Pawrhoij,(Cryst%natom*Dtset%usepaw))
   ABI_ALLOCATE(qp_rhor,(nfftot,nspden*dimrho))
...
&   nfftot,my_ngfft,ucvol,Dtset%paral_kgb,Cryst,Pawtab,MPI_enreg_seq,nbsc,m_lda_to_qp,qp_rhor,prev_Pawrhoij)
 ...
 ABI_ALLOCATE(cg_k,(mpw,mband))
 ABI_ALLOCATE(cg_qpk,(mpw,mband))
 ABI_ALLOCATE(m_tmp,(mband,mband))
...
&       '  and wannier90 datasets.'
    ...
    if (itimrev==0) then
      ...
      write(msg,'(a,i6,a,i6,2a)')&
&       '  Invalid indkk(ikpt,6)',itimrev,'from routine listkk for k-point',ikpt,ch10,&
...
&       " Performing reordering of energies and wavefunctions to be written on the final WKF file."
      ...
      ABI_ALLOCATE(sorted_qpene,(nband_k))
      ABI_ALLOCATE(iord,(nband_k))
 ...
 ABI_DEALLOCATE(cg_k)
 ABI_DEALLOCATE(cg_qpk)
 ABI_DEALLOCATE(m_tmp)
 ...
 if (Dtset%usepaw==1) then
   ...
   nkbz=nkpt
   ABI_ALLOCATE(m_lda_to_qp_BZ,(mband,mband,nkbz,nsppol))
   do isppol=1,nsppol
     do ikbz=1,nkbz
