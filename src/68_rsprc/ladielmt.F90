...
!!
!! FUNCTION
!! Compute a local average of the RPA dielectric matrix
!! throught projection on the wavefunctions.
!!
!!
!! COPYRIGHT
...
!!
!! CHILDREN
!!      fftpac,fourdp,fourwf,hdr_skip,kgindex,leave_new,leave_test,metric
!!      mkffnl,nonlop,out1dm,ph1d3d,rdnpw,rwwf,sphereboundary,timab,wrtout
...
!arrays
 integer,intent(in) :: atindx(dtset%natom),atindx1(dtset%natom)
  ...
  real(dp),intent(in) :: dielmat(2,npwdiel,npwdiel) !!! presently just don't care for the spins
...
!scalars
 ...
 integer :: icg,ider,idir,ierr,ifft,ifft1,ifft2,ii1,ii2,ii3,ikg,ikpt,ilm,index
 integer :: ipw1,ipw2,ispden,isppol,istwf_k,master,matblk,mcg_disk,me,muig,n1
 integer :: n2,n3,n4,n5,n6,nband_k,nkpg,nnlout,npw_k,option,paw_opt,signs
 integer :: spaceComm,tim_fourwf,tim_nonlop,tim_rwwf
 real(dp) :: arg,dummy,min,ucvol,weight
 character(len=500) :: message
 character(len=fnlen) :: filapp
 type(gs_hamiltonian_type) :: gs_hamk
!arrays
 integer :: indpw_prc(npwdiel)
 integer,allocatable :: kg_dum(:,:),kg_k(:,:)
 real(dp) :: cwork1(2,dtset%nfft),enlout(1),gmet(3,3),gprimd(3,3),kpoint(3)
 real(dp) :: lavnlr(dtset%nfft,dtset%nspden),nonlop_dum(1,1),nonlop_dum2(1,1)
 ...
 real(dp) :: ylmgr_dum(1),qphon(3)
 real(dp),allocatable :: bigdielmat(:,:,:),bigrdielmat(:,:,:),buffer(:)
 real(dp),allocatable :: cg_disk(:,:),cwavef(:,:),dummyt2(:,:),dummyt3(:,:,:)
 real(dp),allocatable :: eig_dum(:),eig_k(:),ffnl(:,:,:,:),g2cart(:)
 real(dp),allocatable :: kpg_dum(:,:),lavnl3d(:,:,:),occ_dum(:),occ_k(:)
 real(dp),allocatable :: ph3d(:,:,:),rdielmat(:,:,:,:,:,:,:),resid_k(:)
 real(dp),allocatable :: vnlcwavef(:,:),vnlwfraug(:,:,:,:),wfraug(:,:,:,:)
...
!here we must transfer dielmat to rdielmat:
!from gspace to rspace
!from npwdiel to npw ?? or to n1,n2,n3 ??


!dielmat(2,npwdiel,npwdiel) -> rdielmat(2,n1,n2,n3,n1,n2,n3)
!1)-> bigdielmat(2,nfft,nfft) (g,g')
!2)-> bigrdielmat(2,nfft,nfft) (r,r')
!3)-> rdielmat(2,n1,n2,n3,n1,n2,n3) (x,y,z,x',y',z')


!step 1
 ABI_ALLOCATE(bigdielmat,(2,dtset%nfft,dtset%nfft))
 ABI_ALLOCATE(mask,(npwdiel))
 call kgindex(indpw_prc,kg_diel,mask,mpi_enreg,dtset%ngfft,npwdiel)
 ABI_DEALLOCATE(mask)
 bigdielmat(:,:,:)=zero
 do ipw2=1,npwdiel
   do ipw1=1,npwdiel
     bigdielmat(:,indpw_prc(ipw1),indpw_prc(ipw2))= dielmat(:,ipw1,ipw2)
   end do
 end do
!step 2
 ABI_ALLOCATE(bigrdielmat,(2,dtset%nfft,dtset%nfft))
 do ipw2=1,npwdiel
   call fourdp(2,bigdielmat(:,:,indpw_prc(ipw2)),bigrdielmat(:,:,indpw_prc(ipw2)),&
&   1,mpi_enreg,dtset%nfft,dtset%ngfft,dtset%paral_kgb,0)
 end do
 do ifft=1,dtset%nfft
   cwork1(:,:)=bigrdielmat(:,ifft,:)
   call fourdp(2,cwork1(:,:),bigrdielmat(:,ifft,:),&
&   1,mpi_enreg,dtset%nfft,dtset%ngfft,dtset%paral_kgb,0)
 end do
 ABI_DEALLOCATE(bigdielmat)
!step 3
 ABI_ALLOCATE(rdielmat,(2,n4,n5,n6,n4,n5,n6))
 do i3=1,n3
   do i2=1,n2
     do i1=1,n1
       ifft1=i1+n1*(i2-1+n2*(i3-1))
       do ii3=1,n3
         do ii2=1,n2
           do ii1=1,n1
             ifft2=ii1+n1*(ii2-1+n2*(ii3-1))
             rdielmat(:,i1,i2,i3,ii1,ii2,ii3)=bigrdielmat(:,ifft1,ifft2)
!            rdielmat(:,i1,i2,i3,ii1,ii2,ii3)=&
!            & merge(1.0d0,0.0d0,((i1==ii1).and.(i2==ii2).and.(i3==ii3)))
           end do
         end do
       end do
     end do
...
&        0, &                 ! this is only_SO= flag to get only SO part of V_NL, not scalar relativistic part
...
&        use_gpu_cuda=gs_hamk%use_gpu_cuda) ! governs if we use the gpu or not
...
!        &      istwf_k,kg_k,kg_k,dtset%mgfft,mpi_enreg,1,gs_hamk%ngfft,npw_k,1,n4,n5,n6,0,dtset%paral_kgb,tim_fourwf,dummy,dummy)
...
!        Getting the value of vnlwfraug(r) = sum_r' rdielmat(r,r') * wfraug(r')
         vnlwfraug=zero
         do i3=1,n3
           do i2=1,n2
             do i1=1,n1
               vnlwfraug(1,:,:,:) = vnlwfraug(1,:,:,:) &
&               + rdielmat(1,:,:,:,i1,i2,i3) * wfraug(1,i1,i2,i3)  &
&               - rdielmat(2,:,:,:,i1,i2,i3) * wfraug(2,i1,i2,i3)
               vnlwfraug(2,:,:,:) = vnlwfraug(2,:,:,:) &
&               + rdielmat(1,:,:,:,i1,i2,i3) * wfraug(2,i1,i2,i3)  &
&               + rdielmat(2,:,:,:,i1,i2,i3) * wfraug(1,i1,i2,i3)
...
!    lavnlr(:,ispden) = lavnlr(:,ispden)-min+1.0d0
     ladiel(:)=(lavnlr(:,ispden))

     xred2=dtset%xred_orig(:,:,1)
     write(filapp,'(A15,I3.3)') 'dielmat_avg_1DM'
