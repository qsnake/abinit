...
!!
!! COPYRIGHT
!! Copyright (C) 1998-2012 ABINIT group (DCA, XG, GMR, GZ, MT, FF, DRH)
...
#define ABI_FUNC 'nonlop_pl'
 ...
 use interfaces_65_nonlocal, except_this_one => nonlop_pl
...
!arrays
 ...
 integer,intent(in) :: nattyp(ntypat),ngfft(18),nloalg(5) !,pspso(ntypat) UNUSED
...
!scalars
 ...
 integer :: nproj,nspinso,old_paral_level,rank
...
&   ' nonlop_pl : BUG -',ch10,&
&   '  For nspinortot=2, choice=6 is not yet allowed.'
...
!JWZ, 01-Sep-08
 if (choice==51 .or. choice==52) then
   write(message, '(4a)' ) ch10,&
&   ' nonlop_pl : BUG -',ch10,&
&   '  choice 51 or 52 is not yet allowed.'
   call wrtout(std_out,message,'PERS')
   call leave_new('PERS')
 end if
...
!In sequential, treat the second spinor part first
 if (nspinor==2)then
   ...
   isft = npwin;if (mpi_enreg%nproc_spin>1) isft=0
...
&         '  Their value is ',mincat,' and ',matblk,'.'
       ...
       call ph1d3d(ia3,ia4,kgin,matblk,natom,npwin,&
...
!    Loop on ispinor : 1 for scalar-Relativistic, 2 for spin-orbit
     ...
     do ispinor=1,nspinso
       ...
       if (mpi_enreg%paral_spin==1) ispinor_index=mpi_enreg%me_spin+1
...
!        Only the first spinorial component of vectin is taken into account first
         ispin=1;if (mpi_enreg%paral_spin==1) ispin=ispinor_index
...
!        Parallelism stuff
         ...
         if (mpi_enreg%paral_spin==1) then
           call xcomm_init(mpi_enreg,spaceComm,spaceComm_bandfft=mpi_enreg%comm_spinfft)
           jspin=3-ispin
           gxa(:,:,:,:,ispin)=gxa(:,:,:,:,1)
           gxa(:,:,:,:,jspin)=zero
           if ( ndgxdt>0)then
         ...
         if(ndgxdt>0)call xsum_mpi(dgxdt,spaceComm,ierr)
...
!        Loop over spins:
         do isp=1,nspinor
           ispin=isp;if (mpi_enreg%paral_spin==1) ispin=ispinor_index
...
!                    &            ' nonlop_pl : ERROR -',ch10,&
!                    &            '  ABINIT has been compiled without Legendre polynomial support.'
!                    call wrtout(std_out,message,'PERS')
!                    call leave_new('PERS')
!                    #endif
                   end if !choice==6
...
!DEBUG
!write(std_out,*)'  vectout='
!do ii=1,nspinor*npwout
!write(std_out,'(i3,2es16.6)' )ii,vectout(1:2,ii)
!end do
!ENDDEBUG
 ...
 if (mpi_enreg%paral_spin==1) then
   ...
   call xcomm_init(mpi_enreg,spaceComm,spaceComm_bandfft=mpi_enreg%comm_spin)
   call xsum_mpi(enlout,spaceComm,ierr)
   call xsum_mpi(strsnl,spaceComm,ierr)
   call xsum_mpi(enlk,spaceComm,ierr)
   call xsum_mpi(strsso,spaceComm,ierr)
   mpi_enreg%paral_level=old_paral_level
 end if
...
!  antisymmetric tensor
   if (mpssoang>mpsang) then
     ...
     enlout(1:6)=enlout(1:6)+strssoc(:)/ucvol
   end if
 end if
