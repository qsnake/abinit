...
!!
!! COPYRIGHT
!! Copyright (C) 2011-2012 ABINIT group (FDahm, MT)
...
!Error on bad choice
 if ((choice<0 .or. choice>3).and. choice/=23 .and. choice/=24) then
   write(msg,'(a,i4,a)') '  Does not presently support this choice=',choice,'.'
   MSG_BUG(msg)
 end if
...
& ph3din,ph3dout,signs_,sij,svectout,pi,ucvol,vectin,vectout_)
#endif

 if (choice==1.and.signs==1) then
   if (paw_opt/=3) then
     call dotprod_g(enlout(1),doti,istwf_k,mpi_enreg,npwin*nspinor,1,vectin,vectout_)
   else
     call dotprod_g(enlout(1),doti,istwf_k,mpi_enreg,npwin*nspinor,1,vectin,svectout_)
   end if 
