...
!!!
!! OUTPUT
!!  cprj(ncprj,mcprj) <type(cprj_type)>= projected input wave functions <Proj_i|Cnk> with NL projectors
!!                                       Usually ncprj=natom
!!
!! PARENTS
...
!!      xcomm_init,xdefineoff,xme_init
!!
!! SOURCE
...
!scalars
 ...
 integer,intent(in) :: uncp,unkg,unylm,useylmgr
...
!scalars
 ...
 integer :: matblk,mcg_disk,me_distrb,muig,my_nspinor,n1,n1_2p1,n2,n2_2p1,n3,n3_2p1
...
!Dimensioning and allocation of <p_i|Cnk>
 ...
 if (mkmem==0) then
   call cprj_diskinit_w(atindx_atm,ncprj,iorder_cprj,mkmem,natom,ncpgr,dimlmn,my_nspinor,uncp)
 end if
 ABI_ALLOCATE(cwaveprj,(ncprj,my_nspinor*cprj_bandpp))
...
!Additional statements if band-fft parallelism
 if (nprocband>1) then
   ...
   ABI_ALLOCATE(bufsize,(nprocband*cg_bandpp))
   ABI_ALLOCATE(bufdisp,(nprocband*cg_bandpp))
   ABI_ALLOCATE(bufsize_wf,(nprocband*cg_bandpp))
   ABI_ALLOCATE(bufdisp_wf,(nprocband*cg_bandpp))
 end if
...
&         npw_nk,my_nspinor,ntypat0,phkxred,ph1d_atm,ph3d,ucvol,psps%usepaw,psps%useylm)
       end do
!      Export cwaveprj to big array cprj
       call cprj_put(atindx_atm,cwaveprj,cprj,ncprj,iband_min,ibg,ikpt,iorder_cprj,isppol,&
&       mband,mkmem,mpi_enreg,natom,cprj_bandpp,nband_k,dimlmn,my_nspinor,nsppol,spaceComm_band,uncp,&
&       to_be_gathered=.true.)
