...
!Initial tests
 ...
 if (iat/=0) then
   if (rr<rminl(iat)) then
     fin=.true.
     write(unto,*) 'rr < rmin iat=',iat,' ipos=',ipos
   elseif (rho<aim_rhomin) then
     fin=.true.
     write(unto,*) 'CHARGE LT rhomin ',rho,' < ',aim_rhomin
     if (rho<0._dp) stop 'ERROR RHO < 0 !!!'
   end if
 end if
...
!the principal cycle
 ...
 madw : do while(.not.fin)
   ...
   dg=vnorm(grho,0)
   if (ldeb.or.deb) write(unto,*) 'dg= ',dg
...
!  the time test
   ...
   if (dg<aim_dgmin) then
     write(unto,*) 'gradient < dgmin ',dg,' < ',aim_dgmin
     fin=.true.
     iat=0
     ipos=0
!    testing for the CP
     if (npc>0) then
       ...
       if (ires==0) then
         ...
         if (icpc(npc)==-3) then           ! pseudoatom handling
           npcm3=npcm3+1
           write(unto,*) 'Pseudo-atom founded !!'
         end if

         call vgh_rho(vv,rho,grho,hrho,rr,iat,ipos,slc)
         write(22,formpc) 0,0,(pcrb(jj,npc),jj=1,3),(ev(jj),jj=1,3),icpc(npc),&
&         ev(1)+ev(2)+ev(3),rho
         write(unto,formpc) 0,0,(pcrb(jj,npc),jj=1,3),(ev(jj),jj=1,3),icpc(npc),&
&         ev(1)+ev(2)+ev(3),rho
       else
         write(unto,*) 'C.P. not founded !!'
       end if
     end if
   ...
   hh=h0/dg
   if (ldeb.or.deb) write(unto,*) 'h= ',hh,' h0= ',h0,' dg= ',dg
...
!  
   ...
   do while (((np>npmax).or.(deltar>aim_stmax)).and.(deltar>aim_dmin))
     ...
     if (nit>5) then
       if (deltar>aim_stmax) then
         write(unto,*) 'nit > 5 and deltar > stmax   nit=',nit
       else
         write(unto,*) 'nit > 5 and np > npmax   nit=',nit
       end if
     end if
     do ii=1,3
       vv(ii)=vold(ii)
   ...
   nstep=nstep+1
   if (ldeb.or.deb) write(unto,*) 'h= ',hh
   ...
   if (deb.or.ldeb) then
     write(unto,*) ':POS ',vv
     write(unto,*) ':RBPOS ',vt1
     write(unto,*) ':GRAD ',grho
   end if
   ...
   if (iat /= 0) then
     fin=.true.
     write(unto,*) 'r < rmin iat=',iat,' ipos=',ipos
     cycle madw
   end if

   if (rho<aim_rhomin) then
     fin=.true.
     write(unto,*) 'charge < rhomin ',rho,' < ',aim_rhomin
     if (rho<0._dp) stop 'error RHO < 0 !!!'
     iat=0
     ipos=0
     cycle madw
     ...
     do jj=1,npc
       if (icpc(jj)==(-3)) then
         ...
         if (dist<(aim_dtset%rmin**2*0.1)) then
           ...
           ipos=0
           fin=.true.
           write(unto,*) 'We are inside a pseudo-atom'
           cycle madw
         end if
       end if
     end do
   ...
   if (srch.and.(nsi>=nsimax)) then
     ...
     if (stemp) then
       ...
       if (stemp2) then
         ...
         if (rsmed>xyz) then
           write(unto,*) 'We are inside the surface'
           iat=iatinit
           ipos=iposinit
         else
           write(unto,*) 'We are outside the surface'
           iat=0
           ipos=0
         end if
         fin=.true.
