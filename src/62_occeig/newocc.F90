...
!!  fixmom=if differ from -99.99_dp, fix the magnetic moment (in Bohr magneton)
...
!!
!! PARENTS
!!      fermi,gstate,m_ebands,respfn,suscep,vtorho
!!
!! CHILDREN
!!      getnel,timab,wrtout
!!
...
!scalars
 ...
 real(dp),intent(in) :: fixmom,nelect,stmbias,tphysel,tsmear
...
!Prepare fixed moment calculation
 if(abs(fixmom+99.99_dp)>1.0d-10)then
   sign = 1
   do is = 1, nsppol
     fermihit(is) = fermihi
     fermilot(is) = fermilo
     nelectt(is) = half*(nelect+sign*fixmom)
     sign = -sign
...
!  Usual bissection loop
   do ii=1,niter_max
     fermimid=(fermihi+fermilo)/2.0_dp
...
!    ENDDEBUG
     ...
     if( abs(nelecthi-nelectlo) <= 1.0d-13 .or. &
&     abs(fermihi-fermilo) <= 0.5d-14*abs(fermihi+fermilo) ) exit
...
&     ' newocc : prtvol=0, stop printing more k-point informations'
   ...
   write(message, '(a)' ) '  (2) spin down values  '
   call wrtout(std_out,message,'COLL')
   do ikpt=1,nkpt_eff
     do ii=0,(nband(1)-1)/12
       write(message,'(12f6.3)') &
