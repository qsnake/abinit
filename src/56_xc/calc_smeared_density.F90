...
!type(dataset_type), optional, intent(in) :: dtset
!type(hdr_type), optional, intent(in) :: hdr
...
!debug variables
! integer :: rdwr,fformr,rdwrpaw 
! real(dp) :: etotal
! type(pawrhoij_type) :: pawrhoij_dum
...
!Debug
 ...
 write(msg,'(a,ES29.10E3)') '   Integral of rhotilde(r):', &
& abs_gmgp_sq
 MSG_WARNING(msg)
 abs_gmgp_sq = SUM(ABS(rhor(:)))*ucvol/nfftf
 write(msg,'(a,ES29.10E3)') '       Integral of  rho(r):', &
& abs_gmgp_sq
 MSG_WARNING(msg)
 abs_gmgp_sq = SUM(rhotilder(:)-rhor(:))*ucvol/nfftf
 write(msg,'(a,ES29.10E3)') '   Integral of difference rhotilde(r)-rho(r):', &
& abs_gmgp_sq
 MSG_WARNING(msg)
 abs_gmgp_sq = SUM(ABS(rhotilder(:)-rhor(:)))*ucvol/nfftf
 write(msg,'(a,ES29.10E3)') '   Integral of abs. difference |rhotilde(r)-rho(r)|:', &
& abs_gmgp_sq
 MSG_WARNING(msg)

!TODO output smeared density for debug
!#if 0
!if (present(prtvol)) then
!if ((prtvol>9).and.present(dtset).and.(present(hdr))) then
!rdwr=2 ; fformr=52 ; rdwrpaw=0
!call ioarr(accessfil, rhotilder, dtset, etotal, fformr, '', hdr, mpi_enreg, &
!&               nfftf, pawrhoij_dum, rdwr, rdwrpaw)
!end if
!end if
!#endif
