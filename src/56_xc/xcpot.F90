...
!!     and if mgga,        depsxc(:,3)=d(rho.exc)/d(lapl rho)
...
!!     and if mgga,        depsxc(:,6)=d(rho.exc)/d(lapl rho_up)
...
!!  mgga : true if we use a meta-GGA functional.
...
!Check whether q=0
 ...
 if(ishift==0)then
!  Add the value of depsxc to vxc
   do ispden=1,min(nspden,2)
!    $OMP PARALLEL DO PRIVATE(ifft) &
!    $OMP&SHARED(cplex,depsxc,nfft,vxc)
...
!      Stop the xc timer
!      $OMP PARALLEL DO PRIVATE(ifft) &
!      $OMP&SHARED(cplex,depsxc,ispden,nfft,work)
       do ifft=1,cplex*nfft
...
!        Note : this operation is done on the eventually shifted grid
         if(mgga)then
!          $OMP PARALLEL DO PRIVATE(ifft) &
!          $OMP&SHARED(cplex,idir,ispden,nfft,depsxc,work)
           do ifft=1,cplex*nfft
             if(nspden==1)then
...
!          $OMP END PARALLEL DO
         ...
         if(present(vxctau))then
!          $OMP PARALLEL DO PRIVATE(ifft) &
!          $OMP&SHARED(cplex,idir,ispden,nfft,vxctau,work)
           do ifft=1,cplex*nfft
...
!          $OMP PARALLEL DO PRIVATE(ifft) &
!          $OMP&SHARED(cplex,ispden,nfft,vxctau,work)
           do ifft=1,cplex*nfft
