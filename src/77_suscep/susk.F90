...
!!
!! CHILDREN
!!      fourwf,initmpi_seq,pawsushat,sphereboundary,timab,xallgather_mpi
...
!scalars
 integer,intent(in) :: bdtot_index,extrap,ikpt,isp,lmax_diel,mband,mcg
...
!arrays
 integer,intent(in) :: atindx1(natom),gbound_diel(2*mgfftdiel+8,2),istwfk(nkpt)
...
!scalars
 ...
 integer :: iproc,iproc_fft,ipw,ipw1,ipw2,isp1,isp2,ispinor,istwf_k,iwf,jsp
 integer :: nbdblock,ndatarecv,ndiel1,ndiel2,ndiel3,paral_kgb_diel
...
&     cg_mpi(:,1+(ibdblock-1)*npw_k_loc*nspinor*blocksize+icg_mpi:ibdblock*npw_k_loc*nspinor*blocksize+icg_mpi)
     ...
     blocksize=mpi_enreg%nproc_band
     spaceComm=mpi_enreg%comm_fft
     call xallgatherv_mpi(cwavef_alltoall,2*nspinor*ndatarecv,cwavef_alltoall_gather,&
...
&     0,paral_kgb_diel,tim_fourwf,weight,weight)
...
!      In case of PAW, add compensation charge contribution
       if (usepaw==1.and.extrap==1.and.neglect_pawhat==0) then
         call pawsushat(atindx1,cprj_k,gbound_diel,gylmg_diel,iband,iband,ispinor,ispinor,istwf_k,kg_diel,&
&         lmax_diel,mgfftdiel,mpi_enreg_diel,natom,nband_k,ndiel4,ndiel5,ndiel6,&
...
&       mgfftdiel,mpi_enreg_diel,1,ngfftdiel,1,npwdiel,ndiel4,ndiel5,ndiel6,3,paral_kgb_diel,tim_fourwf,weight,weight)
!      In case of PAW, add compensation charge contribution if not already done
       if (usepaw==1.and.extrap==0.and.neglect_pawhat==0) then
         call pawsushat(atindx1,cprj_k,gbound_diel,gylmg_diel,ibd1,ibd2,ispinor,ispinor,istwf_k,kg_diel,&
&         lmax_diel,mgfftdiel,mpi_enreg_diel,natom,nband_k,ndiel4,ndiel5,ndiel6,&
...
!Stuff for parallelism (bands-FFT)
 if(mpi_enreg%mode_para=='b') then
   ...
   if(occopt>=3)call xsum_mpi(drhode(:,:,isp1:isp2),mpi_enreg%commcart,ier)
   ...
   do i1=isp1,isp2
     ...
     call xsum_mpi(susmat_mpi,mpi_enreg%commcart,ier)
...
!          Performs the Fourier Transform of the product, and store it in wfprod
           ...
           call fourwf(1,rhoaug,dummy,wfprod,wfraug,gbound_diel,gbound_diel,&
&           istwf_k,kg_diel,kg_diel,&
&           mgfftdiel,mpi_enreg,1,ngfftdiel,1,npwdiel,ndiel4,ndiel5,ndiel6,3,paral_kgb_diel,tim_fourwf,weight,weight)
