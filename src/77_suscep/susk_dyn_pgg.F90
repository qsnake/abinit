!{\src2tex{textfont=tt}}
!!****f* ABINIT/susk_dyn_pgg
!! NAME
!! susk_dyn_pgg
!!
!! FUNCTION
!! Compute the squared modulus of the density weighted spin density matrix
!! of one k point from input wavefunctions, band occupations, and k point wts.
!!
!! COPYRIGHT
...
!!        =3 susmat_dyn like 2, but density weighted, needed for PGG-X kernel
...
!! b - Only susopt==3 has been checked. This routine extends susk_dyn
...
!scalars
 ...
 integer :: itask,ndiel1,ndiel2,ndiel3,testocc,tim_fourwf
! integer :: ipw
 real(dp) :: ai,ar,eigdiff,occ_fix,occdiff,tolocc,weight,wght1
...
!DEBUG
!write(std_out,*)'%susk_dyn_pgg: ENTER '
...
!ENDDEBUG
 ...
 if(occopt>=3) then
   write(std_out,*) ' %susk_dyn_pgg: occopt>=3 not implemented: stop ' !MF
   stop
 end if
 ...
 testocc=1
 weight=0._dp
 occ_fix=1._dp !occupation of spin resolved levels for pgg kernel
...
!MF allocate for used bands 'nband_k' onl
...
!DEBUG
!if(extrap==1)then
!write(std_out,*)' %susk_dyn_pgg: extrapolation: weights for diagonal term:'
!write(std_out,*)'      row1: ifreq, wght_dyn | row2: weight (==static case)'
!end if
!ENDDEBUG

!task1: calculate density
!task2: calculate pgg kernel part psi/density
 do itask=1,2 !-----------------------------------------------------
...
!    DEBUG
!    write(std_out,*)' susk_dyn_pgg : will call fourwf for band',iband
!    ENDDEBUG
         ...
         do i3=1,ndiel3
           do i2=1,ndiel2
             do i1=1,ndiel1
               ...
               if(itask==1) then
                 rhoextrap_dyn(1,i1,i2,i3,1)=rhoextrap_dyn(1,i1,i2,i3,1) &
&                 +weight*wfraug(1,i1,i2,i3)
               else
                 wfraug(1,i1,i2,i3)=wfraug(1,i1,i2,i3)/rhoextrap_dyn(1,i1,i2,i3,1)/ucvol

!                DEBUG
!                wfraug(1,i1,i2,i3)=1._dp
!                ENDDEBUG
...
!          DEBUG
!          write(std_out,*) ' %susk_dyn_pgg: show on-diagonal weights:'
...
!        do i2=1,ndiel2
!        do i1=1,ndiel1
!        weight=weight+wfraug(1,i1,i2,i3)
!        end do
!        end do
!        end do
!        write(std_out,'(a,i4,2(1x,es14.6))') 'susk_dyn_pgg: sum wfraug=',iband,&
!        weight/nfftdiel
!        weight=0._dp
!        ENDDEBUG
...
!        For density matrix related case
         if(susopt==2) then
           ifreq=1
           weight=occ_fix*wtk(ikpt)/ucvol  !no 2 for real part, corrsponds to occ=1
           do ipw2=1,npwdiel
...
!  do i2=1,ndiel2
!  do i1=1,ndiel1
!  weight=weight+rhoextrap_dyn(1,i1,i2,i3,1)
!  end do
!  end do
!  end do
!  write(std_out,'(a,i4,2(1x,es14.6))') 'susk_dyn_pgg: sum rhoextrap=',iband,weight/nfftdiel*ucvol
!  weight=0._dp
!  end if
!  ENDDEBUG
...
!DEBUG
!write(std_out,*)' %susk_dyn_pgg: END get real space wavefunctions'
!write(std_out,*)' susk_dyn_pgg : stop '
!stop
!ENDDEBUG
...
!        DEBUG
!        write(std_out,*)'%susk_dyn_pgg : contribution from bands',ibd1,ibd2
...
!          DEBUG
!          write(std_out,*) ' %susk_dyn_pgg: off-diagonal part ibd1, ibd2=',ibd1,ibd2
!          ENDDEBUG

           ifreq=1
           weight=2._dp*occ_fix*wtk(ikpt)/ucvol !2 for pairs of ibd1 and ibd2
           do ipw2=1,npwdiel
             do ipw1=ipw2,npwdiel
               susmat_dyn(1,ipw1,isp,ipw2,isp,ifreq)=susmat_dyn(1,ipw1,isp,ipw2,isp,ifreq)+&
&               weight*(wfprod(1,ipw1)*wfprod(1,ipw2)+wfprod(2,ipw1)*wfprod(2,ipw2))
...
!            DEBUG
!            write(std_out,*) ' %susk_dyn_pgg: show off-diagonal weights:'
...
!            ENDDEBUG
             ...
             ifreq=1
             weight=2._dp*occ_fix*wtk(ikpt)*wtk(ikpt)/ucvol
             do ipw2=1,npwdiel
               do ipw1=ipw2,npwdiel
                 susmat_dyn(1,ipw1,isp,ipw2,isp,ifreq)=susmat_dyn(1,ipw1,isp,ipw2,isp,ifreq)+&
&                 weight*(wfprod(1,ipw1)*wfprod(1,ipw2)+wfprod(2,ipw1)*wfprod(2,ipw2))
...
!  End condition of having more than one band
 ...
 rhoextrap_dyn(:,:,:,:,1)=0.0_dp

!DEBUG
!write(std_out,*)' %susk_dyn_pgg: ! susmat_dyn(): diagonal elements'
!do ifreq=1,nfreq
...
!end do
!end do
!write(std_out,*)' %susk_dyn_pgg : DONE & EXIT'; write(std_out,*)
!ENDDEBUG
