...
!!
!! INPUTS
!!  Wfd(wfs_descriptor)= structure gathering information on wave functions
...
!!  BSt<Bandstructure_type>=Datatype with electronic energies.       
...
& EDIFF_TOL) ! optional
 ...
 use m_fft_mesh,       only : rotate_FFT_mesh, ceigr
...
&                             paw_pwaves_lmn_t, init_paw_pwaves_lmn, destroy_paw_pwaves_lmn, nullify_paw_pwaves_lmn
 use m_wfs,            only : wfd_get_ur, wfs_descriptor, wfd_ug2cprj, wfd_change_ngfft, wfd_paw_get_aeur
...
!scalars
 ...
 integer :: nr1,nr2,nr3,nsym_class,nfft,iat,cplex !,ifgd,nfgd,ifft_sph
 integer :: ii,jj,lmax
...
!arrays
 ...
 type(cprj_type),allocatable :: Cprj_b1(:,:),Cprj_b2(:,:),Cprj_b2rot(:,:)
...
! *************************************************************************
 ...
 ABI_ALLOCATE(irottb,(nfft,Cryst%nsym))
...
&    ' classify_bands will return. Action: change the input variable ngfftf '
 ...
 call init_bands_symmetries(Bsym,kpt,Cryst,only_trace,Wfd%nspinor,first_band,last_band,EDIFF_TOL_,ene_k,tolsym)

 if (Bsym%err_status/=0) then
   write(msg,'(a,i0,a)')" init_bands_symmetries returned err_status= ",Bsym%err_status,&
&    " Band classifications cannot be performed."
 ...
 if (Wfd%usepaw==1) then ! Allocate cprj_k and cprj_krot to store a set of bands for a single (K,SPIN).
   ABI_ALLOCATE(Cprj_b1   ,(Cryst%natom,Wfd%nspinor))
   call cprj_alloc(Cprj_b1,   0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cprj_b2   ,(Cryst%natom,Wfd%nspinor))
   call cprj_alloc(Cprj_b2,   0,Wfd%nlmn_atm)
   ABI_ALLOCATE(Cprj_b2rot,(Cryst%natom,Wfd%nspinor))
   call cprj_alloc(Cprj_b2rot,0,Wfd%nlmn_atm)

   !zarot => Pawang%zarot
   lmax = Pawang%l_max-1
   ABI_ALLOCATE(zarot,(2*lmax+1,2*lmax+1,lmax+1,Cryst%nsym))
   zarot = Pawang%zarot

   ABI_ALLOCATE(tmp_sym,(3,3,Cryst%nsym))
   ...
   call nhatgrid(Cryst%atindx1,Cryst%gmet,Wfd%MPI_enreg,Cryst%natom,Cryst%natom,Cryst%nattyp,Wfd%ngfft,Cryst%ntypat,&
...
&      Psps,Pawtab,Pawrad,Pawfgrtab) 
 ...
 ! ===============================================
 fft_fact=one/nfft 
 ABI_ALLOCATE(ur1,(nfft))
 ABI_ALLOCATE(ur2,(nfft))
 ABI_ALLOCATE(ur2_rot,(nfft))
 !
 ! * Precalculate eig0r = e^{iG0.r} on the FFT mesh.
 ABI_ALLOCATE(eig0r,(nfft,Bsym%nsym_gk))

 do isym=1,Bsym%nsym_gk
   g0=Bsym%g0(:,isym) 
   eig0r(:,isym)=ceigr(g0,nfft,Wfd%ngfft) 
 end do

 if (Bsym%can_use_tr) then
   ABI_ALLOCATE(tr_emig0r,(nfft,Bsym%nsym_trgk))
   do isym=1,Bsym%nsym_trgk
     g0=Bsym%tr_g0(:,isym) 
     tr_emig0r(:,isym)=ceigr(-g0,nfft,Wfd%ngfft) 
   end do
 end if
 !
   ...
   do ib1=ib_start,ib_stop ! First band index in the degenerate set.
     ...
     do ib2=ib_start,ib_stop ! Second band index in the degenerate set.
       ...
       if (ib2==ib1) then
         ...
         if (Wfd%usepaw==1) call cprj_copy(Cprj_b1,Cprj_b2)
       else
         !
         ! debugging: use AE wave on dense FFT mesh.
         if (Wfd%usepaw==1.and.use_paw_aeur) then 
...
&                zarot(:,:,:,isym),Pawtab,Psps,Cprj_b1,Cprj_b2rot,conjg_left=.TRUE.)
 ...
 call finalize_Bands_Sym(Bsym,Wfd%prtvol)

 call print_Bands_Symmetries(Bsym,unit=std_out,prtvol=Wfd%prtvol)
 call print_Bands_Symmetries(Bsym,unit=ab_out ,prtvol=Wfd%prtvol)
 !
 ! ===================
 ! === Free memory ===
 ! ===================
 ...
 ABI_DEALLOCATE(ur1)
 ABI_DEALLOCATE(ur2)
 ABI_DEALLOCATE(ur2_rot)
 ABI_DEALLOCATE(eig0r)
 if (Bsym%can_use_tr)  then
   ABI_DEALLOCATE(tr_emig0r)
 end if

 if (Wfd%usepaw==1) then
   call cprj_free(Cprj_b1)
   ABI_DEALLOCATE(Cprj_b1)
   call cprj_free(Cprj_b2)
   ABI_DEALLOCATE(Cprj_b2)
   call cprj_free(Cprj_b2rot)
   ABI_DEALLOCATE(Cprj_b2rot)
   ABI_DEALLOCATE(zarot)
...
!! Cryst<Crystal_structure>=Datatype gathering info on the unit cell.
!!   typat(natom)=type of each atom.
...
! *************************************************************************
 ...
 !call cprj_copy(cprj_in,cprj_out)
 !RETURN
...
!! Cprj_b1(natom,nspinor)<type(cprj_type)>= projected input wave functions <Proj_i|Cnk> 
!!  with all NL projectors at fixed k-point 
!! Cprj_b2(natom,nspinor)<type(cprj_type)>= projected input wave functions <Proj_i|Cnk> 
!!  with all NL projectors at fixed k-point 
...
!arrays
 ...
 type(Cprj_type),intent(in) :: Cprj_b1(natom,nspinor),Cprj_b2(natom,nspinor)
