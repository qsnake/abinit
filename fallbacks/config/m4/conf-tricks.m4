# -*- Autoconf -*-
#
# Copyright (C) 2006-2011 ABINIT Group (Yann Pouillon)
#
...
AC_DEFUN([ABI_TRICKS_BIGDFT],[
  ...
  CFGFLAGS_BIGDFT="${CFGFLAGS_BIGDFT} --disable-binaries --enable-libbigdft --enable-libpoissonsolver --disable-libxc --without-archives"
  tmpflags_bigdft='--with-moduledir="$(fallbacks_instdir)/include"'
  CFGFLAGS_BIGDFT="${CFGFLAGS_BIGDFT} ${tmpflags_bigdft}"
...
AC_DEFUN([ABI_TRICKS_FOX],[
  ...
  dnl Display changes
  AC_MSG_NOTICE([CFGFLAGS_FOX = ${CFGFLAGS_FOX}])
  AC_MSG_NOTICE([FCFLAGS_FOX  = ${FCFLAGS_FOX}])
]) # ABI_TRICKS_FOX
